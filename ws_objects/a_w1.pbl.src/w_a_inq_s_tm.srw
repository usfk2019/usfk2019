$PBExportHeader$w_a_inq_s_tm.srw
$PBExportComments$single master, tab, multi row Inquiry ( from w_a_s_master)
forward
global type w_a_inq_s_tm from w_a_s_master
end type
type tab_1 from u_tab_inq within w_a_inq_s_tm
end type
type tab_1 from u_tab_inq within w_a_inq_s_tm
end type
type st_horizontal from statictext within w_a_inq_s_tm
end type
end forward
...
global w_a_inq_s_tm w_a_inq_s_tm
...
type dw_cond from w_a_s_master`dw_cond within w_a_inq_s_tm
integer width = 2235
integer taborder = 10
end type

type p_ok from w_a_s_master`p_ok within w_a_inq_s_tm
end type

type p_close from w_a_s_master`p_close within w_a_inq_s_tm
end type

type gb_cond from w_a_s_master`gb_cond within w_a_inq_s_tm
end type

type dw_master from w_a_s_master`dw_master within w_a_inq_s_tm
integer y = 320
integer width = 3008
integer height = 396
integer taborder = 0
...
event ue_tabpage_retrieve;Return 0 //존재하지 않을 시 자손의 윈도의 이벤트로 변환된다.
end event
