$PBExportHeader$w_a_reg_s.srw
$PBExportComments$single master Register ( from w_a_condition)
forward
global type w_a_reg_s from w_a_condition
end type
type dw_detail from u_d_dberr within w_a_reg_s
end type
type p_delete from u_p_delete within w_a_reg_s
end type
type p_insert from u_p_insert within w_a_reg_s
end type
type p_save from u_p_save within w_a_reg_s
end type
type p_reset from u_p_reset within w_a_reg_s
end type
end forward
...
global w_a_reg_s w_a_reg_s

type variables
//NVO For Common Processing
u_cust_db_app iu_cust_db_app

//AncestorReturnValue사용으로 더 이상 필요가 없음.
//Int ii_error_chk
end variables
...
event ue_delete;Constant Integer LI_ERROR = -1
...
type dw_cond from w_a_condition`dw_cond within w_a_reg_s
end type

type p_ok from w_a_condition`p_ok within w_a_reg_s
integer y = 48
end type

type p_close from w_a_condition`p_close within w_a_reg_s
integer y = 48
end type

type gb_cond from w_a_condition`gb_cond within w_a_reg_s
end type
