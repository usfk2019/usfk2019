$PBExportHeader$b0w_reg_connection_rate_1_v20.srw
$PBExportComments$[ssong]접속료 요금정책 정의
forward
global type b0w_reg_connection_rate_1_v20 from w_a_reg_m
end type
type cb_load from commandbutton within b0w_reg_connection_rate_1_v20
end type
type p_saveas from u_p_saveas within b0w_reg_connection_rate_1_v20
end type
type p_fileread from u_p_fileread within b0w_reg_connection_rate_1_v20
end type
end forward
...
global b0w_reg_connection_rate_1_v20 b0w_reg_connection_rate_1_v20

type variables 
String is_priceplan		//Item에 해당하는 Price plan
String is_svccod_control, is_customer_type, is_carrier_type
DataWindowChild idc_itemcod
end variables

forward prototypes
public function integer wfl_get_connection_zone (string as_priceplan, string as_zoncod, ref string as_arezoncod[])
end prototypes
...
public function integer wfl_get_connection_zone (string as_priceplan, string as_zoncod, ref string as_arezoncod[]);Long ll_rows
String ls_zoncod
...
on b0w_reg_connection_rate_1_v20.create
int iCurrent
call super::create
...
on b0w_reg_connection_rate_1_v20.destroy
call super::destroy
...
event open;call super::open;/*--------------------------------------------------------------------------
	Name	:	b0w_reg_zoncst3_v20
	Desc.	:	대역별 표준요율 등록
	Ver.	: 	1.0
	Date	: 	2005.04.15
	Programer: oh hye jin
----------------------------------------------------------------------------*/
String ls_tmp, ls_desc, ls_ref_desc, ls_temp, ls_name[]
...
event type integer ue_extra_insert(long al_insert_row);String ls_filter, ls_parttype, ls_priceplan,  ls_svccod
Long ll_row
...
If ls_priceplan <> 'ALL' Then
	...
	ElseIf ll_row > 0 Then			//첫row의 값을 Setting한다. 
		dw_detail.object.itemcod[al_insert_row] = idc_itemcod.GetItemString(1, "itemcod")
	End if
...
ls_where += " svccod = '" + ls_svccod + "' AND customer_type = '" + is_carrier_type + "' "

If ls_where <> "" Then ls_where += " And "
ls_where += " priceplan = '" + ls_priceplan + "' AND customer_type = '" + is_carrier_type + "' "
...
li_return = wfl_get_connection_zone(ls_priceplan, ls_zoncod, ls_arezoncod[])
...
For ll_row = 1 To ll_rows
	...
	If ls_zoncod = ls_Ozoncod and ls_opendt = ls_Oopendt and ls_tmcod = ls_Otmcod  Then
		f_msg_usr_err(9000, Title, "동일한 대역에 같은 적용 개시일에  같은 시간대가 존재합니다.")
		dw_detail.SetColumn("tmcod")
		dw_detail.SetRow(ll_row)
		dw_detail.ScrollToRow(ll_row)
		dw_detail.SetRedraw(True)
...
//		ls_frpoint2   = String(dw_detail.object.frpoint[ll_rows2])
		...
		If (ls_svccod1 = ls_svccod2) And (ls_priceplan1 = ls_priceplan2) And (ls_zoncod1 = ls_zoncod2) And (ls_tmcod1 = ls_tmcod2) And  (ls_itemcod1 = ls_itemcod2) Then
			If ls_enddt1 >= ls_opendt2 Then
				f_msg_info(9000, Title, "같은 대역[ " + ls_zoncod1 + " ], 같은 시간대[ " + ls_tmcod1 + " ], " &
												+ " 같은 품목[ " + ls_itemcod1 + " ]로 적용개시일이 중복됩니다.")
				Return -2
			End If
		End If
...
type dw_cond from w_a_reg_m`dw_cond within b0w_reg_connection_rate_1_v20
integer x = 41
integer y = 40
integer width = 2464
integer height = 244
string dataobject = "b0dw_reg_cnd_connection_rate_1_v20"
...
type p_ok from w_a_reg_m`p_ok within b0w_reg_connection_rate_1_v20
integer x = 2670
integer y = 44
end type

type p_close from w_a_reg_m`p_close within b0w_reg_connection_rate_1_v20
integer x = 2967
integer y = 44
end type

type gb_cond from w_a_reg_m`gb_cond within b0w_reg_connection_rate_1_v20
integer x = 23
integer width = 2491
integer height = 304
end type

type p_delete from w_a_reg_m`p_delete within b0w_reg_connection_rate_1_v20
integer y = 1600
end type

type p_insert from w_a_reg_m`p_insert within b0w_reg_connection_rate_1_v20
integer y = 1600
end type

type p_save from w_a_reg_m`p_save within b0w_reg_connection_rate_1_v20
integer y = 1600
end type

type dw_detail from w_a_reg_m`dw_detail within b0w_reg_connection_rate_1_v20
integer x = 27
integer y = 312
integer width = 3237
integer height = 1248
string dataobject = "b0dw_reg_det_connection_rate_v20"
end type

event dw_detail::constructor;call super::constructor;dw_detail.SetRowFocusIndicator(off!)
end event

event dw_detail::retrieveend;call super::retrieveend;DataWindowChild ldc, ldc_zoncod
Long ll_row, i
String ls_filter, ls_desc, ls_svccod, ls_priceplan_1
Dec lc_data
//Format 지정
...
Next
	...
	Modify("zoncod.dddw.name=b0dc_dddw_connection_zone_1_v20")
	Modify("zoncod.dddw.DataColumn='zoncod'")
	Modify("zoncod.dddw.DisplayColumn='codenm'")	
...
ls_filter = "svccod = '" + ls_svccod + "' AND priceplan = '" + ls_priceplan_1 + "' "
ldc.SetFilter(ls_filter)			//Filter정함
ldc.Filter()
ldc.SetTransObject(SQLCA)
ll_row =ldc.Retrieve()
...
//	This.Object.itemcod[rowcount] = ''
	Modify("itemcod.dddw.name=b0dc_dddw_item_voice_v20")
...
ElseIf ls_parttype = "S" Then
	...
	Modify("itemcod.dddw.DisplayColumn=''")
//	This.Object.itemcod[rowcount] = ''
	Modify("itemcod.dddw.name=b0dc_dddw_itemcod_voice_v20")
	Modify("itemcod.dddw.DataColumn='itemcod'")
	Modify("itemcod.dddw.DisplayColumn='itemnm'")
...
//가격정책별일때 .. 가격정책별 item가져오기
ll_row = dw_detail.GetChild("itemcod", idc_itemcod)
If ll_row = -1 Then MessageBox("Error", "Not a DataWindowChild22")
...
type p_reset from w_a_reg_m`p_reset within b0w_reg_connection_rate_1_v20
integer x = 1097
integer y = 1600
end type
...
//Open
OpenWithParm(b0w_inq_zoncst3_popup_v20, iu_cust_msg)  //청구 윈도우 연다.
