$PBExportHeader$b0w_reg_pricerate2_1_v20.srw
$PBExportComments$[kem] 서비스별 요율등록 v20
forward
global type b0w_reg_pricerate2_1_v20 from w_a_reg_m_m
end type
end forward

global type b0w_reg_pricerate2_1_v20 from w_a_reg_m_m
end type
global b0w_reg_pricerate2_1_v20 b0w_reg_pricerate2_1_v20

type variables
String is_priceplan		//Price Plan Code 
String is_Month, is_Time, is_Hit, is_DC, is_Packet, is_Sec, is_EA, is_mode, &
       is_mainitem_yn, is_Daily, is_pre_DC, is_pre_sec
...
on b0w_reg_pricerate2_1_v20.create
call super::create
end on

on b0w_reg_pricerate2_1_v20.destroy
call super::destroy
end on
...
is_pre_DC = ls_data[9]    //선납-금액한도월정액
is_pre_Sec= ls_data[10]    //선납-시간한도월정액
...
String ls_tmp[], ls_itemcod_1[], ls_sort, ls_prebil_yn, ls_mainitem_yn
...
For i=1 To ll_rows
	...
	ls_mainitem_yn    = fs_snvl(dw_detail.object.itemmst_mainitem_yn[i], 'N')
	...
	//ohj 2005.03.17 추가
	//ohj 2006.02.22 변경 선납상품 여러개 존재할수 있다.
...
//		End If
		...
		If ls_method <> is_pre_dc And ls_method <> is_pre_sec And ls_method <> is_Month And ls_method <> is_Daily And ls_method <> is_EA  Then
			f_msg_usr_err(9000, Title, "선납상품은 과금방식이 월정액, Daily정액, 판매, 선납-금액한도, 선납-시간한도만 가능합니다.")
			dw_detail.SetRow(i)
			dw_detail.ScrollToRow(i)
			dw_detail.SetColumn("itemcod")
			Return -1
...
//	If ls_method = 'A' Or ls_method = 'M' Then
//		If ldc_validity_term <= 0 Then
//			f_msg_usr_err(200, Title,"사용기간")
//			dw_detail.SetRow(i)
//			dw_detail.ScrollToRow(i)
...
//	
	...
	If ls_method = is_Month Or ls_method = is_daily Or ls_method = is_pre_dc Or ls_method = is_pre_sec Then
...
//		If ls_unitcharge = ""  Or Dec(ls_unitcharge) <= 0 Then
	...
	If ls_method = is_pre_dc Or ls_method = is_pre_sec Then
		...
		If IsNull(ldc_basicrange) Or ldc_basicrange <= 0 Then
			...
			Return -1
		End If
		If ls_mainitem_yn = 'N' Then 
			f_msg_usr_err(9000, Title, "품목이 기본품목이 아니면 과금방식을 선납-시간/금액한도월정액으로 설정할 수 없습니다.")
			dw_detail.setColumn("method")
			dw_detail.setRow(i)
			dw_detail.scrollToRow(i)
			dw_detail.setFocus()
	...
	//시간한도월정액일 경우
	If ls_method = is_Sec Then
//2010.06.30 jhchoi 수정함. voip mobile 이 기본품목은 아닌데 월정액인걸 써야해서리...		
//		If ls_mainitem_yn = 'N' Then 
//			f_msg_usr_err(9000, Title, "품목이 기본품목이 아니면 과금방식을 한도월정액으로 설정할 수 없습니다.")
//			dw_detail.setColumn("method")
//			dw_detail.setRow(i)
...
type dw_cond from w_a_reg_m_m`dw_cond within b0w_reg_pricerate2_1_v20
integer x = 37
integer width = 1353
integer height = 156
string dataobject = "b0dw_cnd_reg_pricerate_v20"
boolean hscrollbar = false
boolean vscrollbar = false
boolean livescroll = false
end type

type p_ok from w_a_reg_m_m`p_ok within b0w_reg_pricerate2_1_v20
integer x = 1541
integer y = 40
end type

type p_close from w_a_reg_m_m`p_close within b0w_reg_pricerate2_1_v20
integer x = 1847
integer y = 40
end type

type gb_cond from w_a_reg_m_m`gb_cond within b0w_reg_pricerate2_1_v20
integer x = 23
integer y = 4
integer width = 1394
integer height = 220
...
type dw_master from w_a_reg_m_m`dw_master within b0w_reg_pricerate2_1_v20
integer x = 23
integer y = 240
integer height = 464
string dataobject = "b0dw_cnd_pricerate_v20"
end type
...
type dw_detail from w_a_reg_m_m`dw_detail within b0w_reg_pricerate2_1_v20
integer x = 23
integer y = 740
integer height = 888
string dataobject = "b0dw_reg_pricerate_v20"
end type
...
For i = 1 To dw_detail.RowCount()
	...
	If This.Object.method[i] = is_pre_dc  Then This.Object.tmp[i] = "W"
	If This.Object.method[i] = is_pre_sec Then This.Object.tmp[i] = "H"
...
Long ll_null
String ls_fromdt, ls_prebil_yn, ls_itemcode, ls_mainitem_yn
...
ElseIf dwo.name = "itemcod" Then
	...
	dw_detail.Object.itemmst_mainitem_yn[row] = ls_mainitem_yn	
...
ElseIf dwo.name = "method" Then
	...
	If data = is_Month  Then 
		...
		This.Object.validity_Term[row]  =0
	...
	//시간한도월정액
	If data = is_Sec Then	
//2010.06.30 jhchoi 수정. voip Mobile 이란 놈 때문에...기본품목이 아닌데 한도월정액을 해야하기 때문에 어쩔 수 없이...		
//		If is_mainitem_yn = 'N' Then
//			f_msg_usr_err(9000, Title, "품목이 기본품목이 아니면 과금방식을 한도월정액으로 설정할 수 없습니다.")
//			dw_detail.setColumn("method")
//			dw_detail.setRow(row)
...
//		End If
	...
	If data = is_EA Then
		...
		This.Object.validity_Term[row]  = 1
	End If
	ls_mainitem_yn = fs_snvl(dw_detail.Object.itemmst_mainitem_yn[row], '')
	ls_prebil_yn = fs_snvl(dw_detail.Object.itemmst_prebil_yn[row], '')
	//선납 - 금액한도월정액 선납품목일때만 선택가능하다	
	If data = is_pre_dc  Then 
		If ls_prebil_yn <> 'Y' Then
			f_msg_usr_err(9000, Title, "품목이 선납품목이 아니면 과금방식을 선납-금액한도월정액으로 설정할 수 없습니다.")
			dw_detail.setColumn("method")
			dw_detail.setRow(row)
			dw_detail.scrollToRow(row)
			dw_detail.setFocus()
		...
		If is_mainitem_yn = 'N' Then
			f_msg_usr_err(9000, Title, "품목이 기본품목이 아니면 과금방식을 선납-금액한도월정액으로 설정할 수 없습니다.")
			dw_detail.setColumn("method")
			dw_detail.setRow(row)
			dw_detail.scrollToRow(row)
			dw_detail.setFocus()
	...
	//선납 - 시간한도월정액 선납품목일때만 선택가능하다	
	If data = is_pre_sec  Then 
		If ls_prebil_yn <> 'Y' Then
			f_msg_usr_err(9000, Title, "품목이 선납품목이 아니면 과금방식을 선납-시간한도월정액으로 설정할 수 없습니다.")
			dw_detail.setColumn("method")
			dw_detail.setRow(row)
			dw_detail.scrollToRow(row)
			dw_detail.setFocus()
		...
		If is_mainitem_yn = 'N' Then
			f_msg_usr_err(9000, Title, "품목이 기본품목이 아니면 과금방식을 선납-시간한도월정액으로 설정할 수 없습니다.")
			dw_detail.setColumn("method")
			dw_detail.setRow(row)
			dw_detail.scrollToRow(row)
			dw_detail.setFocus()
...
type p_insert from w_a_reg_m_m`p_insert within b0w_reg_pricerate2_1_v20
end type

type p_delete from w_a_reg_m_m`p_delete within b0w_reg_pricerate2_1_v20
end type

type p_save from w_a_reg_m_m`p_save within b0w_reg_pricerate2_1_v20
end type

type p_reset from w_a_reg_m_m`p_reset within b0w_reg_pricerate2_1_v20
integer x = 1353
end type

type st_horizontal from w_a_reg_m_m`st_horizontal within b0w_reg_pricerate2_1_v20
integer y = 708
end type
