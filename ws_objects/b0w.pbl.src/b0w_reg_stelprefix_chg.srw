$PBExportHeader$b0w_reg_stelprefix_chg.srw
$PBExportComments$[kjm] 발신지 번호 보정 등록
forward
global type b0w_reg_stelprefix_chg from w_a_reg_m
end type
end forward
...
global b0w_reg_stelprefix_chg b0w_reg_stelprefix_chg

on b0w_reg_stelprefix_chg.create
call super::create
end on

on b0w_reg_stelprefix_chg.destroy
call super::destroy
end on

event open;call super::open;/*------------------------------------------------------------------------
	Name : b0w_reg_stelprefix_chg
	Desc. : 발신지번호보정등록
	Date : 2004.08.12
	Auth : Kwon Jung Min
------------------------------------------------------------------------*/
...
Long		ll_rows
String	ls_stelprefix

ls_stelprefix	= Trim( dw_cond.Object.stelprefix[1] )
...
IF ls_stelprefix <> "" THEN	
	...
	ls_where += "STELPREFIX LIKE '%" + ls_stelprefix + "%'"
END IF
...
String ls_stelprefix,	ls_fromdt,	ls_todt,		ls_chgvalue

// 발신지Prefix별 적용시작일, 적용종료일 check variable
Long ll_chk_row1, ll_chk_row2  
String ls_stelprefix1,	ls_stelprefix2,	ls_fromdt1,	ls_fromdt2,	ls_todt1,	ls_todt2

ll_tot_row = dw_detail.RowCount()

// 필수 입력 사항 check -> 발신지Prefix, 적용시작일, 변경Prefix
FOR ll_row = 1 TO ll_tot_row
	ls_stelprefix = dw_detail.Object.stelprefix[ll_row]
...
FOR ll_chk_row1 = 1 TO dw_detail.RowCount()
	ls_stelprefix1= dw_detail.Object.stelprefix[ll_chk_row1]
	...
	FOR ll_chk_row2 = dw_detail.RowCount() TO ll_chk_row1 -1 Step -1
		...
		ls_stelprefix2 = dw_detail.Object.stelprefix[ll_chk_row2]
		...
		IF ls_stelprefix1 = ls_stelprefix2 THEN
			
			If (ls_todt1 >= ls_fromdt2 AND ls_todt1 <> "") OR (ls_todt2 >= ls_fromdt2 AND ls_todt2 <> "")Then
				f_msg_info(9000, Title, "같은 발신지Prefix[ " + ls_stelprefix1 + " ]로 적용기간이 중복됩니다.")				
				Return -2	
			END IF
		END IF
...
type dw_cond from w_a_reg_m`dw_cond within b0w_reg_stelprefix_chg
integer y = 68
integer width = 1294
integer height = 132
integer taborder = 10
string dataobject = "b0dw_cnd_stelprefix_chg"
boolean vscrollbar = false
end type

type p_ok from w_a_reg_m`p_ok within b0w_reg_stelprefix_chg
integer x = 1431
integer y = 72
end type

type p_close from w_a_reg_m`p_close within b0w_reg_stelprefix_chg
integer x = 1737
integer y = 72
end type

type gb_cond from w_a_reg_m`gb_cond within b0w_reg_stelprefix_chg
integer width = 1344
integer height = 220
integer taborder = 30
end type

type p_delete from w_a_reg_m`p_delete within b0w_reg_stelprefix_chg
end type

type p_insert from w_a_reg_m`p_insert within b0w_reg_stelprefix_chg
end type

type p_save from w_a_reg_m`p_save within b0w_reg_stelprefix_chg
end type

type dw_detail from w_a_reg_m`dw_detail within b0w_reg_stelprefix_chg
integer y = 240
integer width = 2194
integer height = 1336
integer taborder = 20
string dataobject = "b0dw_reg_stelprefix_chg"
end type

event dw_detail::constructor;call super::constructor;SetRowFocusIndicator(off!)
end event

type p_reset from w_a_reg_m`p_reset within b0w_reg_stelprefix_chg
end type
