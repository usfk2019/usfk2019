$PBExportHeader$b0w_reg_zone_cost.srw
$PBExportComments$[kem] 원가대역코드 등록 v20
forward
global type b0w_reg_zone_cost from w_a_reg_m
end type
end forward
...
global b0w_reg_zone_cost b0w_reg_zone_cost

on b0w_reg_zone_cost.create
call super::create
end on

on b0w_reg_zone_cost.destroy
call super::destroy
end on
...
For ll_i = 1 To ll_row
	ls_zoncod = Trim(dw_detail.Object.zoncod_cost[ll_i])
	ls_zonnm = Trim(dw_detail.Object.zonnm_cost[ll_i])	
	
	If IsNull(ls_zoncod) Then ls_zoncod = ""
	If IsNull(ls_zonnm) Then ls_zonnm = ""
	
	If ls_zoncod = "" Then 
		f_msg_usr_err(200, Title, "원가대역코드")
	...
	End If
	If ls_zonnm = "" Then 
		f_msg_usr_err(200, Title, "원가대역명")
		dw_detail.SetRow(ll_i)
		dw_detail.ScrollToRow(ll_i)
		dw_detail.SetColumn("zonnm_cost")
		Return -1
	End If
...
event open;call super::open;/*------------------------------------------------------------------------
	Name 	: b0w_reg_zone_cost
	Desc.	: 원가대역코드 등록
	Ver 	: 1.0
	Date	: 2005.06.13
	Progrmaer: Kim, EunMi(kem)
...
type dw_cond from w_a_reg_m`dw_cond within b0w_reg_zone_cost
integer x = 41
integer y = 44
integer width = 1157
integer height = 140
string dataobject = "b0dw_cnd_reg_zone_cost"
boolean hscrollbar = false
boolean vscrollbar = false
boolean livescroll = false
end type

type p_ok from w_a_reg_m`p_ok within b0w_reg_zone_cost
integer x = 1275
integer y = 44
end type

type p_close from w_a_reg_m`p_close within b0w_reg_zone_cost
integer x = 1573
integer y = 44
boolean originalsize = false
end type

type gb_cond from w_a_reg_m`gb_cond within b0w_reg_zone_cost
integer x = 23
integer width = 1202
integer height = 196
end type

type p_delete from w_a_reg_m`p_delete within b0w_reg_zone_cost
integer x = 315
integer y = 1700
end type

type p_insert from w_a_reg_m`p_insert within b0w_reg_zone_cost
integer x = 23
integer y = 1700
end type

type p_save from w_a_reg_m`p_save within b0w_reg_zone_cost
integer x = 608
integer y = 1700
end type

type dw_detail from w_a_reg_m`dw_detail within b0w_reg_zone_cost
integer x = 23
integer y = 220
integer width = 1829
integer height = 1444
string dataobject = "b0dw_reg_zone_cost"
boolean livescroll = false
end type
...
type p_reset from w_a_reg_m`p_reset within b0w_reg_zone_cost
integer x = 1193
integer y = 1700
end type
