$PBExportHeader$b1w_inq_modified_log.srw
$PBExportComments$[jybaek] 수정이력
forward
global type b1w_inq_modified_log from w_a_inq_m
end type
type st_vertical from statictext within b1w_inq_modified_log
end type
type dw_log from u_d_base within b1w_inq_modified_log
end type
type tv_item from treeview within b1w_inq_modified_log
end type
end forward
...
global b1w_inq_modified_log b1w_inq_modified_log
...
String ls_chg_item, ls_tablenm, ls_pgm_item, ls_codenm
...
For ll_row = 1 To al_rows
	...
	If ls_new1 <> ls_old1 Then
		...
		ls_data = ls_tablenm + "`" + ls_chg_item
		If lb_skip Then
...
on b1w_inq_modified_log.create
int iCurrent
call super::create
...
on b1w_inq_modified_log.destroy
call super::destroy
...
String	ls_where
String	ls_tablenm, ls_chg_item, ls_emp_id
...
If ls_tablenm <> "" Then
	...
	ls_where += "substr(syslog1t.tablenm, 1, 10) = '" + Upper(ls_tablenm) + "' "
End If
If ls_chg_item <> "" Then
	If ls_where <> "" Then ls_where += " and "
	ls_where += "syslog1t.chg_item = '" + ls_chg_item + "' "
End If
If ls_emp_id <> "" Then
	If ls_where <> "" Then ls_where += " and "
...
type dw_cond from w_a_inq_m`dw_cond within b1w_inq_modified_log
integer x = 37
integer y = 40
integer width = 2514
integer height = 192
string dataobject = "b1d_cnd_inq_modified_log"
boolean hscrollbar = false
boolean vscrollbar = false
boolean livescroll = false
end type

type p_ok from w_a_inq_m`p_ok within b1w_inq_modified_log
integer x = 2651
integer y = 40
end type

type p_close from w_a_inq_m`p_close within b1w_inq_modified_log
integer x = 2944
integer y = 40
end type

type gb_cond from w_a_inq_m`gb_cond within b1w_inq_modified_log
integer x = 18
integer width = 2551
integer height = 244
end type

type dw_detail from w_a_inq_m`dw_detail within b1w_inq_modified_log
integer x = 1051
integer y = 256
integer width = 2258
integer height = 1572
string dataobject = "b1d_inq_modified_log"
end type
...
integer taborder = 11
boolean bringtotop = true
string dataobject = "b1d_inq_modified_log_dts"
end type
