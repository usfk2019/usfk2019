$PBExportHeader$b1w_reg_actorder_pre_batch_1.srw
$PBExportComments$[kem] 신규고객개통신청처리Batch
forward
global type b1w_reg_actorder_pre_batch_1 from w_a_inq_m
end type
type p_fileread from u_p_fileread within b1w_reg_actorder_pre_batch_1
end type
type p_reset from u_p_reset within b1w_reg_actorder_pre_batch_1
end type
type p_insert from u_p_insert within b1w_reg_actorder_pre_batch_1
end type
type p_delete from u_p_delete within b1w_reg_actorder_pre_batch_1
end type
end forward
...
global b1w_reg_actorder_pre_batch_1 b1w_reg_actorder_pre_batch_1
...
on b1w_reg_actorder_pre_batch_1.create
int iCurrent
call super::create
...
on b1w_reg_actorder_pre_batch_1.destroy
call super::destroy
...
String ls_sysdt, ls_activedt, ls_birthdt, ls_validkey1[]
...
IF ls_uni_check = "Duplicate" THEN 
	...
	For j = 1 To ll_rows
		ls_validkey1[j] = dw_detail.object.validkey[j]
		CHOOSE CASE j
			CASE 1  // Row가 1행일때 
				IF  ll_rows > 1 THEN	ll_long = dw_detail.Find(" validkey = '" + ls_validkey1[j] + "'", j + 1, ll_rows)
				If ll_long > 0 Then
			...
			CASE ll_rows // Row가 맨 마지막
				IF  ll_rows > 1 THEN	ll_long = dw_detail.Find(" validkey = '" + ls_validkey1[j] + "'", 1, j - 1)
				If ll_long > 0 Then
					f_msg_usr_err(210, Title,"인증Key가 중복되었습니다.")
					dw_detail.SetFocus()
					dw_detail.ScrollToRow(ll_long)
			...
			CASE ELSE	  
				IF ll_rows > 1 THEN
					ll_long = dw_detail.Find(" validkey = '" + ls_validkey1[j] + "'", 1, j -1)
					If ll_long > 0 Then
						f_msg_usr_err(210, Title,"인증Key가 중복되었습니다.")
						dw_detail.SetFocus()
						dw_detail.ScrollToRow(ll_long)
						...
						Return
					Else
						ll_long = dw_detail.Find(" validkey = '" + ls_validkey1[j] + "'", j + 1, ll_rows)
						If ll_long > 0 Then
							f_msg_usr_err(210, Title,"인증Key가 중복되었습니다.")
							dw_detail.SetFocus()
							dw_detail.ScrollToRow(ll_long)
...
END IF											
	...
	String ls_validkey, ls_password, ls_enterdt, ls_pricemodel
	String ls_reg, ls_ref_desc, ls_bil_fromdt
...
lu_dbmgr3.is_caller = "b1w_reg_actorder_pre_batch_1%ok"
...
type dw_cond from w_a_inq_m`dw_cond within b1w_reg_actorder_pre_batch_1
integer x = 55
integer y = 52
integer width = 2542
integer height = 412
string dataobject = "b1dw_cnd_actorder_pre_batch_1"
...
type p_ok from w_a_inq_m`p_ok within b1w_reg_actorder_pre_batch_1
integer x = 3346
integer y = 132
end type

type p_close from w_a_inq_m`p_close within b1w_reg_actorder_pre_batch_1
integer x = 3351
integer y = 248
end type

type gb_cond from w_a_inq_m`gb_cond within b1w_reg_actorder_pre_batch_1
integer width = 2587
integer height = 488
end type

type dw_detail from w_a_inq_m`dw_detail within b1w_reg_actorder_pre_batch_1
integer x = 32
integer y = 520
integer width = 3685
integer height = 1184
string dataobject = "b1dw_actorder_pre_batch_1"
