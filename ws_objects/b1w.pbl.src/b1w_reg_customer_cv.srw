$PBExportHeader$b1w_reg_customer_cv.srw
$PBExportComments$[ceusee] 고객정보 등록- CVnet
forward
global type b1w_reg_customer_cv from w_a_reg_m_tm2
end type
type p_view from u_p_view within b1w_reg_customer_cv
end type
end forward
...
global b1w_reg_customer_cv b1w_reg_customer_cv
...
Long il_row
String is_zipcod, is_addr_1, is_addr_2, is_aptname
...
OpenWithParm(b1w_inq_inv_detail_pop_cv, iu_cust_msg, gw_mdi_frame)
...
on b1w_reg_customer_cv.destroy
call super::destroy
destroy(this.p_view)
end on
...
Choose Case li_tab
	...
	Case 2		//Tab 2 청구지정보
		...
		//주소자동입력
		tab_1.idw_tabpage[li_tab].object.bil_zipcod[1] = is_zipcod
		tab_1.idw_tabpage[li_tab].object.bil_addr1[1] = is_addr_1
		tab_1.idw_tabpage[li_tab].object.bil_addr2[1] = is_addr_2
...
Choose Case li_tab
	Case 1
		...
		ls_buildingno = tab_1.idw_tabpage[li_tab].object.buildingno[1]
		ls_roomno = tab_1.idw_tabpage[li_tab].object.roomno[1]
		
		//아파트 동, 호 대문자로 변경
		ls_buildingno = upper(ls_buildingno)
		ls_roomno = upper(ls_roomno)
		
		//아파트 동, 호가 0~9999 사이일경우 4자리로 맞춘다.
		...
		tab_1.idw_tabpage[li_tab].object.buildingno[1] = ls_buildingno
		tab_1.idw_tabpage[li_tab].object.roomno[1] = ls_roomno
		
		
		
		lu_check.is_caller = "b1w_reg_customer%save_tab1_cv"
		lu_check.is_title = Title
		lu_check.ii_data[1] = li_tab
...
If li_selectedTab = 1 Then						//Tab 1
	...
	If ib_billing Then						
		...
		End If
  End If
ElseIf li_selectedTab = 2 Then	//Tab2
...
type dw_cond from w_a_reg_m_tm2`dw_cond within b1w_reg_customer_cv
integer y = 60
integer width = 2706
integer height = 212
string dataobject = "b1dw_cnd_reg_customer_cv"
end type
...
type p_ok from w_a_reg_m_tm2`p_ok within b1w_reg_customer_cv
integer x = 2898
integer y = 40
end type

type p_close from w_a_reg_m_tm2`p_close within b1w_reg_customer_cv
integer x = 3200
integer y = 40
end type

type gb_cond from w_a_reg_m_tm2`gb_cond within b1w_reg_customer_cv
integer x = 37
integer width = 2752
integer height = 292
end type

type dw_master from w_a_reg_m_tm2`dw_master within b1w_reg_customer_cv
integer y = 324
integer width = 3534
integer height = 540
string dataobject = "b1dw_inq_customer_cv"
end type
...
type p_insert from w_a_reg_m_tm2`p_insert within b1w_reg_customer_cv
integer y = 2056
end type

type p_delete from w_a_reg_m_tm2`p_delete within b1w_reg_customer_cv
integer y = 2056
end type

type p_save from w_a_reg_m_tm2`p_save within b1w_reg_customer_cv
integer x = 622
integer y = 2056
end type

type p_reset from w_a_reg_m_tm2`p_reset within b1w_reg_customer_cv
integer x = 1207
integer y = 2056
end type

type tab_1 from w_a_reg_m_tm2`tab_1 within b1w_reg_customer_cv
integer y = 912
integer width = 3534
integer height = 1116
end type
...
//Tab에 해당하는 dw
is_dwObject[1] = "b1dw_reg_customer_t1_cv"
...
Choose Case ai_tabpage
	...
	Case 3
		...
		OpenWithParm(b1w_reg_validinfo_popup_update, iu_cust_msg)
...
Integer li_exist
Integer li_rc
String ls_zipcod, ls_addr_1, ls_buildingno, ls_roomno, ls_aptname
...
Choose Case tab_1.SelectedTab
	Case 1		//Tab 1
     ...
     Choose Case dwo.name
			Case "location"
				//아파트 정보가 수정되면 주소를 자동으로 변경한다.
				IF NOT IsNull(data) AND data <> "" THEN
					SELECT name, zipcode, addr_1
					INTO :is_aptname,:ls_zipcod,:ls_addr_1
					FROM apartments
					WHERE aptid = :data;
					
					IF LenA(ls_zipcod) = 7 THEN
						ls_zipcod = MidA(ls_zipcod,1,3) + MidA(ls_zipcod,5,3)
					END IF
					
					//IF SQLCA.SQLCODE > 0 THEN
						tab_1.idw_tabpage[1].Object.zipcod[row] = ls_zipcod
						tab_1.idw_tabpage[1].Object.addr1[row] = ls_addr_1
						tab_1.idw_tabpage[1].Object.addr2[row] = is_aptname
						is_zipcod = ls_zipcod
						is_addr_1 = ls_addr_1
					//END IF
				END IF
			...
			Case "buildingno"
				...
				//동이 입력되면 나머지주소를 변경한다.
				IF NOT IsNull(data) AND data <> "" THEN
					ls_roomno = tab_1.idw_tabpage[1].Object.roomno[row]
					IF ls_roomno <> "" THEN
						tab_1.idw_tabpage[1].Object.addr2[row] = is_aptname + " " + UPPER(data) + "동 " + UPPER(ls_roomno) + "호"
						is_addr_2 = is_aptname + " " + UPPER(data) + "동 " + UPPER(ls_roomno) + "호"
					END IF
				END IF
			...
			Case "roomno"
				...
				//호가 입력되면 나머지주소를 변경한다.
				IF NOT IsNull(data) AND data <> "" THEN
					ls_buildingno = UPPER(tab_1.idw_tabpage[1].Object.buildingno[row])
					IF ls_buildingno <> "" THEN
						tab_1.idw_tabpage[1].Object.addr2[row] = is_aptname + " " + UPPER(ls_buildingno) + "동 " + UPPER(data) + "호"
						is_addr_2 = is_aptname + " " + UPPER(ls_buildingno) + "동 " + UPPER(data) + "호"
					END IF
				END IF
...
type st_horizontal from w_a_reg_m_tm2`st_horizontal within b1w_reg_customer_cv
integer y = 868
end type
