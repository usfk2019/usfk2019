$PBExportHeader$b1w_reg_quotainfo_pop_cl.srw
$PBExportComments$[kem] 서비스 신청시 장비/할부 등록
forward
global type b1w_reg_quotainfo_pop_cl from w_a_reg_m
end type
type dw_detail2 from datawindow within b1w_reg_quotainfo_pop_cl
end type
end forward
...
global b1w_reg_quotainfo_pop_cl b1w_reg_quotainfo_pop_cl
...
//장비정보, 미수금정보 가져오기
lu_dbmgr.is_caller = "b1w_reg_quotainfo_pop_cl%inq"
...
on b1w_reg_quotainfo_pop_cl.create
int iCurrent
call super::create
...
on b1w_reg_quotainfo_pop_cl.destroy
call super::destroy
destroy(this.dw_detail2)
end on

event open;call super::open;/*------------------------------------------------------------------------
	Name	:	b1w_reg_quotainfo_pop_cl
...
//window 중앙에
f_center_window(b1w_reg_quotainfo_pop_cl)
...
lu_dbmgr = Create b1u_dbmgr
lu_dbmgr.is_caller = "b1w_reg_quotainfo_pop_cl%getdata"
...
If li_cnt > 0 Then
	...
	//할부 개월수 계산
	lu_dbmgr.is_caller = "b1w_reg_quotainfo_pop_cl%insert"
	lu_dbmgr.is_title = Title
...
//장비 등록
lu_dbmgr = Create b1u_dbmgr
lu_dbmgr.is_caller = "b1w_reg_quotainfo_pop_cl%hw_save"
lu_dbmgr.is_title = Title
lu_dbmgr.is_data[1] = is_customerid
lu_dbmgr.is_data[2] = is_main_itemcod
lu_dbmgr.is_data[3] = is_orderno
...
type dw_cond from w_a_reg_m`dw_cond within b1w_reg_quotainfo_pop_cl
integer x = 46
integer y = 40
integer width = 2962
integer height = 704
...
type p_ok from w_a_reg_m`p_ok within b1w_reg_quotainfo_pop_cl
integer x = 3058
integer y = 56
boolean originalsize = false
end type

type p_close from w_a_reg_m`p_close within b1w_reg_quotainfo_pop_cl
integer x = 3058
integer y = 180
boolean originalsize = false
end type

type gb_cond from w_a_reg_m`gb_cond within b1w_reg_quotainfo_pop_cl
integer width = 2994
integer height = 760
end type

type p_delete from w_a_reg_m`p_delete within b1w_reg_quotainfo_pop_cl
boolean visible = false
integer x = 1010
integer y = 1888
end type

type p_insert from w_a_reg_m`p_insert within b1w_reg_quotainfo_pop_cl
boolean visible = false
integer x = 709
integer y = 1888
end type

type p_save from w_a_reg_m`p_save within b1w_reg_quotainfo_pop_cl
integer x = 50
integer y = 1896
end type

type dw_detail from w_a_reg_m`dw_detail within b1w_reg_quotainfo_pop_cl
integer x = 23
integer y = 1124
integer width = 3342
integer height = 764
...
type p_reset from w_a_reg_m`p_reset within b1w_reg_quotainfo_pop_cl
integer x = 352
integer y = 1896
