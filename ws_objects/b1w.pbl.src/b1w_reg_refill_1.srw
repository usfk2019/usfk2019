$PBExportHeader$b1w_reg_refill_1.srw
$PBExportComments$[juede] 선불제수동충전 - 기본료적용
forward
global type b1w_reg_refill_1 from w_a_reg_m_sql
end type
end forward
...
global b1w_reg_refill_1 b1w_reg_refill_1
...
on b1w_reg_refill_1.create
call super::create
end on

on b1w_reg_refill_1.destroy
call super::destroy
end on
...
If ll_row > 0 Then
	...
	If ll_priceplan_cnt > 1 then
		f_msg_usr_err(2100,Title, "서로 다른 가격정책이 존재하므로 선택하신 데이타를 충전할 수 없습니다.")
...
lu_dbmgr.is_caller = "b1w_reg_refill_new%save"
...
lu_dbmgr.ic_data[3] = ic_basic_rate       //기본료율
lu_dbmgr.ic_data[4] = ic_basic_fee        //기본료금액
lu_dbmgr.il_data[1] = il_extdays          //연장일수
...
type dw_cond from w_a_reg_m_sql`dw_cond within b1w_reg_refill_1
integer y = 40
integer width = 2181
integer height = 936
...
event dw_cond::itemchanged;DataWindowChild ldc_partner_prefix
Long ll_row, li_return
Dec ldc_rate
Dec{2} lc_sale_amt, lc_amt, ldc_refill_amt
String ls_partner, ls_priceplan, ls_refilldt, ls_refill_type, ls_opendt
//2005.07.31 juede add --start
Dec  lc_basic_rate  
ldc_refill_amt =0
//2005.07.31 juede add --end
...
type p_ok from w_a_reg_m_sql`p_ok within b1w_reg_refill_1
integer x = 2386
boolean originalsize = false
end type

type p_close from w_a_reg_m_sql`p_close within b1w_reg_refill_1
integer x = 2386
end type

type gb_cond from w_a_reg_m_sql`gb_cond within b1w_reg_refill_1
integer width = 2240
integer height = 988
end type

type p_save from w_a_reg_m_sql`p_save within b1w_reg_refill_1
integer x = 2386
end type

type dw_detail from w_a_reg_m_sql`dw_detail within b1w_reg_refill_1
integer y = 1012
integer width = 2935
integer height = 1332
