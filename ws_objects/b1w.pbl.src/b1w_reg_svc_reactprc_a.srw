$PBExportHeader$b1w_reg_svc_reactprc_a.srw
$PBExportComments$[jhchoi] 서비스 일시정지 처리 - 2009.04.15
forward
global type b1w_reg_svc_reactprc_a from w_a_reg_m_m
end type
type dw_detail2 from u_d_base within b1w_reg_svc_reactprc_a
end type
type dw_detail3 from u_d_base within b1w_reg_svc_reactprc_a
end type
type dw_detail4 from datawindow within b1w_reg_svc_reactprc_a
end type
type gb_1 from groupbox within b1w_reg_svc_reactprc_a
end type
type gb_2 from groupbox within b1w_reg_svc_reactprc_a
end type
type gb_3 from groupbox within b1w_reg_svc_reactprc_a
end type
type gb_4 from groupbox within b1w_reg_svc_reactprc_a
end type
end forward
...
global b1w_reg_svc_reactprc_a b1w_reg_svc_reactprc_a
...
IF ll_not_cnt <= 0  and ls_svccod <> '940CT' THEN
	FOR ii = 1 TO ll_row
	...
	IF ll_cnt > 0 THEN
		F_MSG_INFO(9000, Title, "인증장비 해소 처리 오류입니다.")
		ai_return = -1
		RETURN  
	END IF
END IF	
...
on b1w_reg_svc_reactprc_a.create
int iCurrent
call super::create
...
on b1w_reg_svc_reactprc_a.destroy
call super::destroy
...
// Desciption : 서비스 일시정지 해소 처리	                 //
// Name       : b1w_reg_svc_reactprc_b		                 //
// Contents   : 서비스 일시정지를 해소 처리한다.			  //
// Data Window: dw - b1dw_cnd_reg_suspend	       		     // 
//							b1dw_inq_svc_reactprc		  			  //
//							b1dw_inq_termorder_popup				  //
//							b1dw_inq_validkey				 			  //
//							b1dw_inq_equipment				  		  //
//							b1dw_reg_svc_reactprc_a				  	  //
// 작성일자   : 2009.04.20                                 //
// 작 성 자   : 최재혁 대리                                //
// 수정일자   :                                            //
// 수 정 자   :                                            //
...
//중지처리 가능한 인증상태
ls_ref_desc = ""
is_equipstatus = fs_get_control("U0", "E200", ls_ref_desc)		
...
STRING	ls_orderdt,		ls_requestdt,		ls_partner,		ls_where,		ls_ref_desc,		ls_react_status,	&
...
If ls_orderdt <> "" Then
	If ls_where <> "" Then ls_where += " AND "
//	ls_where += " SVC.ORDERDT <= TO_DATE('" + ls_orderdt + "', 'YYYYMMDD') "
...
If ll_row = 0 Then
	f_msg_info(1000, Title, "Reactivation Order")
...
event ue_extra_save;STRING	ls_actdt,			ls_contractseq,  		ls_remark,			ls_errmsg,		ls_fromdt
DATE		ld_actdt
...
ll_return  = 0
//ll_orderno = 0
SQLCA.UBS_REG_REACTIVEORDER(ls_contractseq,	ll_orderno,		ld_actdt,		'prc',		ls_remark,  &
...
IF li_rc = -1 OR li_rc = -2 THEN
	...
	IF iu_cust_db_app.ii_rc = -1 THEN
		...
		RETURN LI_ERROR
	END IF
	f_msg_info(3010,THIS.Title,"재개통 처리")
	RETURN LI_ERROR
ELSEIF li_rc = 0 THEN
	//COMMIT와 동일한 기능
	...
	IF iu_cust_db_app.ii_rc = -1 THEN
		...
		RETURN LI_ERROR
	END IF
	f_msg_info(3000,THIS.Title,"재개통 처리")
...
type dw_cond from w_a_reg_m_m`dw_cond within b1w_reg_svc_reactprc_a
integer y = 40
integer width = 3310
integer height = 240
...
type p_ok from w_a_reg_m_m`p_ok within b1w_reg_svc_reactprc_a
integer x = 3401
integer y = 88
end type

type p_close from w_a_reg_m_m`p_close within b1w_reg_svc_reactprc_a
integer x = 681
integer y = 1900
end type

type gb_cond from w_a_reg_m_m`gb_cond within b1w_reg_svc_reactprc_a
integer width = 3694
integer taborder = 0
end type

type dw_master from w_a_reg_m_m`dw_master within b1w_reg_svc_reactprc_a
integer x = 27
integer y = 312
integer width = 3698
integer height = 556
...
type dw_detail from w_a_reg_m_m`dw_detail within b1w_reg_svc_reactprc_a
integer x = 1755
integer y = 940
integer width = 1943
integer height = 904
integer taborder = 50
string dataobject = "b1dw_reg_svc_reactprc_a"
boolean hscrollbar = false
boolean vscrollbar = false
...
event type integer dw_detail::ue_retrieve(long al_select_row);STRING	ls_where,			ls_contractseq,		ls_termtype,		ls_partner,		&
			ls_partnernm,		ls_orderno,				ls_susseq
LONG		ll_row
DEC		ldc_contractseq
DATETIME ldt_termdt
...
THIS.object.actdt[1] = fdt_get_dbserver_now() //This.object.svcorder_requestdt[1]
...
type p_insert from w_a_reg_m_m`p_insert within b1w_reg_svc_reactprc_a
boolean visible = false
integer y = 1816
end type

type p_delete from w_a_reg_m_m`p_delete within b1w_reg_svc_reactprc_a
boolean visible = false
integer y = 1816
end type

type p_save from w_a_reg_m_m`p_save within b1w_reg_svc_reactprc_a
integer x = 59
integer y = 1900
end type

type p_reset from w_a_reg_m_m`p_reset within b1w_reg_svc_reactprc_a
integer x = 370
integer y = 1900
end type

type st_horizontal from w_a_reg_m_m`st_horizontal within b1w_reg_svc_reactprc_a
boolean visible = false
integer y = 844
end type
