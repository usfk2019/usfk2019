$PBExportHeader$b1w_reg_svcorder_update.srw
$PBExportComments$[ceusee] 신청 내역 수정
forward
global type b1w_reg_svcorder_update from w_a_reg_s
end type
end forward
...
global b1w_reg_svcorder_update b1w_reg_svcorder_update

type variables
String is_actorder, is_date_check, is_cancel
end variables

on b1w_reg_svcorder_update.create
call super::create
end on

on b1w_reg_svcorder_update.destroy
call super::destroy
end on

event open;call super::open;f_center_window(b1w_reg_svcorder_update)
...
event type integer ue_extra_save();call super::ue_extra_save;String ls_reqdt, ls_partner, ls_sale_partner, ls_reg_partner, ls_maintain_partner
String ls_sysdt, ls_customerid
...
If dw_detail.object.status[1] = is_actorder or &
	...
	If fb_reqdt_check(Title,ls_customerid,ls_reqdt,"신청요청일") Then
	Else
		dw_detail.SetFocus()
		dw_detail.SetColumn("requestdt")
		Return -1
...
type dw_cond from w_a_reg_s`dw_cond within b1w_reg_svcorder_update
boolean visible = false
end type

type p_ok from w_a_reg_s`p_ok within b1w_reg_svcorder_update
boolean visible = false
end type

type p_close from w_a_reg_s`p_close within b1w_reg_svcorder_update
integer x = 2437
integer y = 720
end type

type gb_cond from w_a_reg_s`gb_cond within b1w_reg_svcorder_update
boolean visible = false
end type

type dw_detail from w_a_reg_s`dw_detail within b1w_reg_svcorder_update
integer y = 32
integer width = 2711
integer height = 640
string dataobject = "b1dw_reg_svcorder_update"
...
type p_delete from w_a_reg_s`p_delete within b1w_reg_svcorder_update
boolean visible = false
integer y = 744
end type

type p_insert from w_a_reg_s`p_insert within b1w_reg_svcorder_update
boolean visible = false
integer y = 744
end type

type p_save from w_a_reg_s`p_save within b1w_reg_svcorder_update
integer x = 2126
integer y = 720
end type

type p_reset from w_a_reg_s`p_reset within b1w_reg_svcorder_update
boolean visible = false
integer y = 744
end type
