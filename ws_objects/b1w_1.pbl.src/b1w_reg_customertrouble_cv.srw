$PBExportHeader$b1w_reg_customertrouble_cv.srw
$PBExportComments$[chooys] 고객 민원접수 및 처리 cvnet
forward
global type b1w_reg_customertrouble_cv from w_a_reg_m_tm2
end type
end forward
...
global b1w_reg_customertrouble_cv b1w_reg_customertrouble_cv
...
on b1w_reg_customertrouble_cv.create
int iCurrent
call super::create
end on

on b1w_reg_customertrouble_cv.destroy
call super::destroy
end on
...
Else
		...
		IF ls_buildingno <> "" Then 
			If ls_where <> "" Then ls_where += " And "
			ls_where += "customerm.buildingno like '%" + ls_buildingno + "' "
		End If
		
		IF ls_roomno <> "" Then 
			If ls_where <> "" Then ls_where += " And "
			ls_where += "customerm.roomno like '%" + ls_roomno + "' "
		END IF
...
type dw_cond from w_a_reg_m_tm2`dw_cond within b1w_reg_customertrouble_cv
integer x = 41
integer y = 44
integer width = 2523
integer height = 372
string dataobject = "b1dw_cnd_reg_customeridtrouble_cv"
...
type p_ok from w_a_reg_m_tm2`p_ok within b1w_reg_customertrouble_cv
integer x = 2693
end type

type p_close from w_a_reg_m_tm2`p_close within b1w_reg_customertrouble_cv
integer x = 2994
boolean originalsize = false
end type

type gb_cond from w_a_reg_m_tm2`gb_cond within b1w_reg_customertrouble_cv
integer width = 2551
integer height = 436
end type

type dw_master from w_a_reg_m_tm2`dw_master within b1w_reg_customertrouble_cv
integer y = 468
integer width = 3209
integer height = 484
string dataobject = "b1dw_reg_customeridtrouble_m_cv"
end type
...
type p_insert from w_a_reg_m_tm2`p_insert within b1w_reg_customertrouble_cv
integer y = 1940
end type

type p_delete from w_a_reg_m_tm2`p_delete within b1w_reg_customertrouble_cv
integer y = 1940
end type

type p_save from w_a_reg_m_tm2`p_save within b1w_reg_customertrouble_cv
integer y = 1940
end type

type p_reset from w_a_reg_m_tm2`p_reset within b1w_reg_customertrouble_cv
integer y = 1940
end type

type tab_1 from w_a_reg_m_tm2`tab_1 within b1w_reg_customertrouble_cv
integer y = 984
integer width = 3209
	...
	Case 1														//Tab 1
		Choose Case dwo.name
			...
			Case "troubletypeb_troubletypec"
				...
				//해당 priceplan에 대한 itemcod
				ll_row = tab_1.idw_tabpage[1].GetChild("troubletypea_troubletypeb", ldc)
				If ll_row = -1 Then MessageBox("Error", "Not a DataWindowChild")
			...
			Case "troubletypemst_troubletypea"
				...
				//해당 priceplan에 대한 itemcod
				ll_row = tab_1.idw_tabpage[1].GetChild("customer_trouble_troubletype", ldc)
				If ll_row = -1 Then MessageBox("Error", "Not a DataWindowChild")
...
type st_horizontal from w_a_reg_m_tm2`st_horizontal within b1w_reg_customertrouble_cv
integer y = 952
end type
