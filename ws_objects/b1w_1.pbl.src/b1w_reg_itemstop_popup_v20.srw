$PBExportHeader$b1w_reg_itemstop_popup_v20.srw
$PBExportComments$[ohj] 품목사용정지 popup v20
forward
global type b1w_reg_itemstop_popup_v20 from w_a_reg_m
end type
type gb_1 from gb_cond within b1w_reg_itemstop_popup_v20
end type
end forward
...
global b1w_reg_itemstop_popup_v20 b1w_reg_itemstop_popup_v20

type variables
String is_pgm_gu, is_itemcod, is_priceplan, is_item[]
Long il_contractseq//, il_grouptype
Date id_cont_fromdt, id_reqdt_end, id_reqdt
...
on b1w_reg_itemstop_popup_v20.create
int iCurrent
call super::create
...
on b1w_reg_itemstop_popup_v20.destroy
call super::destroy
destroy(this.gb_1)
end on

event open;call super::open;/*------------------------------------------------------------------------
	Name	:	b1w_reg_itemstop_popup_v20
	Desc	: 	품목이용중지
	Ver.	:	1.0
	Date	: 	2005.05.21
	programer : ohj
...
String   ls_where, ls_ref_desc, ls_temp, ls_requestdt, ls_method, ls_bilfromdt, &
         ls_fromdt, ls_todt
Date     ldt_bilfromdt, ldt_date_next, ldt_date_next_1, ld_frdt, ld_todt

//window 중앙에
f_center_window(b1w_reg_itemstop_popup_v20)
...
ld_frdt     	= iu_cust_msg.id_data[4]  
ld_todt		   = iu_cust_msg.id_data[5]
...
//ls_cu_date = string(fdt_get_dbserver_now(), 'yyyymmdd')
//j = 1
...
If isnull(ld_frdt) = False Then
	dw_detail.Object.bil_fromdt[1]  = ld_frdt//date(ls_fromdt)
End If

If isnull(ld_todt) = False Then
	dw_detail.Object.bil_todt[1] = ld_todt//date(ls_todt)
Else
	dw_detail.Object.bil_todt[1] = fdt_get_dbserver_now()
End If
...
If newheight < (dw_detail.Y + iu_cust_w_resize.ii_button_space) Then
	...
	p_delete.Y	= dw_detail.Y + iu_cust_w_resize.ii_dw_button_space   + 35
	...
	p_delete.Y	= newheight - iu_cust_w_resize.ii_button_space  + 35
...
event type integer ue_extra_save();Long    ll_row, ll_cnt, ll_type, i
String ls_bil_fromdt, ls_cont_fromdt, ls_bil_todt
Date   ld_reqdt_pre
...
ls_bil_todt    = fs_snvl(string(dw_detail.object.bil_todt[1]  , 'yyyymmdd'), '')
ls_cont_fromdt = string(id_cont_fromdt, 'yyyymmdd')

IF ls_bil_todt = '' Then
	f_msg_usr_err(210, Title, "과금종료일을 입력하세요!")
	dw_detail.SetColumn("bil_todt")	
	return -1	
End If

If ls_bil_fromdt > ls_bil_todt Then
...
//	f_msg_usr_err(210, Title, "과금시작일은 해당고객 청구마감일이후 이여야 합니다..")	
//	dw_detail.SetColumn("bil_fromdt")
//	return -1
//End If

ld_reqdt_pre = fd_date_pre(id_reqdt,1) //2006.02.08 juede add

//If ls_bil_todt < string(id_reqdt, 'yyyymmdd') Then  //2006.02.08 juede comment
If ls_bil_todt < String(ld_reqdt_pre,'yyyymmdd') Then  //2006.02.08 juede modify (청구기준일 하루전날까지)
	f_msg_usr_err(210, Title, "과금종료일은 해당고객 청구마감일이후 이여야 합니다..")	
	dw_detail.SetColumn("bil_todt")
	return -1
End If
...
//		If ll_type = 0 Then
//			f_msg_info(9000, Title, " 선택한 품목의 선택유형이 0입니다. ~r"  &
//			                      + " 기존에 등록된 품목중 같은 유형이 존재하여 추가 할 수 없습니다." )
//			Return -1
//		End If
//	Next		
//	
...
event type integer ue_extra_delete();call super::ue_extra_delete;Long    ll_row, ll_cnt, ll_type, i
String ls_bil_fromdt, ls_cont_fromdt, ls_bil_todt
...
//If ls_bil_todt < string(id_reqdt, 'yyyymmdd') Then
//	f_msg_usr_err(210, Title, "과금종료일은 해당고객 청구마감일이후 이여야 합니다..")	
//	dw_detail.SetColumn("bil_todt")
//	return -1
//End If
...
event type integer ue_delete();If This.Trigger Event ue_extra_delete() < 0 Then
	Return -1
End if
...
If SQLCA.SQLCode < 0 Then
	f_msg_sql_err(title, "DELETE Error(CONTRACTDET)")
	RollBack;
	Return -1
End If	
...
type dw_cond from w_a_reg_m`dw_cond within b1w_reg_itemstop_popup_v20
...
type p_ok from w_a_reg_m`p_ok within b1w_reg_itemstop_popup_v20
boolean visible = false
integer x = 585
integer y = 1228
end type

type p_close from w_a_reg_m`p_close within b1w_reg_itemstop_popup_v20
integer x = 859
integer y = 428
boolean originalsize = false
end type

type gb_cond from w_a_reg_m`gb_cond within b1w_reg_itemstop_popup_v20
boolean visible = false
integer x = 14
integer y = 552
end type

type p_delete from w_a_reg_m`p_delete within b1w_reg_itemstop_popup_v20
integer x = 247
integer y = 428
boolean enabled = true
end type

type p_insert from w_a_reg_m`p_insert within b1w_reg_itemstop_popup_v20
boolean visible = false
end type

type p_save from w_a_reg_m`p_save within b1w_reg_itemstop_popup_v20
integer x = 553
integer y = 428
boolean enabled = true
end type

type dw_detail from w_a_reg_m`dw_detail within b1w_reg_itemstop_popup_v20
integer x = 64
integer y = 88
integer width = 1029
integer height = 184
integer taborder = 20
string dataobject = "b1dw_reg_itemstop_v20"
boolean hscrollbar = false
boolean vscrollbar = false
...
type p_reset from w_a_reg_m`p_reset within b1w_reg_itemstop_popup_v20
boolean visible = false
integer x = 270
integer y = 1224
boolean enabled = true
