$PBExportHeader$b1w_reg_priceplan_keytype_v20.srw
$PBExportComments$[ohj]가격정책별 인증 key type v20
forward
global type b1w_reg_priceplan_keytype_v20 from w_a_reg_m
end type
end forward

global type b1w_reg_priceplan_keytype_v20 from w_a_reg_m
end type
global b1w_reg_priceplan_keytype_v20 b1w_reg_priceplan_keytype_v20

on b1w_reg_priceplan_keytype_v20.create
call super::create
end on

on b1w_reg_priceplan_keytype_v20.destroy
call super::destroy
end on
...
String	ls_priceplan, ls_validkey_type, ls_priceplan1, ls_validkey_type1, &
         ls_priceplan2, ls_validkey_type2, ls_type, ls_type2
...
For ll_rows1 = 1 To dw_detail.RowCount()
	...
	ls_priceplan1     = fs_snvl(Trim(dw_detail.object.priceplan_validkey_type_priceplan[ll_rows1]), '')
	ls_validkey_type1	= fs_snvl(Trim(dw_detail.Object.priceplan_validkey_type_validkey_type[ll_rows1]), '')
	ls_type = ''
	
	select type
	  into :ls_type
	  from validkey_type
	 where validkey_type = :ls_validkey_type1  ;
	
	If ls_type = 'VALIDITEM' Then
		For ll_rows2 = dw_detail.RowCount() To ll_rows1 - 1 Step -1
			If ll_rows1 = ll_rows2 Then
				Exit
			End If
			
			ls_priceplan2     = fs_snvl(Trim(dw_detail.object.priceplan_validkey_type_priceplan[ll_rows2]), '')
			ls_validkey_type2	= fs_snvl(Trim(dw_detail.Object.priceplan_validkey_type_validkey_type[ll_rows2]), '')
			ls_type2 = ''
			
			select type
			  into :ls_type2
			  from validkey_type
			 where validkey_type = :ls_validkey_type2  ;
			 
			If ls_type2 = 'VALIDITEM' Then 
				If ls_priceplan1 = ls_priceplan2 Then
					f_msg_info(9000, Title, "Type 구분이[ VALIDITEM ]인 인증key type를 하나이상 선택하셨습니다. ")
					dw_detail.SetColumn("priceplan_validkey_type_validkey_type")
					Return -2
				End If
			End If
	...
	IF dw_detail.GetItemStatus(ll_rowcnt,0,Primary!) = DataModified! THEN
		dw_detail.Object.priceplan_validkey_type_updt_user[ll_rows1]	= gs_user_id
		dw_detail.Object.priceplan_validkey_type_updtdt[ll_rows1]		= fdt_get_dbserver_now()
	END IF	
...
type dw_cond from w_a_reg_m`dw_cond within b1w_reg_priceplan_keytype_v20
integer x = 78
integer y = 76
integer width = 2240
integer height = 200
string dataobject = "b1dw_cnd_reg_priceplan_validkeytype_v20"
boolean hscrollbar = false
boolean vscrollbar = false
boolean livescroll = false
end type

type p_ok from w_a_reg_m`p_ok within b1w_reg_priceplan_keytype_v20
integer x = 2464
integer y = 52
end type

type p_close from w_a_reg_m`p_close within b1w_reg_priceplan_keytype_v20
integer x = 2770
integer y = 52
end type

type gb_cond from w_a_reg_m`gb_cond within b1w_reg_priceplan_keytype_v20
end type

type p_delete from w_a_reg_m`p_delete within b1w_reg_priceplan_keytype_v20
end type

type p_insert from w_a_reg_m`p_insert within b1w_reg_priceplan_keytype_v20
end type

type p_save from w_a_reg_m`p_save within b1w_reg_priceplan_keytype_v20
end type

type dw_detail from w_a_reg_m`dw_detail within b1w_reg_priceplan_keytype_v20
string dataobject = "b1dw_master_priceplan_validkeytype_v20"
end type
...
type p_reset from w_a_reg_m`p_reset within b1w_reg_priceplan_keytype_v20
end type
