$PBExportHeader$b1w_reg_quotainfo_b_v20.srw
$PBExportComments$[kem] 장비 할부내역 등록
forward
global type b1w_reg_quotainfo_b_v20 from w_a_reg_m_m
end type
end forward
...
global b1w_reg_quotainfo_b_v20 b1w_reg_quotainfo_b_v20
...
on b1w_reg_quotainfo_b_v20.create
call super::create
end on

on b1w_reg_quotainfo_b_v20.destroy
call super::destroy
end on
...
ls_where = ""
ls_where += "(svc.status = '" + is_active + "' or svc.status = '" + is_reqactive + "') "
...
//개통신청, 개통신청(할부미등록) 상태 코드
ls_ref_desc = ""
is_reqactive = fs_get_control("B0", "P220", ls_ref_desc)
is_active = fs_get_control("B0", "P241", ls_ref_desc)	

	
end event

type dw_cond from w_a_reg_m_m`dw_cond within b1w_reg_quotainfo_b_v20
integer width = 1755
integer height = 276
...
type p_ok from w_a_reg_m_m`p_ok within b1w_reg_quotainfo_b_v20
integer x = 1920
integer y = 48
end type

type p_close from w_a_reg_m_m`p_close within b1w_reg_quotainfo_b_v20
integer x = 2226
integer y = 48
boolean originalsize = false
end type

type gb_cond from w_a_reg_m_m`gb_cond within b1w_reg_quotainfo_b_v20
integer width = 1792
integer height = 340
end type

type dw_master from w_a_reg_m_m`dw_master within b1w_reg_quotainfo_b_v20
integer x = 27
integer y = 360
integer width = 2629
integer height = 528
...
type dw_detail from w_a_reg_m_m`dw_detail within b1w_reg_quotainfo_b_v20
integer x = 23
integer y = 944
integer width = 2638
integer height = 780
string dataobject = "b1dw_reg_quotainfo_b"
end type

event type integer dw_detail::ue_retrieve(long al_select_row);call super::ue_retrieve;String ls_orderno, ls_status, ls_where, ls_check, ls_ref_desc
Long ll_row
...
//개통신청(할부미등록) 상태인지 확인 
ls_ref_desc = ""
ls_check = fs_get_control("B0", "P241", ls_ref_desc)
...
Long   ll_master_row, ll_orderno, ll_contractseq, ll_row, ll_rows, i, j
String ls_customerid, ls_itemcod, ls_itemnm, ls_priceplan, ls_quota_yn, ls_quota
Boolean lb_quota = False, lb_rental = False
String ls_customernm
...
For ll_row = 1 To dw_detail.RowCount()
	ls_quota_yn = Trim(dw_detail.object.itemmst_quota_yn[ll_row])
	
	If ls_quota_yn = 'Y' Then
		lb_quota = True
	ElseIf ls_quota_yn = 'R' Then
		lb_rental = True
	End If
Next

If lb_quota = False And lb_rental = False Then
	f_msg_info(9000, Title, "할부 또는 임대인 품목이 없습니다.")
	Return -1
Else
...
//	ls_itemnm = Trim(dw_detail.object.itemmst_itemnm[row])
	...
	//2005.11.10 kem Modify
	ls_customernm  = Trim(dw_master.Object.customerm_customernm[ll_master_row])
	ll_contractseq = dw_master.object.svcorder_ref_contractseq[ll_master_row]
	

	If lb_quota Then
		iu_cust_msg = Create u_cust_a_msg
		iu_cust_msg.is_pgm_name = "장비할부 정보"
		...
		For ll_rows = 1 To dw_detail.RowCount()
			ls_quota = Trim(dw_detail.object.itemmst_quota_yn[ll_rows])
			
			If ls_quota = 'Y' Then
				i++
				ls_itemcod = Trim(dw_detail.object.contractdet_itemcod[ll_rows])
				iu_cust_msg.is_data[i] = ls_itemcod
			End If
			ls_quota = ""
			ls_itemcod = ""
		Next
		
		iu_cust_msg.ib_data[1] = ib_order
		iu_cust_msg.il_data[1] = ll_orderno
		iu_cust_msg.il_data[2] = ll_contractseq
	//	iu_cust_msg.is_data[1] = ls_itemcod
		iu_cust_msg.is_data2[2] = ls_customerid
		iu_cust_msg.is_data2[3] = gs_pgm_id[gi_open_win_no]
		iu_cust_msg.is_data2[4] = "N"
		iu_cust_msg.is_data2[5] = ls_customernm
	
		OpenWithParm(b1w_reg_quotainfo_pop_2_v20, iu_cust_msg)
	...
	If lb_rental Then
		iu_cust_msg = Create u_cust_a_msg
		iu_cust_msg.is_pgm_name = "장비임대 정보"
		iu_cust_msg.is_grp_name = "서비스 신청"
		//iu_cust_msg.is_data[5] = ls_orderno			//order number
		//iu_cust_msg.is_data[1] = ls_customerid			//customer ID
		//iu_cust_msg.is_data[2] = ls_itemcod				//item code
		...
		For ll_rows = 1 To dw_detail.RowCount()
			...
			If ls_quota = 'R' Then
				j++
				ls_itemcod = Trim(dw_detail.object.contractdet_itemcod[ll_rows])
				iu_cust_msg.is_data[j] = ls_itemcod
			End If
			ls_quota = ""
			ls_itemcod = ""
		Next
...
type p_insert from w_a_reg_m_m`p_insert within b1w_reg_quotainfo_b_v20
boolean visible = false
integer y = 1692
end type

type p_delete from w_a_reg_m_m`p_delete within b1w_reg_quotainfo_b_v20
boolean visible = false
integer y = 1692
end type

type p_save from w_a_reg_m_m`p_save within b1w_reg_quotainfo_b_v20
boolean visible = false
integer x = 69
integer y = 1696
end type

type p_reset from w_a_reg_m_m`p_reset within b1w_reg_quotainfo_b_v20
integer x = 46
integer y = 1764
end type

type st_horizontal from w_a_reg_m_m`st_horizontal within b1w_reg_quotainfo_b_v20
integer y = 892
integer height = 40
end type
