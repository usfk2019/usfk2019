$PBExportHeader$b1w_reg_validkeymst_v20.srw
$PBExportComments$[ohj]인증Key 마스터 관리 v20
forward
global type b1w_reg_validkeymst_v20 from w_a_reg_m_tm2
end type
end forward
...
global b1w_reg_validkeymst_v20 b1w_reg_validkeymst_v20

on b1w_reg_validkeymst_v20.create
int iCurrent
call super::create
end on

on b1w_reg_validkeymst_v20.destroy
call super::destroy
end on
...
event ue_ok();call super::ue_ok;	Long		ll_rows
	...
	IF ls_sale_flag <> "" THEN
		IF ls_where <> "" THEN ls_where += " AND "
		ls_where += " a.sale_flag = '" + ls_sale_flag + "'"
	END IF
	...
	IF ls_idate <> "" THEN
		IF ls_where <> "" THEN ls_where += " AND "
		ls_where += "to_char(a.idate,'YYYYMMDD') >= '" + ls_idate + "'"
	END IF
	
	IF ls_idate_2 <> "" THEN
		IF ls_where <> "" THEN ls_where += " AND "
		ls_where += "to_char(a.idate,'YYYYMMDD') <= '" + ls_idate_2 + "'"
	END IF
	...
	IF ls_activedt <> "" THEN
		IF ls_where <> "" THEN ls_where += " AND "
		ls_where += "to_char(a.activedt,'YYYYMMDD') >= '" + ls_activedt + "'"
	END IF
	
	IF ls_activedt_2 <> "" THEN
		IF ls_where <> "" THEN ls_where += " AND "
		ls_where += "to_char(a.activedt,'YYYYMMDD') <= '" + ls_activedt_2 + "'"
	END IF
...
type dw_cond from w_a_reg_m_tm2`dw_cond within b1w_reg_validkeymst_v20
integer x = 64
integer y = 64
integer width = 2528
integer height = 356
string dataobject = "b1dw_reg_cnd_validkeymst_v20"
end type
...
type p_ok from w_a_reg_m_tm2`p_ok within b1w_reg_validkeymst_v20
integer x = 2683
end type

type p_close from w_a_reg_m_tm2`p_close within b1w_reg_validkeymst_v20
integer x = 2985
end type

type gb_cond from w_a_reg_m_tm2`gb_cond within b1w_reg_validkeymst_v20
integer x = 37
integer width = 2574
integer height = 440
end type

type dw_master from w_a_reg_m_tm2`dw_master within b1w_reg_validkeymst_v20
integer x = 37
integer y = 456
integer width = 3214
integer height = 560
string dataobject = "b1dw_reg_master_validkeymst_v20"
end type
...
type p_insert from w_a_reg_m_tm2`p_insert within b1w_reg_validkeymst_v20
boolean visible = false
end type

type p_delete from w_a_reg_m_tm2`p_delete within b1w_reg_validkeymst_v20
boolean visible = false
end type

type p_save from w_a_reg_m_tm2`p_save within b1w_reg_validkeymst_v20
integer x = 32
end type

type p_reset from w_a_reg_m_tm2`p_reset within b1w_reg_validkeymst_v20
integer x = 699
end type

type tab_1 from w_a_reg_m_tm2`tab_1 within b1w_reg_validkeymst_v20
integer x = 37
integer y = 1052
integer width = 3214
integer height = 760
...
//Tab에 해당하는 dw
is_dwObject[1] = "b1dw_reg_validkeymst_t1_v20"
is_dwObject[2] = "b1dw_reg_validkeymst_t2_v20"
end event
...
type st_horizontal from w_a_reg_m_tm2`st_horizontal within b1w_reg_validkeymst_v20
integer x = 37
integer y = 1020
end type
