$PBExportHeader$b5u_dbmgr9_v20.sru
$PBExportComments$[ohj]대리점 인증key 할당 db
forward
global type b5u_dbmgr9_v20 from u_cust_a_db
end type
end forward

global type b5u_dbmgr9_v20 from u_cust_a_db
end type
global b5u_dbmgr9_v20 b5u_dbmgr9_v20
...
//		  ls_acct_owner_bef, ls_groupnm, ls_limit_amt[], ls_invdetail_yn, ls_zoncod, ls_zonnm//, ls_s_bilamt
//Long    ll_header, ll_itemkey_data, ll_cur_balance, ll_pre_balance, ll_supplyamt, ll_surtax, ll_no, &
//        ll_biltime, ll_payamt1[], ll_payamt, ll_hh, ll_mi, ll_ss, ll_limit, ll_bilamt, ll_tt = 0
//Dec     ld_biltime	  
////iu_db.is_title = Title//
//ls_cnd_invf_type  = is_data[1]   // 파일타입
//ls_trdt           = is_data[2]  // 청구주기
...
//		  , A.CHARGEDT														//청구주기
//		  , A.REQNUM														//청구번호
//		  , trim(substrb(A.CUSTOMERNM, 1,50))		CUSTOMERNM	//고객명
//		  , A.LASTNAME														//고객명()
//		  , TRANSLATE(A.BIL_ZIPCOD, 'T-', 'T')  	BIL_ZIPCOD	//우편번호
//		  , trim(substrb(A.BIL_ADDR1, 1,60))	 	BIL_ADDR1	//주소
//		  , trim(substrb(A.BIL_ADDR2, 1,50)) 	 	BIL_ADDR2	//주소
//		  , A.CORPNM														//법인명
//		  , A.CORPNM_2   													//법인명		  
//		  , A.BANK 	 														//은행코드
//		  , NVL(B.CODENM, 'NoDefine')    			BANKNM      //은행명
//		  , A.BANKSHOP														//은행
//		  , TRANSLATE(A.ACCTNO, 'T-', 'T')  		ACCTNO
//		  , trim(substrb(A.ACCT_OWNER, 1,30))  	ACCT_OWNER 	//예금주
//		  , A.INPUTCLOSEDTCUR											//입금마감일
//		  , A.PAY_METHOD     	                      	 		//결제채널 '1' 지로, '2' 자동이체
//		  , DECODE(A.CTYPE2, :ls_ctype_20, A.CREGNO, '**********')   CREGNO	//사업자 등록번호개인이면  **********
//		  , A.CARD_TYPE                               			//카드사코드
//		  , NVL(C.CODENM, 'NoDefine')    		 	CARDNM		//카드사명
//		  , TRANSLATE(A.CARD_NO, 'T-', 'T')  	 	CARD_NO  	//카드사
//		  , trim(substrb(A.CARD_HOLDER, 1,30))  	CARD_HOLDER //카드소유인
//		  , NVL(A.INVDETAIL_YN, 'Y') 					INVDETAIL_YN// 청구서 CDR 출력여부
//	  FROM REQINFO A
//	     , (select code, codenm 
//  		       from syscod2t 
//		      where grcode = 'B400') B
...
//		      where grcode = 'B450') C
//	 WHERE A.BANK      = B.code(+)
//	   AND A.CARD_TYPE = C.code(+)
//	   AND A.TRDT        = to_date(:ls_trdt, 'yyyy-mm-dd')
//	   AND A.CHARGEDT    = :ls_cnd_chargedt
//	   AND A.PAY_METHOD IN (:ls_cnd_pay_method, :ls_cnd_bankpay, :ls_cnd_creditpay, :ls_etc)
////		AND INV_TYPE    = NVL(:ls_cnd_inv_type, INV_TYPE)
//		AND A.INV_YN      = 'Y'    
//		AND A.CTYPE2      = :ls_ctype2
// ORDER BY A.PAYID ;
//	 
//	OPEN reqinfo_cu;
//	
...
//		Into :ls_payid   , :ld_trdt, :ls_chargedt, :ls_reqnum, :ls_customernm
//	      , :ls_lastname, :ls_bil_zipcod, :ls_bil_addr1, :ls_bil_addr2, :ls_corpnm, :ls_corpnm_2
//			, :ls_bank    , :ls_banknm, :ls_bankshop, :ls_acctno, :ls_acct_owner, :ld_inputclosedtcur
//			, :ls_pay_method, :ls_cregno, :ls_card_type, :ls_cardnm, :ls_card_no, :ls_card_holder   , :ls_invdetail_yn        ;
//		
//		If SQLCA.SQLCode < 0 Then
...
//				  , trim(substrb(ACCT_OWNER, 1,30)) //예금주OR소유주
//				  , to_char(PAYDT, 'yyyymmdd')  PAYDT     //전월 출금일
//				  , pay_method
//				  , PAYAMT   	//전월 출금액
//			  FROM REQRECEIPT
...
//		CLOSE reqreceipt_cu;
//		
//		ll_up = UpperBound(ls_paydt1) 
////		messagebox('',string(ll_payamt1[1]) + ' ' +string(ll_payamt1[2]))
//		ll_itemkey_data = UpperBound(ls_itemkey_data4)
//		If Isnull(ll_itemkey_data) Then ll_itemkey_data = 0
//		
//		If ll_itemkey_data > 0 Then
...
//							ls_data4[tt] = fs_fill_pad(ls_paydt1[2], 8, '2', ' ')   //전월입금일
//						Else							
//							ls_data4[tt] = fs_fill_pad(ls_paydt1[1], 8, '2', ' ')   //전월입금일
//						End If
//					Case '10124'
//						If ll_payamt1[2] <> 0 Then
//							ls_data4[tt] = fs_fill_pad(string(ll_payamt1[2]), 10, '1', '0') //전월 입금액
//						else
//							ls_data4[tt] = fs_fill_pad(string(ll_payamt1[1]), 10, '1', '0') //전월 입금액
//						end If
//					Case '10125'	
//						If ll_payamt1[2] <> 0 Then
//							ls_data4[tt] = fs_fill_pad(ls_paydt1[1], 8, '2', ' ')//전월입금일2
//						else	
//						   ls_data4[tt] = fs_fill_pad('', 8, '2', ' ')//전월입금일2
//						End If
//					Case '10126'
//						If ll_payamt1[2] <> 0 Then
//							ls_data4[tt] = fs_fill_pad(string(ll_payamt1[1]), 10, '1', '0')//전월입금액2
//						else	
//						   ls_data4[tt] = fs_fill_pad('', 10, '1', '0')//전월입금일2
//						End If	
//					Case '10115'  //space
//						ls_data4[tt] = fs_fill_pad(ls_space, 17, '1', ' ')
//					Case Else
...
//						End If
//						
//						SELECT trim(substr(COUNTRYNM, 1,30))  COUNTRYNM
//						  INTO :ls_countrynm 
//						  FROM COUNTRY 
//						 WHERE COUNTRYCOD = :ls_countrycod;
//						 
...
//				CLOSE cdr_detail;	
//			//ROAM
//			ElseIf ls_cnd_invf_type = ls_result[5] Or ls_cnd_invf_type = ls_result[6] Then			
//				//cdr 상세
//				DECLARE cdr_detail2 CURSOR FOR
//					SELECT VALIDKEY
//						  , TO_CHAR(STIME, 'yyyymmddhh24miss') STIME 
//						  , BILTIME
//						  , ZONCOD
//						 // , RTELNUM
//						  , DECODE(RTELNUM, NULL, '', SUBSTR(RTELNUM, 1, length(RTELNUM) -2)||'**') RTELNUM
//						  , ROUND(BILAMT, 0) BILAMT
//					  FROM POST_BILCDR
//					 WHERE PAYID = :ls_payid
//						AND TO_CHAR(TRDT, 'YYYYMMDD') = :ls_trdt  
...
//						
//						Fetch cdr_detail2
//						Into :ls_validkey, :ls_stime, :ll_biltime, :ls_zoncod, :ls_rtelnum, :ll_bilamt;//, :ls_s_bilamt ;
//							
//						If SQLCA.SQLCode < 0 Then
//							ll_code = 0
//							Exit
...
//						End If
//						
//						SELECT trim(substrb(ZONNM, 1, 30)) ZONNM
//						  INTO :ls_zonnm 
//						  FROM ZONE 
...
//										ls_cdr[tt] = fs_fill_pad(ls_biltime, 6, '1', '0')  //사용시간(시분초)
//									Case '10131'
//										ls_cdr[tt] = fs_fill_pad(ls_zonnm, 30, '2', ' ')  //대역명	
//									Case '10113'
//										ls_cdr[tt] = fs_fill_pad(ls_rtelnum, 30, '2', ' ')  //착신번호
//									Case '10114'
//										ls_cdr[tt] = fs_fill_pad(string(ll_bilamt), 10, '1', '0')  //통화금액	
...
//End If		
//commit;
//If ll_tt <> 1 Then  //error
//	ii_rc = 0
//End If
//Return
end subroutine

on b5u_dbmgr9_v20.create
call super::create
end on

on b5u_dbmgr9_v20.destroy
call super::destroy
end on
