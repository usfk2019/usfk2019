$PBExportHeader$b5w_prc_reqcancel_v21.srw
$PBExportComments$[ceusee] 청구작업절차별 취소 처리
forward
global type b5w_prc_reqcancel_v21 from w_a_inq_m
end type
type p_select from u_p_select within b5w_prc_reqcancel_v21
end type
end forward
...
global b5w_prc_reqcancel_v21 b5w_prc_reqcancel_v21
   ...
   IF dw_detail.object.reqprocmenu_cancel_yn[ll_currow] = 'Y' Then
		...
		//	    SQLCA.b5InvStart(ls_chargedt, ls_payid, ls_pgm_id, gs_user_id, ll_return, ls_errmsg, lb_count)   
			CASE Upper("b5w_prc_Itemsale_M") //khpark modify 납입고객별처리 수정
				 SQLCA.B5ITEMSALE_MCan_V21(ls_chargedt,ls_payid,ls_pgm_id, gs_user_id, ll_return, ls_errmsg, lb_count)   
...
//				 SQLCA.B5ITEMSALE_MCan(ls_chargedt, ls_pgm_id, gs_user_id, ll_return, ls_errmsg, lb_count) 				 
			...
			CASE Upper("b5w_prc_Itemsale_Upload")
				SQLCA.b5ItemSale_UploadCan(ls_chargedt, ls_payid, ls_pgm_id, gs_user_id, ll_return, ls_errmsg, lb_count)
			...
			CASE Upper("b5CalcVat")
			   	SQLCA.b5Calc_Vat_Can(ls_chargedt, ls_pgm_id, gs_user_id, ll_return, ls_errmsg, lb_count)   
			//선수금 취소2005.01.28
			...
			CASE Upper("b5w_prc_ReqTaxsheet_info")
				SQLCA.b5ReqTaxsheet_infoCan(ls_chargedt, ls_pgm_id, gs_user_id, ll_return, ls_errmsg, lb_count)   		 				
		//	CASE "b5w_prc_InvEnd"
		//	    SQLCA.B5INVEND(ls_chargedt, ls_payid, ls_pgm_id, gs_user_id, ll_return, ls_errmsg, lb_count)   		 
			CASE Upper("b5w_prc_CalcPrePay")
				SQLCA.b5PrepayCloseCAN(ls_chargedt, ls_pgm_id, gs_user_id, ll_return, ls_errmsg, lb_count)   		 			 
...
on b5w_prc_reqcancel_v21.create
int iCurrent
call super::create
...
on b5w_prc_reqcancel_v21.destroy
call super::destroy
...
type dw_cond from w_a_inq_m`dw_cond within b5w_prc_reqcancel_v21
integer x = 82
integer y = 72
integer width = 1755
integer height = 296
...
type p_ok from w_a_inq_m`p_ok within b5w_prc_reqcancel_v21
integer x = 2089
integer y = 56
end type

type p_close from w_a_inq_m`p_close within b5w_prc_reqcancel_v21
integer x = 2688
integer y = 56
boolean originalsize = false
end type

type gb_cond from w_a_inq_m`gb_cond within b5w_prc_reqcancel_v21
integer y = 8
integer width = 1938
integer height = 396
end type

type dw_detail from w_a_inq_m`dw_detail within b5w_prc_reqcancel_v21
integer x = 32
integer y = 428
integer width = 3621
integer height = 1472
