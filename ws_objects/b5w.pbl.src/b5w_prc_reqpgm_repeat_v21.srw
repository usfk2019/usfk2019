$PBExportHeader$b5w_prc_reqpgm_repeat_v21.srw
$PBExportComments$[hhm] 청구재작업절차처리
forward
global type b5w_prc_reqpgm_repeat_v21 from w_a_inq_m
end type
type p_select from u_p_select within b5w_prc_reqpgm_repeat_v21
end type
end forward
...
global b5w_prc_reqpgm_repeat_v21 b5w_prc_reqpgm_repeat_v21
...
//			  SQLCA.B5ITEMSALE_MRep(ls_chargedt, ls_pgm_id, gs_user_id, ll_return, ls_errmsg, lb_count)   
		CASE Upper("b5w_prc_Itemsale_M")       //khpark modify 납입고객별처리 수정
			  SQLCA.B5ITEMSALE_MRep_V21(ls_chargedt, ls_payid, ls_pgm_id, gs_user_id, ll_return, ls_errmsg, lb_count)   			  
...
//			SQLCA.B5ITEMSALE_M_PRERep(ls_chargedt, ls_pgm_id, gs_user_id, ll_return, ls_errmsg, lb_count)		  
		...
		CASE Upper("b5w_prc_Itemsale_Upload")
			SQLCA.B5ITEMSALE_UPLOADREP(ls_chargedt, ls_payid, ls_pgm_id, gs_user_id, ll_return, ls_errmsg, lb_count)
		...
		//부가세 계산 2019/05/01 부터 적용
		CASE Upper("b5CalcVat")
			SQLCA.b5calc_vat_update_rep(ls_chargedt, ls_pgm_id, gs_user_id, ll_return, ls_errmsg, lb_count)   
		...
		CASE Upper("b5w_prc_ReqTaxsheet_info")
			SQLCA.b5ReqTaxsheet_infoRep(ls_chargedt, ls_pgm_id, gs_user_id, ll_return, ls_errmsg, lb_count)   		 				
	//	CASE "b5w_prc_InvEnd"
	//	    SQLCA.B5INVEND(ls_chargedt, ls_payid, ls_pgm_id, gs_user_id, ll_return, ls_errmsg, lb_count)   		 
		CASE Upper("b5w_prc_CalcPrePay")
			SQLCA.b5PrepayCloseREP(ls_chargedt, ls_pgm_id, gs_user_id, ll_return, ls_errmsg, lb_count)   		 			 
	   CASE ELSE
			ll_return = -1
...
on b5w_prc_reqpgm_repeat_v21.create
int iCurrent
call super::create
...
on b5w_prc_reqpgm_repeat_v21.destroy
call super::destroy
...
type dw_cond from w_a_inq_m`dw_cond within b5w_prc_reqpgm_repeat_v21
integer x = 82
integer y = 84
integer width = 1765
integer height = 316
string dataobject = "b5d_cnd_prc_reqcancel_v21"
...
String ls_startdt
Date ld_startdt
Date ld_use_fr, ld_use_to, ld_use_fr_pre, ld_use_to_pre
//Long ll_cnt

If dwo.Name = "chargedt" THEN 
	
	//청구기준일 구하기
	Select reqdt,useddt_fr,useddt_to,pre_useddt_fr,pre_useddt_to
	Into :id_reqdt,:ld_use_fr,:ld_use_to,:ld_use_fr_pre,:ld_use_to_pre
	From reqconf 
	...
	dw_cond.object.frdate_pre[1] = ld_use_fr_pre
	dw_cond.object.todate_pre[1] = ld_use_to_pre
...
type p_ok from w_a_inq_m`p_ok within b5w_prc_reqpgm_repeat_v21
integer x = 2103
integer y = 56
end type

type p_close from w_a_inq_m`p_close within b5w_prc_reqpgm_repeat_v21
integer x = 2702
integer y = 56
boolean originalsize = false
end type

type gb_cond from w_a_inq_m`gb_cond within b5w_prc_reqpgm_repeat_v21
integer y = 4
integer width = 1920
integer height = 428
end type

type dw_detail from w_a_inq_m`dw_detail within b5w_prc_reqpgm_repeat_v21
integer x = 32
integer y = 456
integer width = 3625
integer height = 1484
