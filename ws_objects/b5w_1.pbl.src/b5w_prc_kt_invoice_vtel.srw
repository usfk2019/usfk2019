$PBExportHeader$b5w_prc_kt_invoice_vtel.srw
$PBExportComments$[islim] KT 청구요청파일( 브이텔레콤)
forward
global type b5w_prc_kt_invoice_vtel from w_a_prc
end type
end forward
...
global b5w_prc_kt_invoice_vtel b5w_prc_kt_invoice_vtel
...
String is_kt_code, is_kt_paymethod

end variables

on b5w_prc_kt_invoice_vtel.create
call super::create
end on

on b5w_prc_kt_invoice_vtel.destroy
call super::destroy
end on

event open;call super::open;/*------------------------------------------------------------------------
	Name : b5_prc_kt_invoice_vtel
	Desc.: 청구내역을 KT 통합청구요청
	Date : 2005.08.23
	Auth.: juede  (브이텔레콤 전용)
-------------------------------------------------------------------------*/
String ls_ref_desc
...
//KT합산청구서
is_file = fs_get_control("B5", "P110", ls_ref_desc) 

//KT 합산사 코드
is_kt_code = fs_get_control("B5", "P111", ls_ref_desc) 

//KT 합산 결재방법
is_kt_paymethod = fs_get_control("B5", "P112", ls_ref_desc) 
...
lu_dbmgr.is_caller = "Create KT summary billing vtel"
lu_dbmgr.is_data[1] = is_chargedt
lu_dbmgr.is_data[2] = is_trdt
lu_dbmgr.is_data[3] = is_kt_code
lu_dbmgr.is_data[4] = is_file
lu_dbmgr.is_data[5] = is_kt_paymethod
lu_dbmgr.is_data[6] = is_manager_tel
lu_dbmgr.is_data[7] = is_issue_dt //청구서 발행일자
...
type p_ok from w_a_prc`p_ok within b5w_prc_kt_invoice_vtel
integer x = 1417
integer y = 72
boolean originalsize = false
end type

type dw_input from w_a_prc`dw_input within b5w_prc_kt_invoice_vtel
integer y = 44
integer width = 1262
integer height = 352
string dataobject = "b5dw_cnd_prc_kt_invoice_vtel"
...
type dw_msg_time from w_a_prc`dw_msg_time within b5w_prc_kt_invoice_vtel
integer y = 848
end type

type dw_msg_processing from w_a_prc`dw_msg_processing within b5w_prc_kt_invoice_vtel
integer y = 452
integer height = 380
end type

type ln_up from w_a_prc`ln_up within b5w_prc_kt_invoice_vtel
integer beginy = 432
integer endy = 432
end type

type ln_down from w_a_prc`ln_down within b5w_prc_kt_invoice_vtel
integer beginy = 1136
integer endy = 1136
end type

type p_close from w_a_prc`p_close within b5w_prc_kt_invoice_vtel
integer x = 1417
integer y = 180
end type

type gb_cond from w_a_prc`gb_cond within b5w_prc_kt_invoice_vtel
integer width = 1312
integer height = 416
end type
