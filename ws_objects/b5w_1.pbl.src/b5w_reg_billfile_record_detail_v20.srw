$PBExportHeader$b5w_reg_billfile_record_detail_v20.srw
$PBExportComments$[ohj] 청구file record 상세구성
forward
global type b5w_reg_billfile_record_detail_v20 from w_a_reg_m_m
end type
type p_new from u_p_new within b5w_reg_billfile_record_detail_v20
end type
type p_select from u_p_select within b5w_reg_billfile_record_detail_v20
end type
end forward
...
global b5w_reg_billfile_record_detail_v20 b5w_reg_billfile_record_detail_v20

type variables
String is_priceplan		, is_bill_type
Boolean ib_new
Long il_rows = 0
...
on b5w_reg_billfile_record_detail_v20.create
int iCurrent
call super::create
...
on b5w_reg_billfile_record_detail_v20.destroy
call super::destroy
destroy(this.p_new)
...
event open;call super::open;/*------------------------------------------------------------------------
	Name	:	b1w_reg_billfile_record_Detail
	Desc.	: 	청구서파일 레코드 항목 상세 정의
	Ver.	:	1.0
	Date	: 	2004.02.22
...
event type integer ue_extra_insert(long al_insert_row);//String ls_priceplan, ls_itemcod, ls_target_type
//Long ll_row
//
...
//Save
Long   ll_row, ll_rows, ll_findrow, ll_i, ll_zoncodcnt, i, ll_max_seqno, cnt
Long   ll_rows1, ll_rows2, ll_seqno, ll_maxlength, ll_seq, ll_key, ll_cnt = 1
String ls_itemkey_desc, ls_pad_type, ls_pad_value, ls_item_value, ls_itemtype
...
For ll_row = 1 To ll_rows
	...
	ls_itemtype = dw_detail.Object.itemtype[ll_row]
	
	If ls_itemtype = 'V' Then
		dw_detail.SetItem(ll_row, 'itemkey' , ll_key + ll_cnt)
		ll_cnt ++
	End If
	
	//SEQ
//	Select seq_invf_recorddet.nextval 
//	  into :ll_seq 
//	  from dual;
//	  
//	//item
//	dw_detail.SetItem(ll_row, 'itemkey', ll_seq)
	
	//itemkey_desc
	ls_itemkey_desc = Trim(dw_detail.Object.itemkey_desc[ll_row])

	If IsNull(ls_itemkey_desc) Then ls_itemkey_desc = ""
	
	 //필수 항목 check 
	If ls_itemkey_desc = "" Then
		f_msg_usr_err(200, Title, "Description")
		dw_detail.SetColumn("itemkey_desc")
	...
	//item_value
	ls_item_value = Trim(dw_detail.Object.item_value[ll_row])

	If IsNull(ls_item_value) Then ls_item_value = ""
	
	If ls_itemtype = 'V' Then 
		 //필수 항목 check 
		If ls_item_value = "" Then
			If cnt = 0 Then
//				f_msg_usr_err(9000, Title, "Input value")
				f_msg_usr_err(200, Title , "Input value")
				cnt ++
			End If 
		End If	
	End If
	...
	//maxlength
	ll_maxlength = dw_detail.Object.maxlength[ll_row]

	If IsNull(ll_maxlength) Then ll_maxlength = 0
	
	 //필수 항목 check 
	If ll_maxlength = 0 Then
		f_msg_usr_err(200, Title, "max length")
		dw_detail.SetColumn("maxlength")
		dw_detail.SetRow(ll_row)
		dw_detail.ScrollToRow(ll_row)
		dw_detail.SetRedraw(True)
		Return -2
	...
	//pad_type
	ls_pad_type = dw_detail.Object.pad_type[ll_row]

	If IsNull(ls_pad_type) Then ls_pad_type = ''
	
	 //필수 항목 check 
	If ls_pad_type = '' Then
		f_msg_usr_err(200, Title, "pad type")
		dw_detail.SetColumn("pad_type")
		dw_detail.SetRow(ll_row)
		dw_detail.ScrollToRow(ll_row)
		dw_detail.SetRedraw(True)
		Return -2
	...
	//pad_value
	ls_pad_value = dw_detail.Object.pad_value[ll_row]

	If IsNull(ls_pad_value) Then ls_pad_value = ''
	
 	If ls_pad_type <> '0' Then
		//필수 항목 check 
		If ls_pad_value = '' Then
			f_msg_usr_err(200, Title, "pad value")
			dw_detail.SetColumn("pad_value")
			dw_detail.SetRow(ll_row)
			dw_detail.ScrollToRow(ll_row)
			dw_detail.SetRedraw(True)
			Return -2
...
If newheight < (dw_detail.Y + iu_cust_w_resize.ii_button_space) Then
	dw_detail.Height = 0
   p_select.Y	= dw_detail.Y + iu_cust_w_resize.ii_dw_button_space
...
Else
	...
	p_select.Y	= newheight - iu_cust_w_resize.ii_button_space_1 
...
Long   ll_row, ll_rows, ll_seqno, ll_seq
String ls_invf_type, ls_record
...
ls_invf_type = dw_master.GetItemString(ll_rows, 'invf_type')
ls_record    = dw_master.GetItemString(ll_rows, 'record')

ll_row = dw_detail.InsertRow(dw_detail.GetRow()+1)

dw_detail.SetItem(ll_row, 'invf_type' , ls_invf_type)
dw_detail.SetItem(ll_row, 'record'    , ls_record)
dw_detail.SetItem(ll_row, 'itemtype', 'V')
...
type dw_cond from w_a_reg_m_m`dw_cond within b5w_reg_billfile_record_detail_v20
integer x = 59
integer y = 60
integer width = 1746
integer height = 128
...
type p_ok from w_a_reg_m_m`p_ok within b5w_reg_billfile_record_detail_v20
integer x = 2057
integer y = 52
boolean originalsize = false
end type

type p_close from w_a_reg_m_m`p_close within b5w_reg_billfile_record_detail_v20
integer x = 2363
integer y = 52
end type

type gb_cond from w_a_reg_m_m`gb_cond within b5w_reg_billfile_record_detail_v20
integer x = 23
integer y = 4
integer width = 1934
integer height = 212
...
type dw_master from w_a_reg_m_m`dw_master within b5w_reg_billfile_record_detail_v20
integer x = 23
integer y = 256
integer height = 448
string dataobject = "b5dw_cnd_reg_billfile_record_detail"
...
type dw_detail from w_a_reg_m_m`dw_detail within b5w_reg_billfile_record_detail_v20
integer x = 23
integer y = 736
integer height = 900
string dataobject = "b5dw_reg_billfile_record_detail"
end type
...
String ls_where, ls_invf_type
String ls_record
Long ll_row, i, ll_seqno
Integer li_cnt
Dec{0} lc_data
DataWindowChild ldc
...
If dw_master.RowCount() > 0 Then
	ls_invf_type = Trim(dw_master.object.invf_type[al_select_row])
	ll_seqno     = dw_master.object.seqno[al_select_row]	
	ls_record    = Trim(dw_master.object.record[al_select_row])	
	
	ls_where = ""

	If ls_invf_type <> "" Then
		If ls_where <> "" Then ls_where += " And "
		ls_where += " INVF_TYPE = '" + ls_invf_type + "' "

	End If
	
	If ls_record <> "" Then
		If ls_where <> "" Then ls_where += " And "
		ls_where += " RECORD = '" + ls_record + "' "
...
//       	  , ITEMTYPE
//       	  , PGMCODE
//		  INTO :ls_itemkey_desc
//		     , :ls_itemtype
//			  , :ls_pgmcode
//    	  FROM INVF_BASEITEMKEY
//		 WHERE ITEMKEY = :data         ;
//		 
//		If SQLCA.SQLCode < 0 Then
//			f_msg_sql_err(Title, "select Error(INVF_BASEITEMKEY)")
//			Return -1
//			
//		ElseIf SQLCA.SQLCode = 100 Then
//			f_msg_sql_err(Title, "select no-data(INVF_BASEITEMKEY)")
//
//		Else
//			dw_detail.SetItem(row, 'itemkey_desc', ls_itemkey_desc)
//			dw_detail.SetItem(row, 'itemtype'    , ls_itemtype)
//			dw_detail.SetItem(row, 'pgmcode'     , ls_pgmcode)
//		
//		End If		
//		  
//End Choose
...
type p_insert from w_a_reg_m_m`p_insert within b5w_reg_billfile_record_detail_v20
integer x = 334
integer y = 1660
end type

type p_delete from w_a_reg_m_m`p_delete within b5w_reg_billfile_record_detail_v20
integer x = 626
end type

type p_save from w_a_reg_m_m`p_save within b5w_reg_billfile_record_detail_v20
integer x = 919
end type

type p_reset from w_a_reg_m_m`p_reset within b5w_reg_billfile_record_detail_v20
integer x = 1655
end type

type st_horizontal from w_a_reg_m_m`st_horizontal within b5w_reg_billfile_record_detail_v20
integer y = 704
integer height = 36
end type
...
event clicked;String ls_invf_type, ls_record
Long   ll_seqno, ll_rows
...
iu_cust_msg = Create u_cust_a_msg
iu_cust_msg.is_pgm_name = "청구서파일 기본항목 information"
iu_cust_msg.is_grp_name = "Bill file Record detail"
iu_cust_msg.idw_data[1] = dw_detail
iu_cust_msg.is_data[1] = ls_invf_type
iu_cust_msg.is_data[2] = ls_record

OpenWithParm(b5w_inq_billfile_record_detail_popup, iu_cust_msg)
