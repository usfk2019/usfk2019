$PBExportHeader$b5w_reg_reqinfo_mdf_1_v20.srw
$PBExportComments$[backgu-2002/09-26] 월별 고객 정보 History
forward
global type b5w_reg_reqinfo_mdf_1_v20 from w_a_reg_m
end type
end forward
...
global b5w_reg_reqinfo_mdf_1_v20 b5w_reg_reqinfo_mdf_1_v20

type variables
String is_cur_gu, is_jiro, is_card, is_bank, is_inv_method, is_cur_gu_1, is_cur_gb
end variables
...
event type integer ue_extra_save();call super::ue_extra_save;String ls_payid    , ls_ctype2    , ls_customernm  , ls_ssno
...
//messagebox("is_cur_gu"+is_cur_gu ,"is_cur_gb"+is_cur_gb)

dw_detail.SetRedraw(False)

If is_cur_gu = 'Y' Then
	dw_detail.DataObject = "b5d_reg_reqinfo_mdf_a"
	dw_detail.SetTransObject(SQLCA)
	dw_detail.is_where = ls_where
	
Else
	dw_detail.DataObject = "b5d_reg_reqinfo_mdf_b"
	dw_detail.SetTransObject(SQLCA)
	dw_detail.is_where = ls_where
End If
...
//dw_detail.is_where = ls_where
dw_detail.TriggerEvent("ue_init")
ll_row = dw_detail.Retrieve(is_cur_gu,is_inv_method,is_jiro,is_bank,is_card, is_cur_gb)
...
on b5w_reg_reqinfo_mdf_1_v20.create
call super::create
end on

on b5w_reg_reqinfo_mdf_1_v20.destroy
call super::destroy
end on
...
is_cur_gu_1 = fs_get_control("B5","A104",ls_ref_desc) //전월 고객정보 수정여부

end event

type dw_cond from w_a_reg_m`dw_cond within b5w_reg_reqinfo_mdf_1_v20
integer x = 55
integer y = 44
integer width = 2112
integer height = 244
...
type p_ok from w_a_reg_m`p_ok within b5w_reg_reqinfo_mdf_1_v20
integer x = 2377
end type

type p_close from w_a_reg_m`p_close within b5w_reg_reqinfo_mdf_1_v20
integer x = 2683
boolean originalsize = false
end type

type gb_cond from w_a_reg_m`gb_cond within b5w_reg_reqinfo_mdf_1_v20
integer width = 2231
end type

type p_delete from w_a_reg_m`p_delete within b5w_reg_reqinfo_mdf_1_v20
boolean visible = false
integer y = 1664
end type

type p_insert from w_a_reg_m`p_insert within b5w_reg_reqinfo_mdf_1_v20
boolean visible = false
integer y = 1664
end type

type p_save from w_a_reg_m`p_save within b5w_reg_reqinfo_mdf_1_v20
integer x = 69
integer y = 1664
end type

type dw_detail from w_a_reg_m`dw_detail within b5w_reg_reqinfo_mdf_1_v20
integer y = 324
integer width = 3337
integer height = 1288
string dataobject = "b5d_reg_reqinfo_mdf_a"
end type
...
event dw_detail::buttonclicked;call super::buttonclicked;String ls_payid, ls_paymethod, ls_bank, ls_acctno, ls_bank_chg, ls_acct_owner, ls_acct_ssno, &
       ls_drawingtype, ls_drawingresult, ls_receiptcod, ls_resultcod, &
		 ls_card_no, ls_card_type, ls_card_remark1, ls_card_group1, &
		 ls_card_holder, ls_card_ssno, ls_card_authyn, ls_bil_addrtype, ls_bil_zipcod, &
		 ls_bil_addr1, ls_bil_addr2
Date   ld_drawingreqdt, ld_card_expdt, ld_receiptdt, ld_creditreqdt
//start 고객청구정보와의 결제정보 동기화를 위한 버튼추가 -jwlee 2006.05.30		 
If dwo.name = "b_copy" Then
	
	ls_payid = Trim(dw_detail.object.payid[1])
	
	SELECT nvl(pay_method   ,' ')   //결제방법
		  , nvl(bank         ,' ')   //은    행       
		  , nvl(acctno       ,' ')   //계좌번호
		  , nvl(acct_owner   ,' ')   //예 금 주
		  , nvl(acct_ssno    ,' ')   //예금주주민번호
		  , drawingreqdt             //이체신청일자
		  , nvl(drawingtype  ,' ')   //이체신청유형
		  , nvl(drawingresult,' ')   //이체신청결과
		  , nvl(receiptcod   ,' ')   //신청접수처
		  , receiptdt                //신청접수일자
		  , nvl(resultcod    ,' ')   //신청결과코드
		  , creditreqdt              //카드결제신청일
		  , nvl(card_no      ,' ')   //신용카드번호
		  , nvl(card_type    ,' ')   //카드사
		  , nvl(card_remark1 ,' ')   //카드유형
		  , nvl(card_group1  ,' ')   //카드그룹
		  , card_expdt               //카드유효기간
		  , nvl(card_holder  ,' ')   //카드회원명
		  , nvl(card_ssno    ,' ')   //카드회원 주민번호
		  , nvl(card_authyn  ,' ')   //카드인증여부
		  , nvl(bil_addrtype ,' ')   //주소구분
		  , nvl(bil_zipcod   ,' ')   //우편번호
		  , nvl(bil_addr1    ,' ')   //주소1
		  , nvl(bil_addr2    ,' ')   //주소2
	  INTO :ls_paymethod       //결제방법
		  , :ls_bank            //은    행       
		  , :ls_acctno          //계좌번호
		  , :ls_acct_owner      //예 금 주
		  , :ls_acct_ssno       //예금주주민번호
		  , :ld_drawingreqdt    //이체신청일자
		  , :ls_drawingtype     //이체신청유형
		  , :ls_drawingresult   //이체신청결과
		  , :ls_receiptcod      //신청접수처
		  , :ld_receiptdt       //신청접수일자
		  , :ls_resultcod       //신청결과코드
		  , :ld_creditreqdt     //카드결제신청일
		  , :ls_card_no         //신용카드번호
		  , :ls_card_type       //카드사
		  , :ls_card_remark1    //카드유형
		  , :ls_card_group1     //카드그룹
		  , :ld_card_expdt      //카드유효기간
		  , :ls_card_holder     //카드회원명
		  , :ls_card_ssno       //카드회원 주민번호
		  , :ls_card_authyn     //카드인증여부
		  , :ls_bil_addrtype    //주소구분
		  , :ls_bil_zipcod      //우편번호
		  , :ls_bil_addr1       //주소1
		  , :ls_bil_addr2       //주소2
	  FROM BILLINGINFO
	...
	If ls_paymethod = is_bank Then                                                                
		dw_detail.Object.pay_method   [1] = ls_paymethod     //결제방법        
		dw_detail.Object.bank         [1] = ls_bank          //은    행       
		dw_detail.Object.acctno       [1] = ls_acctno        //계좌번호       
		dw_detail.Object.acct_owner   [1] = ls_acct_owner    //예 금 주       
		dw_detail.Object.acct_ssno    [1] = ls_acct_ssno     //예금주주민번호 
		dw_detail.Object.drawingreqdt [1] = ld_drawingreqdt  //이체신청일자   
		dw_detail.Object.drawingtype  [1] = ls_drawingtype   //이체신청유형   
		dw_detail.Object.drawingresult[1] = ls_drawingresult //이체신청결과   
		dw_detail.Object.receiptcod   [1] = ls_receiptcod    //신청접수처     
		dw_detail.Object.receiptdt    [1] = ld_receiptdt     //신청접수일자   
		dw_detail.Object.resultcod    [1] = ls_resultcod     //신청결과코드 
	ElseIf ls_paymethod = is_card Then    
		dw_detail.Object.creditreqdt  [1] = ld_creditreqdt   //카드결제신청일
		dw_detail.Object.card_no      [1] = ls_card_no       //신용카드번호
		dw_detail.Object.card_type    [1] = ls_card_type     //카드사
		dw_detail.Object.card_remark1 [1] = ls_card_remark1  //카드유형
		dw_detail.Object.card_group1  [1] = ls_card_group1   //카드그룹
		dw_detail.Object.card_expdt   [1] = ld_card_expdt    //카드유효기간
		dw_detail.Object.card_holder  [1] = ls_card_holder   //카드회원명
		dw_detail.Object.card_ssno    [1] = ls_card_ssno     //카드회원 주민번호
		dw_detail.Object.card_authyn  [1] = ls_card_authyn   //카드인증여부
	Else
		Messagebox('확인','결제방법이 자동이체와 카드결제일때만 가능합니다.')
		Return  -1
	End If
		dw_detail.Object.bil_addrtype [1] = ls_bil_addrtype  //주소구분 
		dw_detail.Object.bil_zipcod   [1] = ls_bil_zipcod    //우편번호 
		dw_detail.Object.bil_addr1    [1] = ls_bil_addr1     //주소1 
		dw_detail.Object.bil_addr2    [1] = ls_bil_addr2     //주소2
		
	Messagebox('성공','수정완료 되었습니다.')
	Return  -1
End If
//end 고객청구정보와의 결제정보 동기화를 위한 버튼추가 -jwlee 2006.05.30	
end event

type p_reset from w_a_reg_m`p_reset within b5w_reg_reqinfo_mdf_1_v20
integer x = 402
integer y = 1664
end type
