$PBExportHeader$b8w_reg_admst_1.srw
$PBExportComments$[chooys] 장비마스터정보관리 - Window
forward
global type b8w_reg_admst_1 from w_a_reg_m_tm2
end type
end forward
...
global b8w_reg_admst_1 b8w_reg_admst_1
...
on b8w_reg_admst_1.create
int iCurrent
call super::create
end on

on b8w_reg_admst_1.destroy
call super::destroy
end on
...
event ue_ok();call super::ue_ok;	Long		ll_rows
	...
	String	ls_pid
	String	ls_movedtfrom, ls_movedtto, ls_saledt_fr, ls_saledt_to
	String	ls_mv_partner
	String	ls_adstat
	String 	ls_controlno_fr, ls_controlno_to, ls_note

	//Condition
	ls_modelno		= Trim(dw_cond.Object.modelno[1])
	ls_controlno_fr= Trim(dw_cond.Object.contno_fr[1])
	ls_controlno_to= Trim(dw_cond.Object.contno_to[1])	
	...
	IF IsNull(ls_controlno_fr)	THEN ls_controlno_fr = ""
	IF IsNull(ls_controlno_to)	THEN ls_controlno_to = ""	
	...
	IF ls_controlno_fr <> "" THEN
		IF ls_where <> "" THEN ls_where += " AND "
		ls_where += "contno = '" + ls_controlno_fr + "'"
	END IF
	
	IF ls_controlno_to <> "" THEN
		IF ls_where <> "" THEN ls_where += " AND "
		ls_where += "contno = '" + ls_controlno_to + "'"
	END IF
...
IF li_tab_index = 1 THEN
   //Update정보
	...
	string ls_status, ls_dbstatus, ls_action
	long ll_adseq, ll_seq
	
	//status 변경시 장비이력 저장
	SELECT ref_content INTO :ls_action  FROM sysctl1t //800 : Item상태변경
	...
	select status into :ls_dbstatus from admst
	where adseq = :ll_adseq;
	
	if ls_status <> ls_dbstatus then
...
//tab_1 에서 변경한 status를 master에도 반영해준다.
ll_master_row = dw_master.GetSelectedRow(0)
this.triggerevent("ue_ok")

dw_master.SelectRow(0, FALSE)
dw_master.SelectRow(ll_master_row, TRUE)
dw_master.scrolltorow(ll_master_row)
...
type dw_cond from w_a_reg_m_tm2`dw_cond within b8w_reg_admst_1
integer x = 41
integer y = 40
integer width = 3003
integer height = 456
...
type p_ok from w_a_reg_m_tm2`p_ok within b8w_reg_admst_1
integer x = 3081
integer y = 72
boolean originalsize = false
end type

type p_close from w_a_reg_m_tm2`p_close within b8w_reg_admst_1
integer x = 3081
integer y = 184
end type

type gb_cond from w_a_reg_m_tm2`gb_cond within b8w_reg_admst_1
integer x = 27
integer width = 3045
integer height = 504
end type

type dw_master from w_a_reg_m_tm2`dw_master within b8w_reg_admst_1
integer x = 23
integer y = 536
integer width = 3314
integer height = 460
...
type p_insert from w_a_reg_m_tm2`p_insert within b8w_reg_admst_1
boolean visible = false
end type

type p_delete from w_a_reg_m_tm2`p_delete within b8w_reg_admst_1
boolean visible = false
end type

type p_save from w_a_reg_m_tm2`p_save within b8w_reg_admst_1
integer x = 23
integer y = 1784
end type

type p_reset from w_a_reg_m_tm2`p_reset within b8w_reg_admst_1
integer x = 667
integer y = 1784
end type

type tab_1 from w_a_reg_m_tm2`tab_1 within b8w_reg_admst_1
integer x = 23
integer y = 1032
integer width = 3301
integer height = 728
...
//Tab Title
is_tab_title[1] = "Equip. Info"
is_tab_title[2] = "Equip. History"



//Tab에 해당하는 dw
is_dwObject[1] = "b8dw_inq_admst_t1_1"
...
String 	ls_adseq, ls_status, &
			ls_temp, ls_ref_desc, ls_sta_rt100, ls_sta_sn100
String ls_where
Long ll_row
Int li_tab
...
ls_status 	= Trim(dw_master.object.status[al_master_row])

ls_temp 			= fs_get_control("E1", "A102", ls_ref_desc)
ls_sta_rt100 	= trim(ls_temp) //Return
ls_temp 			= fs_get_control("E1", "A104", ls_ref_desc)
ls_sta_sn100 	= trim(ls_temp) //Entering
...
Choose Case li_tab
	Case 1								//Tab 1
		...
		IF ls_status = ls_sta_rt100 OR &
		   ls_status = ls_sta_sn100 THEN
...
string ls_status, ls_emp_group, ls_partner, ls_contno
long ll_adseq
datawindowchild dw_status
		...
		if adwo_dwo.name= 'status' then
			...
			If ls_status = 'SN100' or ls_status = 'RT100' Then
				...
				IF ls_partner <> ls_emp_group THEN
				 ...
				 dw_status.SetFilter  ("codenm = 'Displayed Goods'")
				 dw_status.Filter( ) 				 
			...
			elseif  ls_status = 'DG100'  Then
				IF ls_partner <> ls_emp_group THEN
						...
						Return     -1
			  	END IF
				 dw_status.SetFilter  ("codenm = 'Entering Goods' ")
				 dw_status.Filter( ) 
			else
				dw_status.SetFilter  ("")
				dw_status.Filter( ) 
...
type st_horizontal from w_a_reg_m_tm2`st_horizontal within b8w_reg_admst_1
integer x = 14
integer y = 996
end type
