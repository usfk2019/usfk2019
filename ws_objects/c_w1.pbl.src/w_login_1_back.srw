$PBExportHeader$w_login_1_back.srw
$PBExportComments$[ceusee] 암호화된 Login
forward
global type w_login_1_back from window
end type
type ddlb_shop from dropdownlistbox within w_login_1_back
end type
type st_shop from statictext within w_login_1_back
end type
type p_2 from u_p_change within w_login_1_back
end type
type st_2 from statictext within w_login_1_back
end type
type st_1 from statictext within w_login_1_back
end type
type sle_password from singlelineedit within w_login_1_back
end type
type sle_user_id from singlelineedit within w_login_1_back
end type
type p_cancel from u_p_cancel within w_login_1_back
end type
type p_ok from u_p_ok within w_login_1_back
end type
type p_1 from picture within w_login_1_back
end type
end forward
...
global w_login_1_back w_login_1_back
...
double ll_return
String ls_passwd_out, ls_errmsg
Long	 ll_days
...
ElseIf Trim(ls_password) = "" Then
	...
	Return
ElseIf Trim(ls_password) <> "" Then
	If LenA(ls_password) > 15 Then
		f_msg_usr_err(201, Title, "115자리 이하로 입력하세요.")
		sle_password.SetFocus()
		Return
	End If
End If
...
iu_cust_db_app.is_data[2] = ls_passwd_out  //암호화된 Password
iu_cust_db_app.uf_prc_db()
