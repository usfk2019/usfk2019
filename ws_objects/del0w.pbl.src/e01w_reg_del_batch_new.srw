$PBExportHeader$e01w_reg_del_batch_new.srw
$PBExportComments$[parkkh] 연체자처리방안일괄등록
forward
global type e01w_reg_del_batch_new from w_a_reg_m_sql
end type
end forward
...
global e01w_reg_del_batch_new e01w_reg_del_batch_new

type variables
e01u_dbmgr iu_db01
end variables

on e01w_reg_del_batch_new.create
call super::create
end on

on e01w_reg_del_batch_new.destroy
call super::destroy
end on
...
If Not IsNull(li_delay_months) Then
	If ls_where <> "" then ls_where += " and " 
	ls_where += " (A.DLY_INT_MONTH >= " + String(li_delay_months) + " OR A.DLY_MOB_MONTH >= " + String(li_delay_months) + ") "
End If
If Not IsNull(li_delay_months_to) Then
	If ls_where <> "" then ls_where += " and " 
	ls_where += " (A.DLY_INT_MONTH <= " + String(li_delay_months_to) + " OR A.DLY_MOB_MONTH <= " + String(li_delay_months_to) + ") "
End If
...
If ls_dlyamt <> "" Then
	If ls_where <> "" Then ls_where += " AND "
	ls_where += " A.RETRY_INT_AMT + A.RETRY_MOB_AMT >= " + ls_dlyamt
End If
...
event ue_save_sql;Long 		ll_whole, 			ll_row 
...
FOR ll_row = 1 TO ll_whole
	...
	iu_db01.is_caller = "e01w_reg_del_batch_new%save"
...
type dw_cond from w_a_reg_m_sql`dw_cond within e01w_reg_del_batch_new
integer x = 41
integer y = 40
integer width = 2190
integer height = 448
string dataobject = "e01d_cnd_reg_del_batch_new"
...
type p_ok from w_a_reg_m_sql`p_ok within e01w_reg_del_batch_new
integer x = 2272
integer y = 48
end type

type p_close from w_a_reg_m_sql`p_close within e01w_reg_del_batch_new
integer x = 2857
integer y = 48
end type

type gb_cond from w_a_reg_m_sql`gb_cond within e01w_reg_del_batch_new
integer width = 2208
integer height = 504
end type

type p_save from w_a_reg_m_sql`p_save within e01w_reg_del_batch_new
integer x = 2565
integer y = 48
end type

type dw_detail from w_a_reg_m_sql`dw_detail within e01w_reg_del_batch_new
integer x = 23
integer y = 512
integer width = 3099
string dataobject = "e01d_reg_del_batch_new"
end type
