$PBExportHeader$b1u_dbmgr9_sams.sru
$PBExportComments$[kem] DB Manager/서비스신청(장비임대포함)
forward
global type b1u_dbmgr9_sams from u_cust_a_db
end type
end forward

global type b1u_dbmgr9_sams from u_cust_a_db
end type
global b1u_dbmgr9_sams b1u_dbmgr9_sams
...
String ls_ad_status, ls_contno
date	 ldt_shop_closedt

ii_rc = -1
Choose Case is_caller
	Case "b1w_reg_quotainfo_pop_2_SAMS%getdata"
//		lu_dbmgr.is_data[1] = is_orderno
//		lu_dbmgr.is_data[2] = is_customerid
      
		//Svcorder
		SELECT 	svccod,  		priceplan, 		to_char(orderdt, 'yyyymmdd'), 
					status, 			vpricecod, 		reg_partner
		  INTO 	:is_data[3], 	:is_data[4], 	:is_data[5], 
		  			:is_data[6], 	:is_data[7], 	:is_data[8]
		  FROM 	svcorder
		 WHERE 	to_char(orderno) 	= :is_data[1] 
	...
	Case "b1w_reg_quotainfo_pop_2_SAMS%insert"
...
//		lu_dbmgr.idw_data[1] = dw_detail
	   ...
	   SELECT count(*) 		INTO :li_cnt 		FROM quota_info
		 WHERE TO_CHAR(orderno) = :is_data[3] 
		   AND customerid 		= :is_data[1] 
			AND itemcod  			= :is_data[2] 
			AND rownum 				= 1;
		...
		//청구년월 SELECT
		SELECT BILCYCLE		  INTO :ls_bilcycle		  FROM BILLINGINFO
		 WHERE CUSTOMERID = :is_data[1];
		...
		If ls_bilcycle <> "" Then
			SELECT ADD_MONTHS(REQDT, 1)  INTO :ld_reqdt  FROM REQCONF
	...
	Case "b1w_reg_quotainfo_pop_SAMS%hw_save"
//		lu_dbmgr.is_data[1] = is_customerid
//		lu_dbmgr.is_data[2] = is_main_itemcod
//		lu_dbmgr.is_data[3] = is_orderno
//		lu_dbmgr.is_data[4] = gs_user_id
...
//    lu_dbmgr.idw_data[2] = dw_detail2
		...
		Else
			...
			For ll_rows = 1 To idw_data[2].RowCount()
				...
				ls_contno   	= idw_data[2].object.contno[ll_rows]  			//장비 ControlNo
				ls_adtype     	= Trim(idw_data[2].object.adtype[ll_rows])
				ll_hwseq      	= idw_data[2].object.adseq[ll_rows]
				ls_itemcod1    = idw_data[2].object.itemcod[ll_rows]
				...
				Insert Into ad_salelog ( saleseq, 	
					             ...
					             Values ( seq_ad_saleseq.nextval, 
									 			 :ldt_shop_closedt, 		:GS_SHOPID,	1,					:ldc_saleamt,		
												 :ldc_saleamt,				null, 		:ls_modelno, 	:ls_contno 		);
			...
			If ll_quotacnt = 0 Then
				...
				Insert Into partner_ardtl ( 
							seq, 
							partner, 		trdt, 		artrcod, 		tramt, 			org_partner,
							customerid, 	orderno, 	svccod, 			itemcod, 		crt_user, 
							updt_user,		crtdt, 		updtdt, 			pgm_id							)
				Values ( 
							seq_partnerar.nextval, 
							:is_data[8], 	sysdate, 	:ls_artrcod, 	:ldc_saleamt, 	:is_data[8],
							:is_data[1], 	:ll_orderno,:is_data[9], 	null, 			:is_data[4], 
							:is_data[4],	sysdate, 	sysdate, 		:is_data[5]						) ;
			...
			If ldc_beforeamt <> 0 Then
				...
				Values ( 
							seq_partnerar.nextval, 
							:is_data[8], 		sysdate, 		:ls_artrcod, 	:ldc_beforeamt, 	:is_data[8],
							:is_data[1], 		:ll_orderno, 	:is_data[9], 	null, 				:is_data[4], 
							:is_data[4],		sysdate, 		sysdate, 		:is_data[5]						) ;
			...
			UPDATE partnermst
				SET tot_samt = nvl(tot_samt,0) + nvl((SELECT nvl(a.sale_amt,0)
				 	 ...
				 	 updtdt    = sysdate,
				 	 pgm_id    = :gs_pgm_id[gi_open_win_no]
			 WHERE partner = ( SELECT partner FROM partnermst
									  WHERE evelcod 				= :ls_levelcod
									    AND :ls_reg_prefixno 	like prefixno||'%'
									 	 AND credit_yn 			= 'Y');
	...
	Case "b1w_reg_quotainfo_pop_2_SAMS%inq"
		//장비 정보 가져오기
//		lu_dbmgr.is_data[1] = is_customerid
////		lu_dbmgr.is_data[2] = is_itemcod
//		lu_dbmgr.is_data[3] = is_orderno
...
//    lu_dbmgr.idw_data[2] = dw_detail2
		...
		//=====================--------===============------------===============
		DECLARE customer_hw_c CURSOR FOR
			SELECT sale_flag, 		adtype, 		serialno	  FROM customer_hw
			 WHERE customerid = :is_data[1]
			   AND orderno    = :is_data[3]
			   AND sale_flag  = :ls_sale_flag_hw;
		...
		DECLARE partner_ardtl_c CURSOR FOR
			SELECT artrcod, 		tramt	  FROM partner_ardtl
			 WHERE customerid 	= :is_data[1] 
			   AND orderno 		= :is_data[3] ;
		...
		Do While(True)
			...
			ls_sunsu = fs_get_control("B1", "H200", ls_ref_desc)
			If ls_artrcod = ls_sunsu Then			lc_beforeamt = lc_tramt
			
			ls_deposit 	= fs_get_control("B1", "H201", ls_ref_desc)
			If ls_artrcod = ls_deposit Then		lc_deposit = lc_tramt
		Loop
...
on b1u_dbmgr9_sams.create
call super::create
end on

on b1u_dbmgr9_sams.destroy
call super::destroy
end on
