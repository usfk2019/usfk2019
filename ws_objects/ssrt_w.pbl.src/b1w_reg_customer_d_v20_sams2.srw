$PBExportHeader$b1w_reg_customer_d_v20_sams2.srw
$PBExportComments$[1hera] 고객정보 등록 v20
forward
global type b1w_reg_customer_d_v20_sams2 from w_a_reg_m_tm2
end type
type p_view from u_p_view within b1w_reg_customer_d_v20_sams2
end type
type p_active from picture within b1w_reg_customer_d_v20_sams2
end type
type cb_comment from commandbutton within b1w_reg_customer_d_v20_sams2
end type
type cb_bill from commandbutton within b1w_reg_customer_d_v20_sams2
end type
type p_active_mobile from picturebutton within b1w_reg_customer_d_v20_sams2
end type
end forward
...
global b1w_reg_customer_d_v20_sams2 b1w_reg_customer_d_v20_sams2
...
on b1w_reg_customer_d_v20_sams2.create
int iCurrent
call super::create
...
on b1w_reg_customer_d_v20_sams2.destroy
call super::destroy
destroy(this.p_view)
...
//pgm_id
is_pgm_id = gs_pgm_id[gi_open_win_no]
...
ls_mac			= Trim(dw_cond.object.mac[1])
...
ELSE	
	...
	IF IsNull(ls_mailid) 		Then ls_mailid 		= "" 
	...
	IF ( ls_customerid = "" AND ls_memberid = ""  AND ls_mailid = "" AND ls_firstname = "" AND ls_lastname = "" AND ls_bil_email = "" AND ls_homephone = "" AND ls_buildingno = "" AND ls_roomno = "" AND ls_cellphone = "" AND ls_mac = ""  AND ls_validkey = "") THEN
	...
	If ls_memberid <> "" 	Then 
		...
		ls_where += " cus.memberid like '" 	+ ls_memberid 	+ "%' " 
	END IF
	If ls_customerid <> "" 	Then 
		If ls_where <> "" 		Then ls_where += " And "
		ls_where += " cus.customerid like '" 	+ ls_customerid 	+ "%' " 
	END IF
	
	//E-Mail ID 값 조건내역 추가 요청 - 안경호사원(2011.10.17)
	//kem modify (2011.10.17)
//	If ls_mailid <> "" 		Then 
//		If ls_where <> "" 		Then ls_where += " And "
//		ls_where += " ( cus.mailid like '" 		+ ls_mailid 		+ "%' "
//	END IF
	If ls_mailid <> "" 		Then 
		If ls_where <> "" 		Then ls_where += " And "
		ls_where += " ( cus.email1 like '" + ls_mailid + "%' "
		ls_where += "  OR cus.email2 like '" + ls_mailid + "%' "
		ls_where += "  OR bil.bil_email like '" + ls_mailid + "%' ) "
	End If
	
	If ls_firstname <> "" 	Then 
		If ls_where <> "" 		Then ls_where += " And "
		ls_where += "UPPER(cus.firstname) like '"	+ UPPER(ls_firstname) 	+ "%' "
	END IF
	If ls_lastname <> "" 	Then 
		If ls_where <> "" 		Then ls_where += " And "
		ls_where += "UPPER(cus.lastname) like '"		+ UPPER(ls_lastname) 		+ "%' "
	END IF
	If ls_bil_email <> "" 	Then 
		If ls_where <> "" 		Then ls_where += " And "
		ls_where += " bil.bil_email like '" 	+ ls_bil_email 	+ "%' "
	END IF
	If ls_homephone <> "" 	Then 
		If ls_where <> "" 		Then ls_where += " And "
		ls_where += " ( cus.homephone like '" 	+ ls_homephone 	+ "%' OR cus.cellphone like '" + ls_homephone + "%') "
	END IF
	
	If ls_buildingno <> "" 	Then 
		If ls_where <> "" 		Then ls_where += " And "
		ls_where += " cus.buildingno like '%" 	+ ls_buildingno 	+ "%' "
	END IF

	If ls_roomno <> "" 	Then 
		If ls_where <> "" 		Then ls_where += " And "
		ls_where += " cus.roomno like '%" 	+ ls_roomno 	+ "%' "
	END IF
	
	If ls_cellphone <> "" 	Then 
		If ls_where <> "" 		Then ls_where += " And "
		ls_where += " ( cus.cellphone like '" 	+ ls_cellphone 	+ "%' OR cus.homephone like '" + ls_cellphone + "%') "
	END IF
//2011.08.18 이윤주 대리 요청사항...customer_hw 가 아닌 equipmst 로 조회될 수 있도록...
//	If ls_mac <> "" 	Then 
//		If ls_where <> "" 		Then ls_where += " And "
//		ls_where += " cus.customerid in (select customerid from customer_hw where serialno like '%" 	+ ls_mac 	+ "%' and adtype in ('116000','116001','116003','123001')) "
//	END IF
	
	If ls_mac <> "" 	Then 
		If ls_where <> "" 		Then ls_where += " And "
		ls_where += " cus.customerid in (select customerid from equipmst where mac_addr LIKE LOWER('" + ls_mac 	+ "')||'%' ) "
	END IF	

	If ls_validkey <> "" 	Then 
		If ls_where <> "" 		Then ls_where += " And "
		ls_where += " cus.customerid in (select customerid from validinfo where validkey like '" 	+ ls_validkey 	+ "%') "
	END IF	
...
Choose Case li_tab
	Case 1
	   ...
	   //납입자 오류
		...
		tab_1.idw_tabpage[li_tab].object.updt_user[1] 	= gs_user_id
		tab_1.idw_tabpage[li_tab].object.updtdt[1] 		= fdt_get_dbserver_now()
		tab_1.idw_tabpage[li_tab].object.pgm_id[1]      = is_pgm_id
...
type dw_cond from w_a_reg_m_tm2`dw_cond within b1w_reg_customer_d_v20_sams2
integer x = 82
integer y = 52
integer width = 3282
integer height = 368
string dataobject = "b1dw_1_cnd_reg_customer_sams2"
end type
...
type p_ok from w_a_reg_m_tm2`p_ok within b1w_reg_customer_d_v20_sams2
integer x = 3429
integer y = 40
end type

type p_close from w_a_reg_m_tm2`p_close within b1w_reg_customer_d_v20_sams2
integer x = 3730
integer y = 40
end type

type gb_cond from w_a_reg_m_tm2`gb_cond within b1w_reg_customer_d_v20_sams2
integer x = 37
integer width = 3365
integer height = 432
end type

type dw_master from w_a_reg_m_tm2`dw_master within b1w_reg_customer_d_v20_sams2
integer y = 464
integer width = 3534
integer height = 548
string dataobject = "b1dw_1_inq_customer_sams2"
end type
...
type p_insert from w_a_reg_m_tm2`p_insert within b1w_reg_customer_d_v20_sams2
integer y = 2056
end type

type p_delete from w_a_reg_m_tm2`p_delete within b1w_reg_customer_d_v20_sams2
integer y = 2056
end type

type p_save from w_a_reg_m_tm2`p_save within b1w_reg_customer_d_v20_sams2
integer x = 622
integer y = 2056
end type

type p_reset from w_a_reg_m_tm2`p_reset within b1w_reg_customer_d_v20_sams2
integer x = 1207
integer y = 2056
end type

type tab_1 from w_a_reg_m_tm2`tab_1 within b1w_reg_customer_d_v20_sams2
...
Choose Case li_tab
	Case 1								//Tab 1	
		...
		SELECT NVL(SUM(DECODE(B.IN_YN, 'N', A.TRAMT + NVL(A.TAXAMT,0), 0)) + SUM(DECODE(B.IN_YN, 'Y', A.TRAMT + NVL(A.TAXAMT,0), 0)), 0)
		INTO	 :ldc_balance
		...
		//신청내역 존재하면 고객 삭제 불가능...
		SELECT NVL(COUNT(ORDERNO),0)  INTO :ll_exist  FROM SVCORDER
...
//messagebox("확인","tab_1_constructor(1)")
//가로로 출력 
...
CLOSE CUSTOMER_cur;
//end
//messagebox("확인","tab_1_constructor(2)")
end event
...
//  b_default 버튼이 생기니 무조건 호출하게 돼서 comment 처리함.
...
//	OpenWithParm(b1w_inq_payid_billing_info, ls_payid)				//Open
	...
	Choose Case adwo_dwo.name
		Case "b_default" //기본 패스워드
			ls_ref_desc 	= ""
			ls_temp 			= ""
			ls_temp 			= fs_get_control("Z1", "Z200", ls_ref_desc)
			//messagebox("", ls_temp)
			If ls_temp 		= "" Then
				messagebox("", "설정된 기본패스워드가 없습니다.")
				Return -1
			end if
			
			tab_1.idw_tabpage[1].object.password[al_row] = ls_temp
			tab_1.idw_tabpage[1].object.password_invalid_cnt[al_row] = 0 //초기화
			tab_1.idw_tabpage[1].SetFocus()
			tab_1.idw_tabpage[1].SetColumn("password")
	end choose
End If
...
event tab_1::clicked;//messagebox("확인","tab_1_click")

STRING	ls_customerid,		ls_tab
LONG		ll_master_row
...
type st_horizontal from w_a_reg_m_tm2`st_horizontal within b1w_reg_customer_d_v20_sams2
integer y = 1024
end type
