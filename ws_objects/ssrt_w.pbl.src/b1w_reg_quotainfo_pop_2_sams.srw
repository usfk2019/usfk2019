$PBExportHeader$b1w_reg_quotainfo_pop_2_sams.srw
$PBExportComments$서비스 신청시 장비/할부 등록
forward
global type b1w_reg_quotainfo_pop_2_sams from w_a_reg_m
end type
type dw_detail2 from datawindow within b1w_reg_quotainfo_pop_2_sams
end type
end forward
...
global b1w_reg_quotainfo_pop_2_sams b1w_reg_quotainfo_pop_2_sams
...
public function integer wfi_get_hwseq (string as_contno, long al_row)
end prototypes
...
//기존의 자료가 있는지 확인
SELECT count(*)  INTO :il_cnt FROM quota_info
 WHERE to_char(orderno) = :as_orderno 
   AND customerid 		= :as_customerid;
...
If al_rowcount > 0 Then
	...
	For i = 1 To al_rowcount
		...
		SELECT A.UNITCHARGE
		  INTO :ldc_unitcharge
		  FROM PRICEPLAN_RATE2 A, ADMODEL_ITEM B
		 WHERE A.PRICEPLAN = :as_priceplan
		   AND A.ITEMCOD   = :is_itemcod[i]
			...
			AND TO_CHAR(A.fromdt, 'YYYYMMDD') = ( SELECT MAX(TO_CHAR(C.FROMDT, 'YYYYMMDD'))
															  FROM PRICEPLAN_RATE2 C, ADMODEL_ITEM D
...
Else
	...
	DECLARE itemcod_c	CURSOR FOR
		SELECT con.itemcod
		  FROM contractdet con, itemmst     itm
	...
	For i = 1 To j
		//신청일자 보다 작은것중에 가장 큰 날짜의 요금 가져옴
		SELECT A.UNITCHARGE  INTO :ldc_unitcharge
		  FROM PRICEPLAN_RATE2 A, 	ADMODEL B
		 WHERE A.PRICEPLAN = :as_priceplan
		   AND A.ITEMCOD   = :is_itemcod[i]
			AND A.ITEMCOD   = B.ITEMCOD
			AND TO_CHAR(A.fromdt, 'YYYYMMDD') = ( SELECT MAX(TO_CHAR(C.FROMDT, 'YYYYMMDD'))
															  FROM PRICEPLAN_RATE2 C, ADMODEL D
															 WHERE C.PRICEPLAN = :as_priceplan
															   AND C.ITEMCOD   = :is_itemcod[i]
																AND C.ITEMCOD   = D.ITEMCOD
																AND TO_CHAR(C.FROMDT,'YYYYMMDD') <= :as_orderdt);
...
//할부여서 기본값이 있을때
If il_cnt <> 0 Then
	SELECT Sum(sale_amt) INTO :ldc_saleamt FROM quota_info
	 WHERE to_char(orderno) = :as_orderno 
	   AND customerid 		= :as_customerid;
...
WHERE contno = :as_contno;

//Data Not Fount
If li_cnt = 0 Then
 	f_msg_usr_err(201, Title, "Control No")
	dw_detail2.SetColumn("contno")
	Return - 1
End If
...
on b1w_reg_quotainfo_pop_2_sams.create
int iCurrent
call super::create
...
on b1w_reg_quotainfo_pop_2_sams.destroy
call super::destroy
...
-------------------------------------------------------------------------*/
Integer 	li_rc, 				li_cnt, 		li_row, 		i,&
			ll_rowcount,		li_rows, 	li_rowcnt, 	li_count
String  	ls_ref_desc, 		ls_type, 	ls_name[], 	ls_reqnum_dw, &
			ls_itemcod, 		ls_modelno, ls_modelnm, ls_status, 		&
			ls_filter, 			ls_desc
Dec{6}  	ldc_unitcharge, 	ldc_basicamt

DataWindowChild 	ldc
b1u_dbmgr9_sams	lu_dbmgr
...
lu_dbmgr 				= Create b1u_dbmgr9_sams
lu_dbmgr.is_caller 	= "b1w_reg_quotainfo_pop_2_SAMS%getdata"
...
//	If li_row = -1 Then f_msg_usr_err(2100, Title, "GetChild() : Control No.")
...
SELECT currency_type INTO :ls_type FROM priceplanmst
 WHERE priceplan = :is_priceplan;

If ls_name[1] = ls_type Then
	dw_cond.object.basicamt.Format 	= "#,##0"
	dw_cond.object.beforeamt.Format 	= "#,##0"
	dw_cond.object.deposit.Format 	= "#,##0"
	dw_cond.object.receamt.Format 	= "#,##0"
	dw_cond.object.saleamt.Format 	= "#,##0"
Else
...
String 	ls_modelno, 	ls_serialno,	ls_contno
...
For ll_rows = 1 To dw_detail2.RowCount()
	...
	ls_contno 	= Trim(dw_detail2.object.contno[ll_rows])
...
If ls_gubun = "Y" Then
	...
	If ldc_saleamt <> 0 Then		dw_cond.object.saleamt[1] = ldc_saleamt
Else
...
If li_cnt > 0 Then
	//할부 개월수 계산
	lu_dbmgr.is_caller 		= "b1w_reg_quotainfo_pop_2_SAMS%insert"
	lu_dbmgr.is_title 		= Title
...
//장비 등록
lu_dbmgr 				= Create b1u_dbmgr9_SAMS
lu_dbmgr.is_caller 	= "b1w_reg_quotainfo_pop_2_SAMS%hw_save"
lu_dbmgr.is_title 	= Title
lu_dbmgr.is_data[1] 	= is_customerid
lu_dbmgr.is_data[2] 	= is_main_itemcod
lu_dbmgr.is_data[3] 	= is_orderno
...
//Seqence
FOR i = 1 TO ll_row
	SELECT seq_quota.nextval INTO :ll_seq 	FROM dual;
...
type dw_cond from w_a_reg_m`dw_cond within b1w_reg_quotainfo_pop_2_sams
integer x = 46
integer y = 40
integer width = 2962
integer height = 704
string dataobject = "b1dw_cnd_reg_quotainfo_pop_cl"
...
type p_ok from w_a_reg_m`p_ok within b1w_reg_quotainfo_pop_2_sams
integer x = 3058
integer y = 56
boolean originalsize = false
end type

type p_close from w_a_reg_m`p_close within b1w_reg_quotainfo_pop_2_sams
integer x = 3058
integer y = 180
boolean originalsize = false
end type

type gb_cond from w_a_reg_m`gb_cond within b1w_reg_quotainfo_pop_2_sams
integer width = 2994
integer height = 760
end type

type p_delete from w_a_reg_m`p_delete within b1w_reg_quotainfo_pop_2_sams
boolean visible = false
integer x = 1010
integer y = 1888
end type

type p_insert from w_a_reg_m`p_insert within b1w_reg_quotainfo_pop_2_sams
boolean visible = false
integer x = 709
integer y = 1888
end type

type p_save from w_a_reg_m`p_save within b1w_reg_quotainfo_pop_2_sams
integer x = 50
integer y = 1896
end type

type dw_detail from w_a_reg_m`dw_detail within b1w_reg_quotainfo_pop_2_sams
integer x = 23
integer y = 1124
integer width = 3342
integer height = 764
...
type p_reset from w_a_reg_m`p_reset within b1w_reg_quotainfo_pop_2_sams
integer x = 352
integer y = 1896
...
integer taborder = 11
string title = "none"
string dataobject = "b1dw_cnd_reg_quotainfo_pop_ad_sams"
...
//	
//	li_exist = dw_detail2.GetChild("contno", ldc_child)		//DDDW 구함
//	If li_exist = -1 Then f_msg_usr_err(2100, Title, "GetChild() : Control No.")
//	
