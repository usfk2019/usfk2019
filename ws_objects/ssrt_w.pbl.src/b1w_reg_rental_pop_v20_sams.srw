$PBExportHeader$b1w_reg_rental_pop_v20_sams.srw
$PBExportComments$[kem] 서비스 신청시 장비임대 등록
forward
global type b1w_reg_rental_pop_v20_sams from w_a_reg_m
end type
end forward
...
global b1w_reg_rental_pop_v20_sams b1w_reg_rental_pop_v20_sams
...
on b1w_reg_rental_pop_v20_sams.create
call super::create
end on

on b1w_reg_rental_pop_v20_sams.destroy
call super::destroy
end on
...
String ls_null, ls_contno, ls_modelno
Dec    ldc_saleamt
...
ls_modelno = dw_detail.Object.modelno[1]

IF IsNull(ls_contno)  OR ls_contno = ''  THEN ls_contno = ''
IF IsNull(ls_modelno) OR ls_modelno = '' THEN ls_modelno = ''

IF ls_contno = '' THEN
	f_msg_info(9000, Title, "Control No. Error")
	Return LI_ERROR
END IF

IF ls_modelno = '' THEN
	f_msg_info(9000, Title, "Model Error")
	Return LI_ERROR
END IF	
...
IF is_save_chk = "N" THEN
	MessageBox(This.Title, "장비입력이 완료되지 않았습니다.")
   Return 1
ELSE
	RETURN 0
END IF	
...
type dw_cond from w_a_reg_m`dw_cond within b1w_reg_rental_pop_v20_sams
integer x = 46
integer y = 72
integer width = 2962
integer height = 364
...
type p_ok from w_a_reg_m`p_ok within b1w_reg_rental_pop_v20_sams
...
type p_close from w_a_reg_m`p_close within b1w_reg_rental_pop_v20_sams
integer x = 3058
integer y = 180
boolean originalsize = false
end type

type gb_cond from w_a_reg_m`gb_cond within b1w_reg_rental_pop_v20_sams
integer width = 2994
integer height = 460
end type

type p_delete from w_a_reg_m`p_delete within b1w_reg_rental_pop_v20_sams
boolean visible = false
integer x = 1010
integer y = 1888
end type

type p_insert from w_a_reg_m`p_insert within b1w_reg_rental_pop_v20_sams
boolean visible = false
integer x = 709
integer y = 1888
end type

type p_save from w_a_reg_m`p_save within b1w_reg_rental_pop_v20_sams
integer x = 50
integer y = 904
end type

type dw_detail from w_a_reg_m`dw_detail within b1w_reg_rental_pop_v20_sams
integer x = 37
integer y = 488
integer width = 2990
integer height = 388
...
type p_reset from w_a_reg_m`p_reset within b1w_reg_rental_pop_v20_sams
boolean visible = false
integer x = 352
integer y = 904
