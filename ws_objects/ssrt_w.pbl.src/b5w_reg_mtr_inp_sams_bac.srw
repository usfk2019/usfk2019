$PBExportHeader$b5w_reg_mtr_inp_sams_bac.srw
$PBExportComments$[1hera] 입금수동거래등록 - sams
forward
global type b5w_reg_mtr_inp_sams_bac from w_a_reg_m_sql
end type
type st_horizontal from statictext within b5w_reg_mtr_inp_sams_bac
end type
type dw_detail2 from u_d_indicator within b5w_reg_mtr_inp_sams_bac
end type
type dw_split from datawindow within b5w_reg_mtr_inp_sams_bac
end type
type p_reset from u_p_reset within b5w_reg_mtr_inp_sams_bac
end type
type cb_1 from commandbutton within b5w_reg_mtr_inp_sams_bac
end type
type cb_contract from commandbutton within b5w_reg_mtr_inp_sams_bac
end type
type cb_2 from commandbutton within b5w_reg_mtr_inp_sams_bac
end type
type cb_close2 from commandbutton within b5w_reg_mtr_inp_sams_bac
end type
type st_hotbill from statictext within b5w_reg_mtr_inp_sams_bac
end type
type r_1 from rectangle within b5w_reg_mtr_inp_sams_bac
end type
type r_2 from rectangle within b5w_reg_mtr_inp_sams_bac
end type
end forward
...
global b5w_reg_mtr_inp_sams_bac b5w_reg_mtr_inp_sams_bac
...
event ue_process;STRING 	ls_payid,		ls_paydt,			ls_trdt,				ls_trcod,			ls_transdt, 	&
...
//	lb_tramt 		= dw_split.Object.payamt[li_lp]	
	...
	SQLCA.UBS_REG_REQPAY(ls_payid,											is_paycod,		ls_saletrcod,		DATE(String(ls_paydt, '@@@@-@@-@@')),		&
								DATE(String(ls_req_trdt, '@@@@-@@-@@')),	ls_remark, 		ldc_tramt,			DATE(String(ls_transdt, '@@@@-@@-@@')),	&
...
IF ll_cnt > 0 and ldc_total = 0 then
	...
	FOR ll = 1 to ll_cnt
		ldc_payamt 		= dw_detail2.object.cp_amt[ll]
...
ELSE
	...
	FOR ll = 1 to ll_cnt
		...
		IF dw_detail2.object.COMPLETE_YN[ll] = 'N'  AND ldc_tramt <> 0 THEN
			...
			IF ldc_impack <> 0 THEN
				ldc_imnot 		= dw_detail2.object.impack_not[ll]
				ldc_im			= dw_detail2.object.impack_card[ll]
			END IF		
			...
			IF li_update = 1 THEN
				...
				ldc_income 		+= ldc_impack_in
				
				ldc_receive 	 = ldc_receive - ldc_impack_in
				idc_income_tot += ldc_impack_in
				ldc_total_prc 	+= ldc_impack_in
...
on b5w_reg_mtr_inp_sams_bac.create
int iCurrent
call super::create
...
on b5w_reg_mtr_inp_sams_bac.destroy
call super::destroy
...
type dw_cond from w_a_reg_m_sql`dw_cond within b5w_reg_mtr_inp_sams_bac
integer x = 46
integer y = 20
integer width = 2720
integer height = 616
...
type p_ok from w_a_reg_m_sql`p_ok within b5w_reg_mtr_inp_sams_bac
integer x = 3118
integer y = 60
integer width = 274
...
type p_close from w_a_reg_m_sql`p_close within b5w_reg_mtr_inp_sams_bac
integer x = 681
integer y = 1956
end type

type gb_cond from w_a_reg_m_sql`gb_cond within b5w_reg_mtr_inp_sams_bac
integer width = 3397
integer height = 644
integer taborder = 0
integer textsize = -5
...
type p_save from w_a_reg_m_sql`p_save within b5w_reg_mtr_inp_sams_bac
integer x = 27
integer y = 1956
end type

type dw_detail from w_a_reg_m_sql`dw_detail within b5w_reg_mtr_inp_sams_bac
integer x = 27
integer y = 664
integer width = 3401
integer height = 632
