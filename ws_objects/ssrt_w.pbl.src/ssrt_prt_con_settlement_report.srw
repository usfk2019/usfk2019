$PBExportHeader$ssrt_prt_con_settlement_report.srw
$PBExportComments$[kem] AAFES settlement report 출력물
forward
global type ssrt_prt_con_settlement_report from w_a_print
end type
end forward
...
global ssrt_prt_con_settlement_report ssrt_prt_con_settlement_report
...
on ssrt_prt_con_settlement_report.create
call super::create
end on

on ssrt_prt_con_settlement_report.destroy
call super::destroy
end on
...
//	
ElseIF ls_regtype = '02' Then                       //MOBILE  
...
//	dw_list.Object.t_contractfee.Text = ls_contractfee + '% of sales'
Else 					//CATV
	ls_contype 		= fs_get_control("R1", "R170", ls_ref_desc)
	dw_list.Object.t_type.Text = ls_contype
	
	ls_contractno 	= fs_get_control("R1", "R180", ls_ref_desc)
	dw_list.Object.t_contractno.Text = ls_contractno
...
//SHOP/TYPE별 contractfee 가져오기
SELECT  RATE INTO :ldc_confee 
FROM COMMISSION_RATE
WHERE SHOP = :ls_partner
     AND TYPE = :ls_regtype
     AND FROMDT =  (SELECT MAX(FROMDT) FROM COMMISSION_RATE
                                                                WHERE SHOP = :ls_partner 
                                                                    AND TYPE = :ls_regtype
                                                                    AND TO_CHAR(FROMDT,'YYYYMM') <= TO_CHAR(:ldt_fromdt, 'YYYYMM'));
									  
ls_contractfee = STRING(ldc_confee)							
dw_list.Object.t_contractfee.Text = ls_contractfee + '% of sales'							
...
DO WHILE SQLCA.SQLCODE = 0 
	...
	ElseIf ls_regtype = '02' AND ll_keynum = 6 Then
		dw_list.Object.lsw[ll_row] = '1'
	ElseIf ls_regtype = '03' AND ll_keynum = 7 THEN
		dw_list.Object.lsw[ll_row] = '1'
	Else
...
// 매출이 없어도 조회되도록 2016-03-25 by HMK
...
type dw_cond from w_a_print`dw_cond within ssrt_prt_con_settlement_report
...
type p_ok from w_a_print`p_ok within ssrt_prt_con_settlement_report
boolean visible = false
integer x = 2318
integer y = 60
end type

type p_close from w_a_print`p_close within ssrt_prt_con_settlement_report
integer x = 2619
integer y = 60
end type

type dw_list from w_a_print`dw_list within ssrt_prt_con_settlement_report
integer y = 260
integer width = 3232
integer height = 1348
...
type p_1 from w_a_print`p_1 within ssrt_prt_con_settlement_report
end type

type p_2 from w_a_print`p_2 within ssrt_prt_con_settlement_report
end type

type p_3 from w_a_print`p_3 within ssrt_prt_con_settlement_report
end type

type p_5 from w_a_print`p_5 within ssrt_prt_con_settlement_report
end type

type p_6 from w_a_print`p_6 within ssrt_prt_con_settlement_report
end type

type p_7 from w_a_print`p_7 within ssrt_prt_con_settlement_report
end type

type p_8 from w_a_print`p_8 within ssrt_prt_con_settlement_report
end type

type p_9 from w_a_print`p_9 within ssrt_prt_con_settlement_report
end type

type p_4 from w_a_print`p_4 within ssrt_prt_con_settlement_report
end type

type gb_1 from w_a_print`gb_1 within ssrt_prt_con_settlement_report
end type

type p_port from w_a_print`p_port within ssrt_prt_con_settlement_report
end type

type p_land from w_a_print`p_land within ssrt_prt_con_settlement_report
end type

type gb_cond from w_a_print`gb_cond within ssrt_prt_con_settlement_report
boolean visible = false
integer width = 2254
integer height = 240
end type

type p_saveas from w_a_print`p_saveas within ssrt_prt_con_settlement_report
end type
