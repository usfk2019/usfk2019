$PBExportHeader$ssrt_reg_adsale_sams1_bac.srw
$PBExportComments$[1hera ] Sales Total
forward
global type ssrt_reg_adsale_sams1_bac from w_a_reg_m_m3_sams
end type
type dw_itemlist from datawindow within ssrt_reg_adsale_sams1_bac
end type
end forward
...
global ssrt_reg_adsale_sams1_bac ssrt_reg_adsale_sams1_bac
...
IF dw_sale.RowCount() > 0 THEN
	ldc_tot1 	=  dw_sale.GetItemNumber(dw_sale.RowCount(), "cp_total")
END IF

IF dw_detail.RowCount() > 0 THEN
	ldc_tot2 += dw_detail.GetItemNumber(dw_detail.RowCount(), "cp_total") 
END IF
IF dw_detail2.RowCount() > 0 THEN
	ldc_tot3 += dw_detail2.GetItemNumber(dw_detail2.RowCount(), "cp_total") 
END IF
...
// regcod read
select regcod, ITEMNM, PRIORITY INTO :ls_regcod , :ls_itemnm, :ll_priority FROM ITEMMST
...
// regcod read
SELECT regcod, itemnm, priority INTO :ls_regcod, :ls_itemnm, :ll_priority
FROM ITEMMST
 WHERE ITEMCOD = :ls_itemcod ;
...
// Itemnm, regcod read
select itemnm, 		regcod,	priority   
  INTO :ls_itemnm, 	:ls_regcod,	:ll_priority
  FROM itemmst 
 where itemcod =  :ls_itemcod ;
...
on ssrt_reg_adsale_sams1_bac.create
int iCurrent
call super::create
...
on ssrt_reg_adsale_sams1_bac.destroy
call super::destroy
...
String 	ls_itemcod, 		ls_paydt, 		ls_customerid, 	ls_chk,	&
			...
			ls_svc_check,		ls_contractseq, ls_pid, 			ls_spec_item1
      ...
      ls_chk 	=  trim(dw_detail.Object.itemcod[ll])
		...
		IF IsNull(ldc_amt) OR ldc_amt = 0 OR ls_chk = '' then
			...
			IF ll_dep_cnt > 0 THEN
				...
				IF IsNull(ls_contseq) OR ls_contseq = "" THEN
					IF MessageBox("확인", "보증금 계약번호가 입력되지 않았습니다. 계속 진행하시겠습니까?", Question!, YesNo!, 2) = 2 THEN
						dw_detail.SetFocus()
						dw_detail.SetRow(ll)
...
type dw_cond from w_a_reg_m_m3_sams`dw_cond within ssrt_reg_adsale_sams1_bac
integer y = 44
integer width = 3168
integer height = 560
integer taborder = 0
...
type p_ok from w_a_reg_m_m3_sams`p_ok within ssrt_reg_adsale_sams1_bac
integer x = 3264
integer y = 36
end type

type p_close from w_a_reg_m_m3_sams`p_close within ssrt_reg_adsale_sams1_bac
integer x = 3264
integer y = 144
boolean originalsize = false
end type

type gb_cond from w_a_reg_m_m3_sams`gb_cond within ssrt_reg_adsale_sams1_bac
integer width = 3214
integer height = 636
integer taborder = 0
end type

type dw_master from w_a_reg_m_m3_sams`dw_master within ssrt_reg_adsale_sams1_bac
integer taborder = 0
end type

type dw_detail from w_a_reg_m_m3_sams`dw_detail within ssrt_reg_adsale_sams1_bac
integer x = 425
integer y = 1716
integer width = 5385
integer height = 368
...
String				ls_filter,	ls_partner,		ls_priceplan,		ls_itemcod,		ls_customerid,	&
						ls_sql,		ls_svccod
Long					ll_qty,		ll_cnt,			ll_deposit_cnt,	ll_dep_cnt,		ll_row
DataWindowChild 	ldc_priceplan, 	ldc_itemcod,	ldwc_contractseq
...
type p_insert from w_a_reg_m_m3_sams`p_insert within ssrt_reg_adsale_sams1_bac
integer x = 55
integer y = 2092
end type

type p_delete from w_a_reg_m_m3_sams`p_delete within ssrt_reg_adsale_sams1_bac
integer x = 347
integer y = 2092
end type

type p_save from w_a_reg_m_m3_sams`p_save within ssrt_reg_adsale_sams1_bac
integer x = 645
integer y = 2092
end type

type p_reset from w_a_reg_m_m3_sams`p_reset within ssrt_reg_adsale_sams1_bac
integer x = 946
integer y = 2092
end type

type dw_detail2 from w_a_reg_m_m3_sams`dw_detail2 within ssrt_reg_adsale_sams1_bac
integer x = 174
integer y = 1196
integer width = 5641
integer height = 456
...
type st_horizontal2 from w_a_reg_m_m3_sams`st_horizontal2 within ssrt_reg_adsale_sams1_bac
integer y = 1164
end type

type st_horizontal from w_a_reg_m_m3_sams`st_horizontal within ssrt_reg_adsale_sams1_bac
integer x = 23
integer y = 1656
end type

type dw_sale from w_a_reg_m_m3_sams`dw_sale within ssrt_reg_adsale_sams1_bac
integer x = 23
integer y = 640
integer width = 6377
integer height = 508
...
event dw_sale::getfocus;call super::getfocus;idw = this
is_colnm =  'contno'
wf_set_total()
