$PBExportHeader$b2w_prc_close_salecom.srw
$PBExportComments$[jsmnoh] 월별매출수수료마감
forward
global type b2w_prc_close_salecom from w_a_prc
end type
end forward
...
global b2w_prc_close_salecom b2w_prc_close_salecom

type variables
String is_lastSettleDt //최종 대리점지급수수료정산 작업일
end variables

on b2w_prc_close_salecom.create
call super::create
end on

on b2w_prc_close_salecom.destroy
call super::destroy
end on
...
SQLCA.B2CRT_SALECOMMISSION(ls_closedt,gs_user_id,ls_pgm_id, ldb_return, ls_errmsg, ldb_count)
...
ls_days = fs_get_control('A1', 'C311', ls_desc)				// 최종매출수수료대상기간
...
ls_temp = fs_get_control("A1", "C311", ls_ref_desc)
...
type p_ok from w_a_prc`p_ok within b2w_prc_close_salecom
integer x = 1275
integer y = 48
end type

type dw_input from w_a_prc`dw_input within b2w_prc_close_salecom
integer width = 1125
integer height = 192
...
type dw_msg_time from w_a_prc`dw_msg_time within b2w_prc_close_salecom
integer y = 860
integer width = 1851
integer height = 300
end type

type dw_msg_processing from w_a_prc`dw_msg_processing within b2w_prc_close_salecom
integer y = 296
integer width = 1851
integer height = 548
end type

type ln_up from w_a_prc`ln_up within b2w_prc_close_salecom
integer beginy = 276
integer endy = 276
end type

type ln_down from w_a_prc`ln_down within b2w_prc_close_salecom
integer beginy = 1172
integer endy = 1172
end type

type p_close from w_a_prc`p_close within b2w_prc_close_salecom
integer x = 1586
integer y = 48
end type

type gb_cond from w_a_prc`gb_cond within b2w_prc_close_salecom
integer width = 1170
integer height = 248
end type
