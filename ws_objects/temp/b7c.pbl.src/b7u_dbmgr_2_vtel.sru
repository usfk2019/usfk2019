$PBExportHeader$b7u_dbmgr_2_vtel.sru
$PBExportComments$[jybaek]
forward
global type b7u_dbmgr_2_vtel from u_cust_a_db
end type
end forward

global type b7u_dbmgr_2_vtel from u_cust_a_db
end type
global b7u_dbmgr_2_vtel b7u_dbmgr_2_vtel
...
String ls_msg1, ls_msg2, ls_msg3, ls_msg4, ls_msg5, ls_pay_method, ls_chargedt
int li_temp_cnt 
...
//	  End If	
	  ...
	  //If ls_msg_fromdt = ls_trdt Then
			SELECT msg_1, msg_2, msg_3, msg_4, msg_5
			  ...
			  FROM invoicemsg msg
			 WHERE msg.chargedt = :ls_chargedt
			   AND to_char(msg.trdt,'yyyymmdd')=:ls_trdt
			   AND msg.pay_method =:ls_pay_method;
//		Else
//			
//			SELECT msg_1, msg_2, msg_3, msg_4, msg_5
//			  INTO :ls_msg1, :ls_msg2, :ls_msg3, :ls_msg4, :ls_msg5		
...
//				AND to_char(msg.fromdt,'yyyymmdd') = :ls_msg_fromdt;				
		//End If
		...
		For ll_curRow = 1 To ll_rows
			...
			idw_data[1].Object.paydt[ll_curRow] = ls_paydt  //입금마감일 
...
String ls_paydt_year, ls_paydt_month, ls_paydt_day
String ls_chargedt
...
Choose Case is_caller
	...
	//lu_dbmgr.is_data2[7] = ls_chargedt
		...
		ls_chargedt = is_data2[7]
		
		ls_paydt_year = MidA(ls_paydt,1,4)
		ls_paydt_month = MidA(ls_paydt,6,2)
		ls_paydt_day = MidA(ls_paydt,9,2)
...
//		End If
		...
		For ll_curRow = 1 To ll_rows
			...
			idw_data[1].object.msg4[ll_curRow] = ls_msg4			
			idw_data[1].object.msg5[ll_curRow] = ls_msg5	
			idw_data[1].Object.paydt_year[ll_curRow] = ls_paydt_year
			idw_data[1].Object.paydt_month[ll_curRow] = ls_paydt_month
			idw_data[1].Object.paydt_day[ll_curRow] = ls_paydt_day
         ...
         ldc_btramt[20]=idw_data[1].Object.btramt20[ll_curRow]			
          ...
          //지로고객조회번호left
			ls_giro_customer_no_left = fs_giro_band_left_vtel(ls_payid, ls_trdt, li_delay_cnt , ls_delay_start_trdt)			
			
			idw_data[1].Object.gironum_left[ll_curRow] = ls_giro_customer_no_left

         //지로고객조회번호 right
			ls_giro_customer_no_right = fs_giro_band_right_vtel(ldc_curbalance+ldc_prebalance) 
...
//				AND to_char(msg.fromdt,'yyyymmdd') = :ls_msg_fromdt;				
		...
		//Message Box check END====================================================>			
		idw_data[1].Object.paydt_year[1] = ls_paydt_year
		idw_data[1].Object.paydt_month[1] = ls_paydt_month
		idw_data[1].Object.paydt_day[1] = ls_paydt_day
...
Choose Case is_caller
	...
	//lu_dbmgr.is_data2[12] = ls_chargedt
		...
		idw_data[1].Object.bil_addr2[1] = ls_addr2
		idw_data[1].Object.zipcode[1]   = String(ls_zipcode,"@@@-@@@")
		idw_data[1].Object.paydt[1] = ls_paydt
...
on b7u_dbmgr_2_vtel.create
call super::create
end on

on b7u_dbmgr_2_vtel.destroy
call super::destroy
end on
