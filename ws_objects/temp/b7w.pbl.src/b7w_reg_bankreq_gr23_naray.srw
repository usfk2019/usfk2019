$PBExportHeader$b7w_reg_bankreq_gr23_naray.srw
$PBExportComments$[jwlee] 자동이체(EDI) 출금의뢰_naray용
forward
global type b7w_reg_bankreq_gr23_naray from w_a_reg_m_sql
end type
type p_2 from u_p_create within b7w_reg_bankreq_gr23_naray
end type
type hpb_1 from hprogressbar within b7w_reg_bankreq_gr23_naray
end type
type p_filewrite from u_p_filewrite within b7w_reg_bankreq_gr23_naray
end type
end forward
...
global b7w_reg_bankreq_gr23_naray b7w_reg_bankreq_gr23_naray
...
String is_pay_type[], is_pay_method, is_table, is_coname, is_name
date id_trdt, id_actdt
...
lu_dbmgr.is_caller   = "GR23%FileWrite"
...
on b7w_reg_bankreq_gr23_naray.create
int iCurrent
call super::create
this.p_2=create p_2
...
on b7w_reg_bankreq_gr23_naray.destroy
call super::destroy
destroy(this.p_2)
destroy(this.hpb_1)
...
String ls_dir
Date ld_sysdt, ld_orderdt, ld_outdt, ld_date
Int	 li_cnt, li_day
Setnull(ld_date)

ld_sysdt = date(fdt_get_dbserver_now())

dw_cond.Object.chargedt[1] = '2'
...
If SQLCA.SQLCode < 0 Then
	f_msg_sql_err(this.title, "itemchanged-SELECT reqconf")
...
id_actdt = fd_date_next(ld_sysdt, Integer(li_day))
...
If is_filename[1] = "" Then
	...
	Close(This)
Else
	is_filenm = is_filename[1] + String(ld_outdt, "mmdd")
...
dw_cond.Object.browse[1] = is_filepath+is_filename[1]
is_filepath = is_filepath + is_filename[1]
...
IF(ls_outdt <= String(id_actdt, 'yyyymmdd')) THEN
   f_msg_usr_err(212, Title + "today:" + String(id_actdt, 'yyyy-mm-dd'), "출금의뢰일자")
	dw_cond.SetColumn("outdt")
	dw_cond.SetFocus()
	RETURN 
END IF
...
lu_dbmgr = Create b7u_dbmgr3_naray
lu_dbmgr.is_title    = Title
lu_dbmgr.is_caller   = "EDI GR23%Save"
lu_dbmgr.idw_data[1] = dw_detail
lu_dbmgr.is_data[1]  = is_filenm			           //filename
...
type dw_cond from w_a_reg_m_sql`dw_cond within b7w_reg_bankreq_gr23_naray
integer x = 46
integer y = 60
integer width = 2249
integer height = 604
string dataobject = "b7dw_cnd_reg_banktext_gr23"
...
type p_ok from w_a_reg_m_sql`p_ok within b7w_reg_bankreq_gr23_naray
integer x = 2405
integer y = 92
boolean originalsize = false
end type

type p_close from w_a_reg_m_sql`p_close within b7w_reg_bankreq_gr23_naray
integer x = 2711
integer y = 92
end type

type gb_cond from w_a_reg_m_sql`gb_cond within b7w_reg_bankreq_gr23_naray
integer x = 37
integer y = 8
integer width = 2281
integer height = 668
...
type p_save from w_a_reg_m_sql`p_save within b7w_reg_bankreq_gr23_naray
integer x = 2405
integer y = 220
end type

type dw_detail from w_a_reg_m_sql`dw_detail within b7w_reg_bankreq_gr23_naray
integer x = 37
integer y = 696
integer width = 2985
integer height = 1116
string dataobject = "b7dw_inq_reg_banktext_gr23"
end type
