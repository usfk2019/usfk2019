$PBExportHeader$b7w_reg_girotext_cv_1.srw
$PBExportComments$[chooys] 지로수납파일처리 CVNET 수정버젼
forward
global type b7w_reg_girotext_cv_1 from w_a_reg_m_sql
end type
type p_1 from u_p_fileread within b7w_reg_girotext_cv_1
end type
type p_payment from u_p_payment within b7w_reg_girotext_cv_1
end type
end forward
...
global b7w_reg_girotext_cv_1 b7w_reg_girotext_cv_1
...
String is_workdt
Date id_workdt
Dec{2} idc_transamt_tot, idc_transerr_tot
end variables
...
Long	ll_rows, ll_return, ll_adj_cnt
String 	ls_mmdd
...
ls_file_prefix = fs_get_control("B7","B130",ls_desc)
ls_pathname = ""
ls_file_name = ""
ls_extension = ""
...
b7u_dbmgr2 lu_dbmgr2 
lu_dbmgr2 = Create b7u_dbmgr2
...
If lu_dbmgr2.ii_rc < 0 THen
	...
	SetPointer(Arrow!)
	return
Elseif lu_dbmgr2.ii_rc = 0 then
	COMMIT;
	ls_count = lu_dbmgr2.is_data2[1]
	ls_workdt = lu_dbmgr2.is_data2[2]
	is_file_name = ls_file_name
	is_workdt = ls_workdt
	ldc_transerr_tot = lu_dbmgr2.ic_data[1]       //합계입금정정금액
	ll_adj_cnt = lu_dbmgr2.ii_data[1]             //수납구분(추가처리(C)/반송수납(D)) 존재

    IF ldc_transerr_tot > 0 Then              //합계입금정정금액이 존재할때(기존에 돈을 많이 넣었을때 차감액)
		If ll_adj_cnt > 0 Then                //수납구분(추가처리(C)/반송수납(D)) 존재 (기존에 돈을 작게 넣었을 때 추가처리)
			f_msg_info(3000, Title, "~r~n 1.입금정정금액(" + String(idc_transerr_tot) + "원)이 존재.~r~n" + &
						"   GR17파일(임금정정내역) 확인 후 수동처리요.~r~n~r~n" + &
						" 2.수납구분(추가처리(C)/반송수납(D)) 존재.~r~n   수납일자/수납구분/고객 데이타 확인요.~r~n~r~n" +&
						" **지로수납 파일입력(건수 : " + ls_count+ ")")	
		Else
			f_msg_info(3000, Title, " 1.입금정정금액(" + String(idc_transerr_tot) + "원)이 존재.~r~n" + &
									"   GR17파일(임금정정내역) 확인 후 수동처리요.~r~n~r~n" + &
									" **지로수납 파일입력(건수 : " + ls_count+ ")")	
		End If
		
	ElseIf ll_adj_cnt > 0 Then               //수납구분(추가처리(C)/반송수납(D)) 존재 (기존에 돈을 작게 넣었을 때 추가처리)
		
		f_msg_info(3000, Title, " 1.수납구분(추가처리(C)/반송수납(D)) 존재.~r~n   수납일자/수납구분/고객 데이타 확인요.~r~n~r~n" +&
								" **지로수납 파일입력(건수 : " + ls_count+ ")")	
    Else  //정상분만 있을 경우
...
IF isnull(ls_errmsg) Then ls_errmsg = ''
...
on b7w_reg_girotext_cv_1.create
int iCurrent
call super::create
this.p_1=create p_1
...
on b7w_reg_girotext_cv_1.destroy
call super::destroy
destroy(this.p_1)
...
Dec{0} ldc_takecnt, ldc_takeamt, ldc_transamt, ldc_commamt, ldc_transerr
...
SELECT status, nvl(takecnt,0), nvl(takeamt,0), nvl(transamt,0), nvl(commamt,0)
 INTO :ls_work_status, :ldc_takecnt, :ldc_takeamt, :ldc_transamt, :ldc_commamt
 FROM girotextstatus
WHERE file_name = :ls_file_name
 AND workdt = :ls_workdt;
...
IF LeftA(ls_file_name,4) = 'GR15' Then  //2004.09.20 수정 후 파일Name Prefix
	dw_detail.object.transamt_tot.Text = String(ldc_transamt, '#,##0')
	ldc_transerr  = ldc_takeamt - ldc_transamt - ldc_commamt
	dw_detail.object.transerr_tot.Text = String(ldc_transerr,'#,##0')
Else //2004.09.20 수정 전 자료도 조회하므로....
    dw_detail.object.transamt_tot.Text = String(ldc_takeamt - ldc_commamt, '#,##0')
	dw_detail.object.transerr_tot.Text = '0'
End IF
end event

type dw_cond from w_a_reg_m_sql`dw_cond within b7w_reg_girotext_cv_1
integer width = 2094
integer height = 496
string dataobject = "b7dw_cnd_reg_girotext_1"
...
type p_ok from w_a_reg_m_sql`p_ok within b7w_reg_girotext_cv_1
integer x = 2309
integer y = 116
boolean originalsize = false
end type

type p_close from w_a_reg_m_sql`p_close within b7w_reg_girotext_cv_1
integer x = 2619
integer y = 116
end type

type gb_cond from w_a_reg_m_sql`gb_cond within b7w_reg_girotext_cv_1
integer width = 2149
integer height = 552
end type

type p_save from w_a_reg_m_sql`p_save within b7w_reg_girotext_cv_1
boolean visible = false
end type

type dw_detail from w_a_reg_m_sql`dw_detail within b7w_reg_girotext_cv_1
integer x = 37
integer y = 576
integer width = 3099
integer height = 1284
string dataobject = "b7dw_reg_girotext_mst_cv_1"
end type
