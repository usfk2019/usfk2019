$PBExportHeader$b7w_reg_girotext_v20_vtel.srw
$PBExportComments$[juede] 지로수납파일처리 v20 => vtel (line mode)
forward
global type b7w_reg_girotext_v20_vtel from w_a_reg_m_sql
end type
type p_1 from u_p_fileread within b7w_reg_girotext_v20_vtel
end type
type p_payment from u_p_payment within b7w_reg_girotext_v20_vtel
end type
end forward
...
global b7w_reg_girotext_v20_vtel b7w_reg_girotext_v20_vtel
...
b7u_dbmgr2_v20 lu_dbmgr2 
lu_dbmgr2 = Create b7u_dbmgr2_v20

lu_dbmgr2.is_caller = "b7w_reg_girotext_v20_vtel%ue_fileread"
...
Elseif lu_dbmgr2.ii_rc = 0 then
	...
	IF ldc_transerr_tot > 0 Then              //합계입금정정금액이 존재할때(기존에 돈을 많이 넣었을때 차감액)
		If ll_adj_cnt > 0 Then                //수납구분(추가처리(C)/반송수납(D)) 존재 (기존에 돈을 작게 넣었을 때 추가처리)
			f_msg_info(3000, Title, "~r~n 1.입금정정금액(" + String(ldc_transerr_tot) + "원)이 존재.~r~n" + &
		  						         "   GR17파일(임금정정내역) 확인 후 수동처리요.~r~n~r~n" + &
											" 2.수납구분(추가처리(C)/반송수납(D)) 존재.~r~n   수납일자/수납구분/고객 데이타 확인요.~r~n~r~n" +&
											" **지로수납 파일입력(건수 : " + ls_count+ ")")	
		Else
			f_msg_info(3000, Title, " 1.입금정정금액(" + String(ldc_transerr_tot) + "원)이 존재.~r~n" + &
											"   GR17파일(임금정정내역) 확인 후 수동처리요.~r~n~r~n" + &
											" **지로수납 파일입력(건수 : " + ls_count+ ")")	
		End If
...
on b7w_reg_girotext_v20_vtel.create
int iCurrent
call super::create
this.p_1=create p_1
...
on b7w_reg_girotext_v20_vtel.destroy
call super::destroy
destroy(this.p_1)
...
type dw_cond from w_a_reg_m_sql`dw_cond within b7w_reg_girotext_v20_vtel
integer width = 2094
integer height = 496
...
type p_ok from w_a_reg_m_sql`p_ok within b7w_reg_girotext_v20_vtel
integer x = 2309
integer y = 116
boolean originalsize = false
end type

type p_close from w_a_reg_m_sql`p_close within b7w_reg_girotext_v20_vtel
integer x = 2619
integer y = 116
end type

type gb_cond from w_a_reg_m_sql`gb_cond within b7w_reg_girotext_v20_vtel
integer width = 2149
integer height = 552
end type

type p_save from w_a_reg_m_sql`p_save within b7w_reg_girotext_v20_vtel
boolean visible = false
end type

type dw_detail from w_a_reg_m_sql`dw_detail within b7w_reg_girotext_v20_vtel
integer x = 37
integer y = 576
integer width = 3099
integer height = 1284
string dataobject = "b7dw_reg_girotext_v20"
end type
