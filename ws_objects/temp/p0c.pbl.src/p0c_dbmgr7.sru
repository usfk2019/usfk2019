$PBExportHeader$p0c_dbmgr7.sru
$PBExportComments$[juede] 충전처리 anyuser (refillpolicy -기본료, 유효기간 적용)
forward
global type p0c_dbmgr7 from u_cust_a_db
end type
end forward

global type p0c_dbmgr7 from u_cust_a_db
end type
global p0c_dbmgr7 p0c_dbmgr7
...
Dec lc_tot_sale, lc_tot_price, lc_real_price
Integer li_return
Integer i
Integer li_tmp
Dec lc_issueseq
...
//2005.03.28 basic fee 추가
Dec{2} lc_basic_fee, lc_basic_rate, lc_basic
Long ll_extdays
...
//공통코드 시작
		...
		//iu_db.is_data[13] = is_wkflag2		//멘트언어
		//iu_db.is_data[14] = is_pid_len		//길이
		...
		////변수이름 수정 2005.03.28 end   -------------
		 ...
		 lc_basic_fee = ic_data[4]              //basic fee
		 lc_basic_rate = ic_data[5]             //basic rate
...
Choose Case is_caller
	...
	Case "p0w_prc_cardmst%sale_yes"
		...
		//실제 Balance 값 구하기 - 2005. 1. 26 y.k.min -----확인
		SELECT real_price
		INTO	:lc_real_price
		FROM SALEPRICEMODEL
		...
		FOR i=1 To lc_quantity
			...
			(:ls_pid,:ls_contno,:ls_status_2,:lc_issueseq,to_date(:ls_issuedt, 'yyyy-mm-dd'),:gs_user_id,:ls_priceplan,:ls_model,:ls_lotno,
			 :lc_outseq,:lc_price,0,:lc_sale_amt,:lc_real_price,:ls_partner_prefix,
...
Choose Case is_caller
	//1.판매출고확정 = 'N' - 카드발행만
	Case "p0w_prc_cardmst_2%sale_no"
      //-------------------------------------------------------------
...
//공통코드 끝		
	...
	//2.판매출고확정 = 'Y' - 카드발행&충전
	Case "p0w_prc_cardmst_2%sale_yes"
      //-------------------------------------------------------------
		//Refillpolicy 에서 기본료 & 기본율 & 연장일수 적용 2005.03.29 
		//iu_db.ic_data[1] = ic_price			//카드금액
		//iu_db.ic_data[2] = ic_sale_amt	   //판매금액  
...
//공통코드 끝		
	...
	//1.판매출고확정 = 'N' - 카드발행만
	Case "p0w_prc_cardmst_anyuser%sale_no"
      //-------------------------------------------------------------
		//Refillpolicy 에서 기본료 & 기본율 & 연장일수 적용 2005.03.29 
		//iu_db.ic_data[1] = ic_price			//카드금액
		//iu_db.ic_data[2] = ic_sale_amt	   //판매금액  
...
//공통코드 끝
	...
	//2.판매출고확정 = 'Y' - 카드발행&충전
	Case "p0w_prc_cardmst_anyuser%sale_yes"
      //-------------------------------------------------------------
		//Refillpolicy 에서 기본료 & 기본율 & 연장일수 적용 2005.03.29 
		//iu_db.ic_data[1] = ic_price			//카드금액
		//iu_db.ic_data[2] = ic_sale_amt	   //판매금액  
...
on p0c_dbmgr7.create
call super::create
end on

on p0c_dbmgr7.destroy
call super::destroy
end on
