$PBExportHeader$p0c_dbmgr7_v20.sru
$PBExportComments$[juede] 충전처리 anyuser (refillpolicy -기본료, 유효기간 적용)
forward
global type p0c_dbmgr7_v20 from u_cust_a_db
end type
end forward

global type p0c_dbmgr7_v20 from u_cust_a_db
end type
global p0c_dbmgr7_v20 p0c_dbmgr7_v20
...
String ls_indt, ls_orderdt, ls_wkflag2, ls_pid_len, ls_refill_yn

String ls_contno_prefix, ls_refill_place[]
Dec lc_last_contno, lc_seq_contno
Dec lc_now_contno
Int li_cardno_cipher

ii_rc = -1

Dec{2} lc_basic_fee, lc_basic_rate, lc_basic,lc_basic_minus_balance //2005.03.29
Long ll_extdays //2005.03.29
...
Choose Case is_caller
	//1.판매출고확정 = 'N' - 카드발행만
	Case "p0w_prc_cardmst_2_v20%sale_no"
      //-------------------------------------------------------------
		//Refillpolicy 에서 기본료 & 기본율 & 연장일수 적용 2005.03.29 
      ...
      //-------------------------------------------------------------
		//공통코드 시작
		 ...
		 lc_basic_fee = ic_data[4]			  //기본료   2005.03.29
       lc_basic_rate = ic_data[5]		  //기본료율 2005.03.29
		 ...
		 ll_extdays = il_data[1]           //연장일수  2005.03.29
		 ls_refill_yn = is_data[16]        //충전여부  2005.08.01
...
//    주석처리 ohj20050826   sequence 로 관리함.
//		//마지막 관리번호
...
//			ls_contno = ls_contno_prefix + fs_fill_zeroes(String(lc_now_contno),-(ll_contno_len)) //새 관리번호
         ...
         //ohj 20050826 관리번호 sequence로 관리함..
			SELECT seq_contno.nextval
			  INTO :lc_seq_contno
			  FROM dual;
			  
			 ls_contno = ls_contno_prefix + string(lc_seq_contno)
			...
			END IF
			   //balance  에서 기본료 제외 *******2004.03.29 ********start
				lc_basic_minus_balance = lc_price - (lc_price * (lc_basic_rate/100))
				...
				(pid,contno,status,issueseq,issuedt,issue_user,priceplan,pricemodel,lotno,
				 refillsum_amt,usedsum_amt,salesum_amt,balance,partner_prefix,refill_yn, 
				...
				(:ls_pid,:ls_contno,:ls_status_1,:lc_issueseq,to_date(:ls_issuedt, 'yyyy-mm-dd'),:gs_user_id,:ls_priceplan,:ls_model,:ls_lotno,
				 0,0,0,0,:ls_partner_prefix,:ls_refill_yn,
  			   ...
  			   //balance  에서 기본료 제외 *******2004.03.29 ********end
			...
			lc_basic_minus_balance =0  //초기화 2005.03.29
		Next
		...
		//마지막관리번호 업데이트
		//주석처리 ohj 20050826  sequence로 관리함.
	...
	//2.판매출고확정 = 'Y' - 카드발행&충전
	Case "p0w_prc_cardmst_2_v20%sale_yes"
      //-------------------------------------------------------------
		//Refillpolicy 에서 기본료 & 기본율 & 연장일수 적용 2005.03.29 
		//iu_db.ic_data[1] = ic_price			//카드금액
		//iu_db.ic_data[2] = ic_sale_amt	   //판매금액  
...
//			ls_contno = ls_contno_prefix + fs_fill_zeroes(String(lc_now_contno),-(ll_contno_len)) //새 관리번호
			...
			(:ls_pid,:ls_contno,:ls_status_2,:lc_issueseq,to_date(:ls_issuedt, 'yyyy-mm-dd'),:gs_user_id,:ls_priceplan,:ls_model,:ls_lotno,
			 ...
			 to_date(:ls_issuedt, 'yyyy-mm-dd'),to_date(:ls_issuedt, 'yyyy-mm-dd'),:ls_wkflag2,:ls_refill_yn,
			 sysdate,:gs_user_id,sysdate,:gs_user_id,:gs_pgm_id[gi_open_win_no]);
			...
			( refillseq, pid, contno, refilldt, refill_type, refill_amt, 
			  sale_amt, remark, eday, partner_prefix, crtdt, crt_user, pricemodel, priceplan, refill_place)
			Values 
			( seq_REFILLLOG.Nextval, :ls_pid, :ls_contno, to_date(:ls_opendt, 'yyyy-mm-dd'), :ls_refill_type, :lc_price,
			  :lc_sale_amt, :ls_remark, :ll_extdays , :ls_partner_prefix, sysdate, :gs_user_id,
			  :ls_model, :ls_priceplan, :ls_refill_place[3]);
...
on p0c_dbmgr7_v20.create
call super::create
end on

on p0c_dbmgr7_v20.destroy
call super::destroy
end on
