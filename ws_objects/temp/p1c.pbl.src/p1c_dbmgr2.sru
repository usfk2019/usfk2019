$PBExportHeader$p1c_dbmgr2.sru
$PBExportComments$[y.k.min]
forward
global type p1c_dbmgr2 from u_cust_a_db
end type
end forward

global type p1c_dbmgr2 from u_cust_a_db
end type
global p1c_dbmgr2 p1c_dbmgr2
...
Choose Case is_caller
	Case "p1w_reg_card_refill_anyuser%save"
		...
		//	If IsNull(ls_extdays) Then ls_extdays = ""
			...
			Update p_cardmst Set
					...
					salesum_amt = salesum_amt + :ic_data[2],  //sale amt
			...
			Values 
				...
				:ls_model, :ls_priceplan,:lc_basic);
	...
	Case "p1w_reg_card_refill_basic%save"
		lc_basic_fee = ic_data[4]
		lc_basic_rate = ic_data[3]
		ll_extdays = il_data[1]
		//2005.07.31 juede add --start
...
on p1c_dbmgr2.create
call super::create
end on

on p1c_dbmgr2.destroy
call super::destroy
end on
