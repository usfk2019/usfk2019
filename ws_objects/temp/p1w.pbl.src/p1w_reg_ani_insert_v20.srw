$PBExportHeader$p1w_reg_ani_insert_v20.srw
$PBExportComments$[ohj] Ani 번호 등록 v20
forward
global type p1w_reg_ani_insert_v20 from w_a_reg_m_m
end type
end forward

global type p1w_reg_ani_insert_v20 from w_a_reg_m_m
end type
global p1w_reg_ani_insert_v20 p1w_reg_ani_insert_v20
...
on p1w_reg_ani_insert_v20.create
call super::create
end on

on p1w_reg_ani_insert_v20.destroy
call super::destroy
end on
...
event type integer ue_extra_save();Long		ll_row, ll_rows, ll_cntani, ll_cnt, ll_rows1, ll_rows2
String	ls_anino, ls_pid , ls_fromdt, ls_langtype
String   ls_temp, ls_ref_desc, ls_svctype, ls_svccod, ls_todt, ls_sort, &
         ls_validkey1, ls_fromdt1, ls_todt1, ls_validkey2, ls_fromdt2, ls_todt2
...
For ll_row = 1 To ll_rows
	...
	If ls_todt <> '' Then
		If ls_fromdt > ls_todt Then 
			f_msg_usr_err(9000, Title, "적용 시작일보다 종료일이 크면 안됩니다. 확인하세요.")
	...
	SELECT count(pid) 
	  ...
	  FROM validinfo
	 WHERE pid     <> :ls_pid 
	   AND validkey = :ls_anino
		AND svccod   = :ls_svccod
		AND to_char(fromdt,'yyyymmdd') <= :ls_fromdt   
		And :ls_fromdt < nvl(to_char(todt,'yyyymmdd'),'99991231');
	
	If SQLCA.SQLCode < 0 Then
		f_msg_sql_err(title,  "Database select Error(validinfo)" )
		return -2
	End If
...
ls_sort = "validkey, string(fromdt,'yyyymmdd')"
...
For ll_rows1 = 1 To dw_detail.RowCount()
	ls_validkey1 = fs_snvl(dw_detail.object.validkey[ll_rows1], '')
	...
	For ll_rows2 = dw_detail.RowCount() To ll_rows1 - 1 Step -1
		...
		ls_validkey2 = fs_snvl(dw_detail.object.validkey[ll_rows2], '')
		...
		If (ls_validkey1 = ls_validkey2)  Then
			If ls_todt1 >= ls_fromdt2 Then
				f_msg_info(9000, Title, "ANI# [ "+ls_validkey2+ "] : 적용시작일이 중복됩니다.")
				Return -2
			End If
		End If		
	Next
	...
	If dw_detail.GetItemStatus(ll_rows1, 0, Primary!) = DataModified! THEN
		...
		dw_detail.object.pgm_id[ll_rows1]    = gs_pgm_id[gi_open_win_no]
	End If
Next
...
type dw_cond from w_a_reg_m_m`dw_cond within p1w_reg_ani_insert_v20
integer x = 59
integer y = 52
integer width = 2267
integer height = 204
...
type p_ok from w_a_reg_m_m`p_ok within p1w_reg_ani_insert_v20
end type

type p_close from w_a_reg_m_m`p_close within p1w_reg_ani_insert_v20
end type

type gb_cond from w_a_reg_m_m`gb_cond within p1w_reg_ani_insert_v20
integer height = 268
end type

type dw_master from w_a_reg_m_m`dw_master within p1w_reg_ani_insert_v20
string dataobject = "p1dw_reg_aninum_master_v20"
end type
...
type dw_detail from w_a_reg_m_m`dw_detail within p1w_reg_ani_insert_v20
string dataobject = "p1dw_reg_aninum_detail_v20"
end type
...
type p_insert from w_a_reg_m_m`p_insert within p1w_reg_ani_insert_v20
end type

type p_delete from w_a_reg_m_m`p_delete within p1w_reg_ani_insert_v20
end type

type p_save from w_a_reg_m_m`p_save within p1w_reg_ani_insert_v20
end type

type p_reset from w_a_reg_m_m`p_reset within p1w_reg_ani_insert_v20
end type

type st_horizontal from w_a_reg_m_m`st_horizontal within p1w_reg_ani_insert_v20
end type
