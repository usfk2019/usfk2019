$PBExportHeader$p1w_reg_card_refill.srw
$PBExportComments$[y.k.min] 카드 충전
forward
global type p1w_reg_card_refill from w_a_reg_m_sql
end type
end forward
...
global p1w_reg_card_refill p1w_reg_card_refill
...
on p1w_reg_card_refill.create
call super::create
end on

on p1w_reg_card_refill.destroy
call super::destroy
end on
...
iu_db = Create p1c_dbmgr

iu_db.is_title = Title
iu_db.is_caller = "p1w_reg_card_refill%save"
...
type dw_cond from w_a_reg_m_sql`dw_cond within p1w_reg_card_refill
integer x = 41
integer y = 60
integer width = 2181
integer height = 908
...
Choose Case dwo.name
	...
	Case "refill_type"
		   ...
		   If is_refill_type <> data Then
		      ...
		      ldc_refill_amt = This.object.refill_amt[1]
				...
				lc_rate = fdc_refill_rate(ls_partner, ls_priceplan, ls_refilldt, ldc_refill_amt)
	...
	Case "partner_prefix" 
		If is_refill_type <> ls_refill_type Then
			...
			ldc_refill_amt = This.object.refill_amt[1]
			ls_refilldt = String(This.object.refilldt[1], 'yyyymmdd')
			lc_rate = fdc_refill_rate(data, ls_priceplan, ls_refilldt, ldc_refill_amt)
			lc_sale_amt = ldc_refill_amt * (lc_rate /100)
			This.object.sale_amt[1] = lc_sale_amt
		End If	
	Case "refill_amt" 
		If is_refill_type <> ls_refill_type Then
			...
			ldc_refill_amt = This.object.refill_amt[1]
			ls_refilldt = String(This.object.refilldt[1], 'yyyymmdd')
			lc_rate = fdc_refill_rate(ls_partner, ls_priceplan, ls_refilldt, Dec(data))
			lc_sale_amt = ldc_refill_amt * (lc_rate /100)
			This.object.sale_amt[1] = lc_sale_amt
		End If
...
type p_ok from w_a_reg_m_sql`p_ok within p1w_reg_card_refill
integer x = 2359
boolean originalsize = false
end type

type p_close from w_a_reg_m_sql`p_close within p1w_reg_card_refill
integer x = 2359
end type

type gb_cond from w_a_reg_m_sql`gb_cond within p1w_reg_card_refill
integer width = 2240
integer height = 988
end type

type p_save from w_a_reg_m_sql`p_save within p1w_reg_card_refill
integer x = 2359
end type

type dw_detail from w_a_reg_m_sql`dw_detail within p1w_reg_card_refill
integer y = 1012
integer width = 2816
integer height = 1332
