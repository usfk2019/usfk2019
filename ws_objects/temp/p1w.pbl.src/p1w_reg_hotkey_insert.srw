$PBExportHeader$p1w_reg_hotkey_insert.srw
$PBExportComments$[ssong] 단축번호 등록
forward
global type p1w_reg_hotkey_insert from w_a_reg_m_m
end type
end forward

global type p1w_reg_hotkey_insert from w_a_reg_m_m
end type
global p1w_reg_hotkey_insert p1w_reg_hotkey_insert
...
on p1w_reg_hotkey_insert.create
call super::create
end on

on p1w_reg_hotkey_insert.destroy
call super::destroy
end on
...
event type integer ue_extra_save();Long		ll_row, ll_rows, ll_cnthotkey, ll_cnt
String	ls_pid, ls_hotkey , ls_rtelno
...
For ll_row = 1 To ll_rows
	...
	INTO :ll_cnt
	FROM rtel_hotkey
	WHERE pid = :ls_pid and hotkey = :ls_hotkey;
...
DateTime ld_enddt
String ls_svccod, ls_customerid, ls_svctype, ls_status, ls_contractseq, ls_rtelno, ls_langtype
String ls_tmp, ls_ref_desc, ls_result[]
Integer li_return

ll_cnt = dw_detail.RowCount()
If ll_cnt >= il_validkeycnt Then
	f_msg_usr_err(9000,title,"해당가격정책에 단축번호 등록은 ~r~n~r~n" +string(il_validkeycnt)+ "개까지 등록 가능합니다.")
	Return LI_ERROR
End If
...
dw_detail.Object.rtelno[ll_row] = ls_rtelno
...
type dw_cond from w_a_reg_m_m`dw_cond within p1w_reg_hotkey_insert
integer x = 59
integer y = 52
integer width = 2267
integer height = 204
...
type p_ok from w_a_reg_m_m`p_ok within p1w_reg_hotkey_insert
end type

type p_close from w_a_reg_m_m`p_close within p1w_reg_hotkey_insert
end type

type gb_cond from w_a_reg_m_m`gb_cond within p1w_reg_hotkey_insert
integer height = 268
end type

type dw_master from w_a_reg_m_m`dw_master within p1w_reg_hotkey_insert
...
type dw_detail from w_a_reg_m_m`dw_detail within p1w_reg_hotkey_insert
string dataobject = "p1dw_reg_hotkey_detail"
end type
...
type p_insert from w_a_reg_m_m`p_insert within p1w_reg_hotkey_insert
end type

type p_delete from w_a_reg_m_m`p_delete within p1w_reg_hotkey_insert
end type

type p_save from w_a_reg_m_m`p_save within p1w_reg_hotkey_insert
end type

type p_reset from w_a_reg_m_m`p_reset within p1w_reg_hotkey_insert
end type

type st_horizontal from w_a_reg_m_m`st_horizontal within p1w_reg_hotkey_insert
end type
