$PBExportHeader$p2w_reg_refillpolicy_1.srw
$PBExportComments$[juede] 판매금액 Rate 등록- 충전기본료적용
forward
global type p2w_reg_refillpolicy_1 from w_a_reg_m_m
end type
type p_all from u_p_all within p2w_reg_refillpolicy_1
end type
end forward
...
global p2w_reg_refillpolicy_1 p2w_reg_refillpolicy_1

type variables
String is_partner		//partner code
String  is_mode, is_partner_yn
end variables
...
on p2w_reg_refillpolicy_1.create
int iCurrent
call super::create
...
on p2w_reg_refillpolicy_1.destroy
call super::destroy
destroy(this.p_all)
end on

event ue_ok();call super::ue_ok;String ls_where, ls_partner, ls_partnernm, ls_union, ls_prefixno, ls_levelcod
Long ll_row
...
For i = 1 To ll_row
	...
	lc_extdays = dw_detail.Object.extdays[i]
	...
	If ls_priceplan = "" Then
		...
		Return -2
	Else
		If ls_partner = 'ALL' AND ls_priceplan <> 'ALL' Then
			f_msg_usr_err(9000, Title, "ALL 대리점에는 ALL 가격정책만 존재해야 합니다.")
			dw_detail.SetColumn("priceplan")
			dw_detail.SetRow(i)
			dw_detail.ScrollToRow(i)
			dw_detail.SetRedraw(True)
...
// kem 2005.8.11 0보다 작은 금액도 가능하도록 수정
//	If lc_from_amt < 0 Or IsNull(lc_from_amt) Then
	If IsNull(lc_from_amt) Then
   ...
   ElseIf lc_from_amt >= lc_to_amt Then
	...
	If IsNull(lc_extdays) Or lc_extdays <= 0 Then
		f_msg_usr_err(9000, Title, "연장일수를 입력하세요")
...
If is_partner_yn = 'Y' Then   //관리대상대리점이상만 판매출고 가능
	dw_cond.Modify("partner.dddw.name=''")
	dw_cond.Modify("partner.dddw.DataColumn=''")
	dw_cond.Modify("partner.dddw.DisplayColumn=''")
	dw_cond.Object.partner[1] = ''				
	dw_cond.Modify("partner.dddw.name=p2dc_dddw_control_par")
	dw_cond.Modify("partner.dddw.DataColumn='partner'")
	dw_cond.Modify("partner.dddw.DisplayColumn='partnernm'")
...
Else                          //모든 대리점 판매출고 가능
	...
	dw_cond.Modify("partner.dddw.DisplayColumn=''")
	dw_cond.Object.partner[1] = ''				
	dw_cond.Modify("partner.dddw.name=p2dc_dddw_control_par1")
	dw_cond.Modify("partner.dddw.DataColumn='partner'")
	dw_cond.Modify("partner.dddw.DisplayColumn='partnernm'")	
End IF
...
type dw_cond from w_a_reg_m_m`dw_cond within p2w_reg_refillpolicy_1
integer x = 37
integer y = 44
integer width = 2213
integer height = 180
string dataobject = "p2dw_cnd_refillpolicy_1"
boolean hscrollbar = false
boolean vscrollbar = false
boolean livescroll = false
end type

type p_ok from w_a_reg_m_m`p_ok within p2w_reg_refillpolicy_1
integer x = 2382
integer y = 72
end type

type p_close from w_a_reg_m_m`p_close within p2w_reg_refillpolicy_1
integer x = 2688
integer y = 72
end type

type gb_cond from w_a_reg_m_m`gb_cond within p2w_reg_refillpolicy_1
integer x = 23
integer y = 4
integer width = 2254
integer height = 240
...
type dw_master from w_a_reg_m_m`dw_master within p2w_reg_refillpolicy_1
integer x = 23
integer y = 268
integer height = 448
integer taborder = 30
string dataobject = "p2dw_inq_reg_refillpolicy_2"
end type
...
type dw_detail from w_a_reg_m_m`dw_detail within p2w_reg_refillpolicy_1
integer x = 23
integer y = 752
integer height = 888
integer taborder = 40
string dataobject = "p2dw_reg_refillpolicy_1"
end type
...
type p_insert from w_a_reg_m_m`p_insert within p2w_reg_refillpolicy_1
end type

type p_delete from w_a_reg_m_m`p_delete within p2w_reg_refillpolicy_1
integer y = 1656
end type

type p_save from w_a_reg_m_m`p_save within p2w_reg_refillpolicy_1
integer x = 617
integer y = 1656
end type

type p_reset from w_a_reg_m_m`p_reset within p2w_reg_refillpolicy_1
integer x = 1353
end type

type st_horizontal from w_a_reg_m_m`st_horizontal within p2w_reg_refillpolicy_1
integer y = 720
end type
