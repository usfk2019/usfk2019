$PBExportHeader$ubs_dbmgr_activeorder.sru
$PBExportComments$[jhchoi] 서비스 개통신청 (저장 Object) - 2009.05.06
forward
global type ubs_dbmgr_activeorder from u_cust_a_db
end type
end forward

global type ubs_dbmgr_activeorder from u_cust_a_db
end type
global ubs_dbmgr_activeorder ubs_dbmgr_activeorder
...
Long i, j, ll_row, ll_long, ll_result_sec, ll_priority, ll_addmonths, ll_item_cnt
...
Long ll_length[],ll_length_h, ll_non_cnt, ll_cnt_dup
...
long ll_callforwarding_info_seq, ll_addunit,ll_validity_term, ll_callingnum_cnt, ll_siid_cnt
date ld_call_fromdt, ld_call_todt, ld_item_todt[], ld_requestdt, ld_bil_fromdt, ld_item_todt_temp
String ls_bil_fromdt, ls_selfequip, ls_dacom_svctype,	ls_macaddr, ls_ref_desc2, ls_status2, ls_macaddr2
long ll_non_valid_cnt

ii_rc = -1
Choose Case is_caller
	Case "ubs_w_reg_activeorder%save"  //장비임대 포함 수정,입중계출중계서비스추가,인증KeyType별추가,착신전환부가서비스추가
...
//		ls_auth_method      = Trim(idw_data[1].object.auth_method[1])
		...
		ls_selfequip		  = Trim(idw_data[1].object.selfequip[1])
		ls_macaddr			  = Trim(idw_data[1].object.mac_addr[1])
		ls_macaddr2			  = Trim(idw_data[1].object.mac_addr2[1])
		ls_userid           = is_data[1]
		ls_pgm_id           = is_data[2]
...
//2009.11.02 JHCHOI 수정. fd_month_next 가 엉망이다. 2월을 전혀 생각하지 않는 funtion임!				
//수정전
...
//				End Choose		
//수정후		
				ll_addmonths = ll_addunit * ll_validity_term
				...
				Choose Case ls_item_method		
					Case ls_M_method     //월정액방식일경우		
						IF ll_validity_term > 0 Then
							...
							//플랜폰 서비스 아이템인지 확인
							SELECT COUNT(*) INTO :ll_item_cnt
							FROM   SYSCOD2T
							WHERE  GRCODE = 'UBS13'
							AND    CODE = :ls_itemcod[j];
							//플랜폰 아이템인 경우
							IF ll_item_cnt > 0 THEN	
								//매월 1일에 계약이 시작될 경우. 
								IF MidA(ls_bil_fromdt, 7, 2) = '01' THEN
									SELECT ADD_MONTHS(:ld_bil_fromdt, :ll_addmonths)
									INTO   :ld_item_todt_temp
									FROM   DUAL;
								ELSE
									SELECT ADD_MONTHS(:ld_bil_fromdt, :ll_addmonths) - 1
									INTO   :ld_item_todt_temp
									FROM   DUAL;									
								END IF
							ELSE
							...
							ld_item_todt[j] = ld_item_todt_temp
						End IF
...
//2009.11.02 JHCHOI 수정 END!!!
	   ...
	   //2009-11-27 jhchoi 수정.같은날 같은 고객번호로 해지된 인증키가 있는지 확인.ㅡㅡ;
		SELECT COUNT(VALIDKEY)
		INTO   :ll_cnt_dup
		FROM   VALIDINFO
		WHERE  VALIDKEY = :ls_validkey[1]
		AND    CUSTOMERID = :ls_customerid
		AND    TO_CHAR(TODT - 1, 'YYYYMMDD') = :ls_requestdt;
		
		IF ll_cnt_dup = 1 THEN
			UPDATE VALIDINFO
			SET    TODT = TO_DATE(:ls_requestdt, 'YYYYMMDD'),
					 UPDT_USER = 'DUP',
					 UPDTDT = SYSDATE
			WHERE  VALIDKEY = :ls_validkey[1]
		   AND    CUSTOMERID = :ls_customerid
		   AND    TO_CHAR(TODT - 1, 'YYYYMMDD') = :ls_requestdt;
			...
			If SQLCA.SQLCode < 0 Then
				f_msg_sql_err(is_title, " VALIDKEY Error(DUP)")
				RollBack;
				ii_rc = -1				
				Return 
			End If		
   	...
   	Update svcorder set related_orderno = :ll_orderno where orderno = :ll_related_orderno;
		...
		Insert Into svcorder (orderno, reg_prefixno, customerid, orderdt, requestdt,
					...
					crt_user, updt_user, crtdt, updtdt, pgm_id, remark,order_type,related_orderno, selfequip_yn )
		 Values (:ll_orderno, :ls_reg_prefix, :ls_customerid, to_date(:ls_orderdt,'yyyy-mm-dd'), to_date(:ls_requestdt,'yyyy-mm-dd'),
				 ...
				 :ls_userid, :ls_userid, sysdate, sysdate, :ls_pgm_id, :ls_remark, :ls_act_status,:ll_related_orderno, :ls_selfequip);
...
//		IF ls_priceplan = "PRCM06" OR ls_priceplan = "PRCM08" OR ls_priceplan = "PRCM18" OR ls_priceplan = "PRCM19" OR ls_priceplan = "PRWF02" THEN
//			ll_siid_cnt = 0
//		END IF
		
		IF ll_siid_cnt > 0 and ll_non_valid_cnt = 0  THEN
			
			SELECT DACOM_SVCTYPE INTO :ls_dacom_svctype
			FROM   PRICEPLANINFO
			WHERE  PRICEPLAN = :ls_priceplan;
			...
			If SQLCA.SQLCode <> 0 Then
				RollBack;
				ii_rc = -1
				f_msg_sql_err(is_title, is_caller + " SELECT PRICEPLANINFO Table(DACOM_SVCTYPE)")
				Return 
			End If			
			
			INSERT INTO SIID ( DACOM_SVCTYPE, CONTRACTSEQ, EX_SIID, ORDERNO,
									 STATUS, SIID, OIID, CRTDT, EMSG, UPDTDT, CUSTOMERID)			
			VALUES(:ls_dacom_svctype, '', :ll_orderno, :ll_orderno, '100', '0', '', SYSDATE, '', SYSDATE, :ls_customerid);
			
			If SQLCA.SQLCode <> 0 Then
				RollBack;
				ii_rc = -1
				f_msg_sql_err(is_title, is_caller + " Insert SIID Table")
				Return 
			End If
		END IF
		...
		IF ls_selfequip = 'Y' THEN
			ls_ref_desc2 = ""
			ls_status2 = fs_get_control("U0", "E800", ls_ref_desc2)
			
			UPDATE EQUIPMST
			SET    STATUS 	 	 = :ls_status2,
					 ORDERNO	  	 = :ll_orderno,
					 SPEC_ITEM1  = :ls_selfequip,
					 CONTRACTSEQ = NULL,
					 CUSTOMERID  = :ls_customerid
			WHERE  MAC_ADDR  	 = :ls_macaddr;
		
			IF SQLCA.SQLCODE <> 0 THEN		//For Programer
				RollBack;
				ii_rc = -1
				f_msg_sql_err(is_title, is_caller + " Update EQUIPMST Table")
				Return 
			END IF
			
			//WIFI 인 경우 장비가 2개까지 들어올 수 있어서..ㅡㅡ
			IF IsNull(ls_macaddr2) = FALSE OR ls_macaddr2 <> "" THEN
				UPDATE EQUIPMST
				SET    STATUS 	 	 = :ls_status2,
						 ORDERNO	  	 = :ll_orderno,
						 SPEC_ITEM1  = :ls_selfequip,
...
public subroutine uf_prc_db_07 ();/********************************************************************************/
/*	 개요 : 단말 할부 계약을 일시불 처리로 바꾸는 작업을 한다						  */
/*	        일시불 품목으로 계약아이템을 생성하고, 기존 할부 아이템은            */
/*			  더이상 청구되지 않도록 하는 것이 포인트..									  */
/*																										  */
/********************************************************************************/

string 	ls_userid,	ls_item_old, ls_item_new, 	ls_paytype, ls_remark, ls_pgm_id
long 		ll_orderno, ll_old_orderno, ll_old_seqno, ll_seqno
dec		ld_saleamt, ld_payamt


ii_rc = -1

Choose Case is_caller
	case "ubs_w_reg_chg_paytype%save"   // 모바일 할부-> 일시불 전환
		...
		if idw_data[3].rowcount() <= 0 then
			messagebox("단말정보", "처리할 할부정보가 없습니다.")
			Return
		end if

		//변경전 orderno
		ll_old_orderno = idw_data[3].object.orderno[1]
		if isnull(ll_old_orderno) or string(ll_old_orderno) = '' then
			messagebox("이전정보", "처리할 할부정보를 가져올 수 없습니다.")
			Return
		end if
		
		ll_old_seqno 	= idw_data[3].object.seqno[1]
		
		ls_paytype = idw_data[4].object.pay_type[1]
		ld_saleamt 		= idw_data[4].object.sale_amt[1]
		ld_payamt		= idw_data[4].object.pay_amt[1]
		ls_remark		= idw_data[4].object.remark[1]
	   ...
	   /*************************************************************************/
		/*     계약품목 처리 (CONTRACTDET)                                       */
		/*  	 할부품목 update, 일시불 품목 insert										 */
		/*************************************************************************/
		
		ls_item_old = idw_data[3].object.itemcod[1]
		ls_item_new = idw_data[4].object.itemcod[1]
				
		Insert into contractdet 
		( 	ORDERNO, ITEMCOD, CONTRACTSEQ, 
			BIL_FROMDT, BIL_TODT, BIL_CURFROMDT, 
			CUSTOM_PRICE, DC_PRICE, BIL_CURTODT, 
			BIL_CURFROMDT_BEF, BIL_CURTODT_BEF, CRT_USER, 
			UPDT_USER, CRTDT, UPDTDT)
		(select ORDERNO, :ls_item_new, CONTRACTSEQ, 
			trunc(sysdate), null, BIL_CURFROMDT, 
			:ld_payamt, DC_PRICE, BIL_CURTODT, 
			BIL_CURFROMDT_BEF, BIL_CURTODT_BEF, :ls_userid, 
			UPDT_USER, sysdate, UPDTDT 
		from contractdet
		where orderno = :ll_old_orderno
		  and itemcod = :ls_item_old);
	   ...
	   Update contractdet Set
			bil_todt 			= trunc(sysdate),
			custom_price 		= 0,
			updtdt 				= sysdate,
			updt_user 			= :ls_userid
		where orderno = :ll_old_orderno
		...
		/*************************************************************************/
		/*     단말기 할부 (CONT_QUOTASALEMST, CONT_QUOTASALEDET)                */
		/*************************************************************************/		
		
	   IF ls_paytype = 'S' then   // 일시불일때만 처리한다
							
				SELECT SEQ_CONT_QUOTASALEMST.nextval INTO :ll_seqno FROM DUAL;
				...
				INSERT INTO CONT_QUOTASALEMST
				(  SEQNO, 					ORDERNO, 				CONTRACTSEQ, 
					ITEMCOD, 				SALE_MONTH, 			SALE_AMT, 
					SALE_TYPE, 				QUOTA_MONTH_CNT, 		QUOTA_AMT, 
					PAYMENT_MONTH_CNT, 	ADSEQ_ADMST, 			CANCEL_YN, 
					CANCELDT, 				CANCEL_USER, 			REF_SEQNO, 
					SALEDT, 					DO_INVC_YN, 			MODELNO, 
					SALE_FLAG, 				REMARK, 					CRT_USER, 
					UPDT_USER, 				CRTDT, 					UPDT, 				PGM_ID )
				(SELECT  :ll_seqno, 		ORDERNO, 				CONTRACTSEQ, 
					:ls_item_new, 			trunc(sysdate), 						:ld_saleamt, 
					:ls_paytype, 				1, 		:ld_payamt, 
					0, 	ADSEQ_ADMST, 			CANCEL_YN, 
					CANCELDT, 				CANCEL_USER, 			:ll_old_seqno, 
					sysdate, 					'N', 			MODELNO, 
					SALE_FLAG, 				:ls_remark, 					:ls_userid, 
					null, 				sysdate, 					null, 				:ls_pgm_id
				 FROM CONT_QUOTASALEMST
				 WHERE SEQNO = :ll_old_seqno);
			
					
				If SQLCA.SQLCode < 0 Then
						f_msg_sql_err(is_title, is_caller + " Insert Error(CONT_QUOTASALEMST)")
						ii_rc = -1		
						RollBack;
						Return 
				End If	
				...
				UPDATE CONT_QUOTASALEMST SET
						CANCEL_YN 	= 'Y',
						CANCELDT		= TRUNC(SYSDATE),
						CANCEL_USER	= :ls_userid,
						DO_INVC_YN 	= 'N',
						UPDT      = SYSDATE,
						UPDT_USER 	= :ls_userid
				WHERE SEQNO = :ll_old_seqno;
				...
				//신규만 insert 하고, 기존 det는 냅둔다(mst의 cancel_yn으로 판단하기 때문에)
				INSERT INTO CONT_QUOTASALEDET
					(  SEQNO_QUOTASALEMST, 		QUOTA_MONTH, 	QUOTA_MONTH_AMT, 
						QUOTA_MONTH_CUSTOM_AMT, INVC_YN, 		PAYMENT_AMT, 
						RSEQNO, 						CRT_USER, 		UPDT_USER, 
						CRTDT, 						UPDT, 			PGM_ID)
					VALUES
					(	:ll_seqno,					trunc(sysdate),	:ld_payamt,
						0,								'N',				0,
						null,							:ls_userid,		null,
						sysdate,						null, 			:ls_pgm_id );
						
					If SQLCA.SQLCode < 0 Then
								f_msg_sql_err(is_title, is_caller + " Insert Error(CONT_QUOTASALEDET)")
								ii_rc = -1		
								RollBack;
								Return 
					End If	
...
public subroutine uf_prc_db_05 ();
/*** 서비스신청 및 공통관련 ****************************************************************************/
String ls_customerid, 		ls_svccod, 		ls_priceplan,  		ls_orderdt, 		ls_requestdt
String ls_reg_partner, 		ls_sale_partner, ls_maintain_partner,  ls_partner, ls_settle_partner
String ls_status, 			ls_ref_desc, 	ls_userid, 				ls_pgm_id
String ls_itemcod[], 		ls_check[], 		ls_temp
String ls_act_status, 		ls_term_status, 	ls_customerm        
Long 	 ll_orderno, 			ll_contractseq
Long 	 i, j, ll_row, ll_priority, ll_addmonths, ll_item_cnt
String ls_remark, ls_quota_status,  ls_reg_prefix
string ls_item_method,ls_method[],ls_M_method,ls_D_method
long 	 ll_addunit,ll_validity_term 
date 	 ld_item_todt[], ld_requestdt, ld_bil_fromdt, ld_item_todt_temp
String ls_bil_fromdt, ls_selfequip
string ls_reg_gubun, ls_penalty_cd


/***장비처리 관련*****************************************************************************************/
String ls_sale_flag_hw,		ls_adtype,		ls_serialno,		ls_modelnm,		ls_itemcod1,		ls_modelno,	 &
		 ls_sale_flag,			ls_adstatus,	ls_action,			ls_contno,		ls_paydt,			ls_log_action
long 	 ll_rows, ll_hwseq
dec	 ldc_saleamt


/***단말기 할부 관련 *************************************************************************************/
string 	ls_itemcod2, ls_paytype
dec		ld_saleamt,  ld_payamt
integer 	li_month, li_m, li_mm, li_item_cnt, li_modelitem_cnt
long 		ll_seqno
...
Choose Case is_caller
	Case "ubs_w_reg_activeorder%save"  
		...
		ls_reg_partner      = Trim(idw_data[1].object.reg_partner[1])
		ls_sale_partner     = Trim(idw_data[1].object.sale_partner[1])
		ls_settle_partner	  = Trim(idw_data[1].object.settle_partner[1])  //개통처
		ls_partner          = Trim(idw_data[1].object.partner[1])
		ls_remark           = Trim(idw_data[1].object.remark[1])
		ls_selfequip		  = Trim(idw_data[1].object.selfequip[1])
		ls_reg_gubun		  = Trim(idw_data[1].object.reg_gubun[1])  //가입구분(신규'N', 재가입'R')
		ls_penalty_cd		  = Trim(idw_data[3].object.penalty_cd[1]) //위약방식
		...
		IF Isnull(ls_bil_fromdt) or ls_bil_fromdt = "" Then ls_bil_fromdt = ls_requestdt
		IF Isnull(ld_bil_fromdt) or ld_bil_fromdt = 1900-01-01 Then ld_bil_fromdt = ld_requestdt
...
//		fi_cut_string(ls_temp, ";" , ls_crt_kind_code[])		
		...
		//svcorder insert
		Insert Into svcorder (orderno, reg_prefixno, customerid, orderdt, requestdt,
					status, svccod, priceplan, prmtype, maintain_partner, reg_partner, sale_partner, settle_partner,
					order_priority, hopenum, acttype, chgopt, vpricecod, partner, ref_contractseq,
					crt_user, updt_user, crtdt, updtdt, pgm_id, remark,order_type,related_orderno, selfequip_yn )
		 Values (:ll_orderno, :ls_reg_prefix, :ls_customerid, to_date(:ls_orderdt,'yyyy-mm-dd'), to_date(:ls_requestdt,'yyyy-mm-dd'),
				 :ls_act_status, :ls_svccod, :ls_priceplan, :ls_penalty_cd, :ls_maintain_partner, :ls_reg_partner, :ls_sale_partner, :ls_settle_partner,
				 :ll_priority, null, 'N', :ls_reg_gubun, null , :ls_partner, :ll_contractseq, 
				 :ls_userid, :ls_userid, sysdate, sysdate, :ls_pgm_id, :ls_remark, :ls_status ,null, :ls_selfequip);
		...
		//신청 품목  check
		ll_row = idw_data[4].RowCount()
		j = 1
		For i = 1 To ll_row
		   ls_check[i] = Trim(idw_data[4].object.chk[i])
			If ls_check[i] = "Y" or ls_check[i] = 'M' Then
				ls_itemcod[j] = Trim(idw_data[4].object.itemcod[i])
		...
		//contractdet insert
	   For i =1 To UpperBound(ls_itemcod[])
			Insert Into contractdet(orderno, itemcod, contractseq, bil_fromdt,bil_todt, crt_user, crtdt)
			    Values(:ll_orderno, :ls_itemcod[i], :ll_contractseq, to_date(:ls_bil_fromdt,'yyyy-mm-dd'),:ld_item_todt[i],:ls_userid,  sysdate );
		...
		CHOOSE CASE ls_selfequip
				
			CASE 'Y' //자가폰 (CUSTOMER_HW)
				
						//장비임대코드(customer_hw)
						ls_sale_flag_hw = fs_get_control("E1", "A710", ls_ref_desc)
						ls_sale_flag    = '1'
						
						//ls_adtype = fs_get_control("E1", "A601", ls_ref_desc) -> '103' 장비구분(단말기)
						
						ls_serialno = upper(idw_data[1].object.serialno[1]) //serialno
						ls_modelnm = upper(idw_data[1].object.modelno[1])   //modelno
						
						If IsNull(ls_serialno) Or ls_serialno = "" Then  //장비 등록할 내용이 없다는 것임
							f_msg_info(200, is_title, "serial No")
							idw_data[1].SetRow(1)
						...
						If IsNull(ls_modelnm) Or ls_modelnm = "" Then  //장비 등록할 내용이 없다는 것임
						...
						Insert Into customer_hw (hwseq, rectype, customerid, sale_flag, adtype,
										...
										Values ( seq_customerhwno.nextval, 'A', :ls_customerid, :ls_sale_flag_hw, 'PRT01',
													:ls_serialno, :ls_modelnm, :ll_orderno, :ls_userid, :ls_userid ,
													sysdate, sysdate, :ls_pgm_id, null);
			...
			CASE 'N' //UBS폰  (ADMST)
						...
						//장비이력행위(admstlog) : 출고(판매/임대)
						ls_action = fs_get_control("E1", "A301", ls_ref_desc)
						
						//장비재고상태코드(admstlog)
						ls_adstatus = fs_get_control("E1", "A103", ls_ref_desc)  //장비재고상태코드 : Saling Goods(판매된것)
						...
						For ll_rows = 1 To idw_data[2].RowCount()
							...
							Insert Into admstlog ( adseq, seq, action, status, actdt,
											...
											Values( :ll_hwseq, seq_admstlog.nextval, :ls_action, :ls_adstatus, sysdate,
													  :ls_customerid, :ls_reg_partner, :ls_userid, sysdate,:ls_pgm_id);
							...
							//ADMSTLOG_NEW 테이블에 ACTION 값 ( Item판매 )
							SELECT ref_content INTO :ls_log_action FROM sysctl1t 
							...
							INSERT INTO ADMSTLOG_NEW		
								( ADSEQ, SEQ, ACTION, ACTDT, FR_PARTNER, TO_PARTNER, CONTNO, STATUS, 
								  ...
								  PGM_ID, IDATE )
							SELECT ADSEQ, seq_admstlog.nextval, :ls_log_action, SYSDATE, SN_PARTNER, TO_PARTNER, CONTNO, STATUS,
									 SALEDT, MV_PARTNER, 1, SALE_AMT, 0, MODELNO, CUSTOMERID, CONTRACTSEQ,
									 ORDERNO, RETDT, NULL, REMARK, :ls_userid, SYSDATE, UPDT_USER, UPDTDT,
									 :ls_pgm_id, IDATE
							FROM   ADMST
							WHERE  ADSEQ = :ll_hwseq;
		...
		/*     단말기 할부 (CONT_QUOTASALEMST, CONT_QUOTASALEDET)                */
		/*************************************************************************/
	   if ls_selfequip = 'N' THEN   // UBS 폰일때만 단말할부에 Insert 한다.
				ls_itemcod2 = idw_data[3].object.itemcod[1]
				ld_saleamt = idw_data[3].object.sale_amt[1]
				ls_paytype = idw_data[3].object.pay_type[1]
				ld_payamt = idw_data[3].object.pay_amt[1]
				
				//단말품목
				select count(*) into :li_modelitem_cnt from admodel_item
				where itemcod = :ls_itemcod2;
	
				
				if ls_paytype = 'S' then //일시불
					li_month = 1
					if li_modelitem_cnt > 0 then
						ld_payamt  = idw_data[2].object.sale_amt[1]//출고가
						ld_saleamt = idw_data[2].object.sale_amt[1]//출고가
						
					end if
				else
					li_month = idw_data[3].object.sale_month[1]
				end if	
				...
				(  SEQNO, 					ORDERNO, 				CONTRACTSEQ, 
					...
					UPDT_USER, 				CRTDT, 					UPDT, 				PGM_ID )
				VALUES
				(	:ll_seqno, 				:ll_orderno, 			NULL,
					:ls_itemcod2, 			to_date(:ls_requestdt,'yyyy-mm-dd'), :ld_saleamt,
					:ls_paytype, 			:li_month, 				:ld_payamt,        
					0, 						:ll_hwseq, 				'N',
					null, 					null, 					null,
					sysdate, 				decode(:li_month, 1,'N','Y'), 						:ls_modelno, 
					null, 					null, 					:ls_userid,
					null, 					sysdate, 				null, 					:ls_pgm_id);
				...
				FOR li_m = 1 to li_month
					...
					(  SEQNO_QUOTASALEMST, 		QUOTA_MONTH, 	QUOTA_MONTH_AMT, 
						...
						CRTDT, 						UPDT, 			PGM_ID)
					VALUES
					(	:ll_seqno,					add_months(to_date(:ls_requestdt,'yyyy-mm-dd'), :li_mm),	:ld_payamt,
						0,								'N',				0,
						null,							:ls_userid,		null,
						sysdate,						null, 			:ls_pgm_id );
		...
		END IF
		/********************************************************************************************************************************/
		/* UPDATE CONTRACTDET.CUSTOM_PRICE                                                                                              */
		/* 일시불의 경우 admodel_item의itemcod(Mobile Sales)을 판매하게 되는데, 이 항목이 기존 priceplan_rate2에 세팅되어 있어서..      */
		/* 청구 과금시(Hotitemsale_M_cont, b5ItemSale_M) 기존 계약은 priceplan_rate2의 금액을 가져가고 있어서 분리할 수가 없다.         */
		/* cont_quotasalemst에 들어가는 신규계약에서는 priceplan_rate2에 세팅되어 있더라도, contractdet.custom_prcie로 과금하도록 한다. */
		/********************************************************************************************************************************/
		SELECT COUNT(*) INTO :li_item_cnt
		FROM CONTRACTDET
		WHERE ORDERNO  = :ll_orderno
		  AND ITEMCOD  = :ls_itemcod2;
		  
		IF li_item_cnt > 0 THEN 
			UPDATE CONTRACTDET SET
					CUSTOM_PRICE = :ld_payamt
			WHERE ORDERNO  = :ll_orderno
			  AND ITEMCOD  = :ls_itemcod2;
...
CASE "ubs_w_reg_activation%save" ////모바일서비스는 (grcode ='ZM103') 개통시 번호할당 하므로 해당 서비스에만 국한됨.
	
	string ls_validkey, ls_validkey_type, ls_crt_kind, ls_n_auth_method, ls_svc_type
	int li_validkey_cnt
	long ll_cnt, ll_cnt_dup
	
	//lu_dbmgr.idw_data[1] = dw_detail
	//lu_dbmgr.is_data[1]  = gs_user_id
	//lu_dbmgr.is_data[2]  = gs_pgm_id[gi_open_win_no]
	
	ls_priceplan = idw_data[1].object.svcorder_priceplan[1]
	ls_validkey  = idw_data[1].object.validkey[1]
	ls_requestdt = string(idw_data[1].object.svcorder_requestdt[1],'yyyymmdd')
	ls_svccod    = idw_data[1].object.svcorder_svccod[1]
	ls_customerid = idw_data[1].object.svcorder_customerid[1]
	ll_orderno	  = idw_data[1].object.svcorder_orderno[1]
	
	ls_ref_desc = ""
	//개통신청상태코드
 	ls_status = fs_get_control("B0", "P220", ls_ref_desc)
	 
	/*계약 중복 개통을 막기 위한 조치 - UBS_REG_ACTIVATION 프로시저 에서  v_svcorder.ref_contractseq > 0이면 중복에러 띄운다*/ 
	UPDATE SVCORDER SET
		REF_CONTRACTSEQ = 0
	WHERE ORDERNO = :ll_orderno;
	...
	SELECT SVCTYPE INTO :ls_svc_type
	FROM SVCMST
	WHERE SVCCOD = :ls_svccod;
	
	If sqlca.sqlcode < 0 Then
		f_msg_sql_err(is_title, is_caller +" Select SVCMST(SVCTYPE)")	
		ii_rc = -1
		Return 
	End If
		
	//해당 가격정책에 등록된 인증KEY 갯수가 0보다 클때 입력한 인증KEY check
	SELECT VALIDKEYCNT INTO :li_validkey_cnt 
	FROM PRICEPLANMST
	WHERE PRICEPLAN = :ls_priceplan;
	
	If sqlca.sqlcode < 0 Then
		f_msg_sql_err(is_title, is_caller +" Select PRICEPLANMST(VALIDKEYCNT)")	
		ii_rc = -1
		Return 
	End If
	
	IF li_validkey_cnt > 0   THEN
		
		SELECT b.validkey_type, b.crt_kind, b.auth_method INTO :ls_validkey_type, :ls_crt_kind, :ls_n_auth_method
		FROM priceplan_validkey_type a, validkey_type b
		WHERE type = 'VALIDKEY'
		  AND a.validkey_type = b.validkey_type
		  and a.priceplan = :ls_priceplan;
		  
		If sqlca.sqlcode < 0 Then
			f_msg_sql_err(is_title, is_caller +" Select validkey_type(validkey_type)")	
			ii_rc = -1
			Return 
		End If
		
		//인증KEY의 validkey_type에 따른 처리
		if ls_crt_kind = 'M' then
			...
			If ll_cnt > 0 Then
				f_msg_usr_err(9000, is_title, "[" + ls_validkey + "]에 적용시작일과 적용종료일이 중복됩니다.~r~n~r~n다시 입력하세요!!")
				idw_data[1].SetFocus()
				idw_data[1].SetRow(i)
				idw_data[1].ScrollToRow(i)
				idw_data[1].SetColumn("validkey")
				setnull(ls_validkey)
				idw_data[1].object.validkey[1] = ls_validkey
				ii_rc = -3					
				return 
			End if
			
			//같은날 같은 고객번호로 해지된 인증키가 있는지 확인
			SELECT COUNT(VALIDKEY)
			INTO   :ll_cnt_dup
			FROM   VALIDINFO
			...
			IF ll_cnt_dup = 1 THEN
				...
				If SQLCA.SQLCode < 0 Then
					f_msg_sql_err(is_title, is_caller + " VALIDKEY Error(DUP)")
					RollBack;
					ii_rc = -1				
					Return 
				End If	
			...
			Insert Into validinfo
					(validkey, fromdt, status, 
					 ...
					 auth_method, validitem1, validitem2, validitem3,
					 crt_user, updt_user, crtdt, updtdt, pgm_id, validkey_loc)
			 Values(:ls_validkey, to_date(:ls_requestdt,'yyyy-mm-dd'), :ls_status,
					 'N', null, :ls_svc_type,
					 null, :ls_customerid, :ls_svccod, :ls_priceplan,
					:ll_orderno, null, null,
					:ls_n_auth_method, null, null, null,
					:gs_user_id, :gs_user_id, sysdate, sysdate, :ls_pgm_id, null);
...
on ubs_dbmgr_activeorder.create
call super::create
end on

on ubs_dbmgr_activeorder.destroy
call super::destroy
end on
