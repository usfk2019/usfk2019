$PBExportHeader$w_pop_scan_change_order_del.srw
$PBExportComments$문서마스터 조회 및 ORDER변경 및 취소요청 삭제시(POP)
forward
global type w_pop_scan_change_order_del from w_base
end type
type cb_1 from commandbutton within w_pop_scan_change_order_del
end type
type cbx_1 from checkbox within w_pop_scan_change_order_del
end type
type p_reset from u_p_reset within w_pop_scan_change_order_del
end type
type p_ok from u_p_ok within w_pop_scan_change_order_del
end type
type dw_master from u_d_base within w_pop_scan_change_order_del
end type
type dw_cond from u_d_help within w_pop_scan_change_order_del
end type
type p_close from u_p_close within w_pop_scan_change_order_del
end type
type gb_2 from groupbox within w_pop_scan_change_order_del
end type
end forward
...
global w_pop_scan_change_order_del w_pop_scan_change_order_del
...
on w_pop_scan_change_order_del.create
int iCurrent
call super::create
...
on w_pop_scan_change_order_del.destroy
call super::destroy
destroy(this.cb_1)
destroy(this.cbx_1)
...
for i = 1 to li_rowcnt
	...
	if li_chk = 1 then
		DELETE ORDER_DOCGROUP
		WHERE  ORDERNO = :ll_orderno
		AND    U_REQNO = :il_u_reqno;
...
li_cnt = dw_master.retrieve(il_u_reqno)
...
integer height = 460
integer taborder = 30
string dataobject = "d_pop_scan_change_order_del"
