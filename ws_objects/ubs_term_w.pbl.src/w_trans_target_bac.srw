$PBExportHeader$w_trans_target_bac.srw
$PBExportComments$해지DB이관대상 고객추출
forward
global type w_trans_target_bac from w_a_reg_m_m
end type
end forward
...
global w_trans_target_bac w_trans_target_bac
...
on w_trans_target_bac.create
call super::create
end on

on w_trans_target_bac.destroy
call super::destroy
end on
...
event ue_ok;call super::ue_ok;//1.해지자 이관 2.보유기간 만료고객 
//1-1 추출전 상태 1-2 추출후 상태
//2-1 보유기간 추출전 상태 2-2 보유기간 추출후 상태

//조회 데이터 윈도우 작업이력(worklog) 조회, 작업결과(terms_target)
//1-1에서 대상일자 안에 대상기간내에 추출이력이 없을경우(worklog) 작업결과(쿼리 실제 대상자 sams테이블 고객 가져옴)
//만약 있을 경우 팝업 메시지 추출을 할 수 없음.. (약간 애매한 경우가 기간으로 되어 있어서 대상일자 정하기가 어려움)
//예를 들어 대상일자 2016년 10월 1일 2016년 10월 10일 로 (실제로는 6개월전) 쿼리에서 대상자를 가져와서 추출 등록햇을
//경우 다음에 할때는 10월 11일 부터 로 지정해줘어야함..
//1-2에서는 대상 기간에 작업이력이 있는 경우. worklog 조회하고 workno에 맞는 trans_target 보여주고 있음

// 보유기간 만료 선택시에도 마찬가지로 worklog 데이터가 없음. 2-1상태 (work_gubn=200) 위와 마찬가지로 조회시
// 대상일자안에 없을 경우에만 CUSTOMERM.TERM_TRANS_DT를 이용하여 작업결과에 값을 가져옴.
// 대상일자 54개월전 일자 사용 (좀 애매한 부분인데 worklog가 있고 없고는 대상일자(입력된)를 가지고 파악함.)
// 2-2 worklog가 있는 경우는 trans_target에서 가져옴..


string  ls_work_type, ls_status, ls_fdate, ls_tdate, ls_operator, ls_fdate2, ls_tdate2, ls_pre_fdate, ls_pre_tdate
...
//작업이력에 값이 존재하지 않을경우
if ll_rows = 0  then
	if ls_work_type = '100' then //해지자이관
		if ls_status = '100' then   //상태가 추출일경우만 조회함
		     li_ret = messagebox("확인", "해지자이관 추출작업 시작하시겠습니까?", Exclamation!, YESNO!, 2)
			if li_ret = 1 then
		...
		if ls_status = '100' then //상태가 추출일경우만 조회함
		     li_ret = messagebox("확인", "해지자이관 추출작업 시작하시겠습니까?", Exclamation!, YESNO!, 2)
			if li_ret = 1 then				
				dw_detail.retrieve( 0, '3', ls_fdate, ls_tdate)
			end if
...
date   ld_fdate, ld_tdate
long   ll_work_no, ll_rowcnt, ll_rcnt, ll_row
...
//ls_pre_fdate   = dw_cond.object.pre_fdate[1] 
//ls_pre_tdate   = dw_cond.object.pre_tdate[1] 
//ld_fdate       = date(ls_pre_fdate)
//ld_tdate       = date(ls_pre_tdate)
...
event ue_delete;int    li_cnt, li_row
long   ll_work_no
...
type dw_cond from w_a_reg_m_m`dw_cond within w_trans_target_bac
integer y = 64
integer width = 3145
integer height = 284
...
type p_ok from w_a_reg_m_m`p_ok within w_trans_target_bac
integer x = 3415
end type

type p_close from w_a_reg_m_m`p_close within w_trans_target_bac
integer x = 3721
end type

type gb_cond from w_a_reg_m_m`gb_cond within w_trans_target_bac
integer width = 3186
integer height = 380
end type

type dw_master from w_a_reg_m_m`dw_master within w_trans_target_bac
integer y = 412
integer width = 4151
integer height = 644
...
type dw_detail from w_a_reg_m_m`dw_detail within w_trans_target_bac
integer y = 1112
integer width = 4165
integer height = 896
...
type p_insert from w_a_reg_m_m`p_insert within w_trans_target_bac
integer x = 50
integer y = 2032
end type

type p_delete from w_a_reg_m_m`p_delete within w_trans_target_bac
integer x = 343
integer y = 2032
end type

type p_save from w_a_reg_m_m`p_save within w_trans_target_bac
integer x = 635
integer y = 2032
boolean enabled = true
end type

type p_reset from w_a_reg_m_m`p_reset within w_trans_target_bac
integer x = 928
integer y = 2032
boolean enabled = true
end type
...
type st_horizontal from w_a_reg_m_m`st_horizontal within w_trans_target_bac
integer y = 1068
end type
