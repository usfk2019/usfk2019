$PBExportHeader$b1w_reg_directpaycan_pop_sams.srw
$PBExportComments$[jhchoi] 서비스 신청시 즉시불 취소 - 2009.06.20
forward
global type b1w_reg_directpaycan_pop_sams from w_a_reg_m
end type
type dw_detail2 from datawindow within b1w_reg_directpaycan_pop_sams
end type
end forward
...
global b1w_reg_directpaycan_pop_sams b1w_reg_directpaycan_pop_sams
...
on b1w_reg_directpaycan_pop_sams.create
int iCurrent
call super::create
...
on b1w_reg_directpaycan_pop_sams.destroy
call super::destroy
...
DEC{2}	ldc_unitamt,		ldc_unititem
date 		ldt_paydt
...
//START BY HMK
//환불처리시 받은 PAYMETHOD 대로 돌려주기
//[RQ-UBS-201304-03]에 의해 작업함 
DEC{2}	ldc_amt1, ldc_amt2, ldc_amt3, ldc_amt4, ldc_amt5, ldc_amt6



SELECT      SUM(DECODE( A.PAYMETHOD, :is_method[1], A.PAYAMT+ NVL(A.TAXAMT,0), 0)) * -1 AS AMT1,
            SUM(DECODE( A.PAYMETHOD, :is_method[2], A.PAYAMT+ NVL(A.TAXAMT,0), 0)) * -1 AS AMT2,
            SUM(DECODE( A.PAYMETHOD, :is_method[3], A.PAYAMT+ NVL(A.TAXAMT,0), 0)) * -1 AS AMT3,
            SUM(DECODE( A.PAYMETHOD, :is_method[4],A.PAYAMT+ NVL(A.TAXAMT,0), 0)) * -1 AS AMT4,
            SUM(DECODE(A.PAYMETHOD, :is_method[5], A.PAYAMT+ NVL(A.TAXAMT,0), 0)) * -1 AS AMT5,
            SUM(DECODE(A.PAYMETHOD, :is_method[6], A.PAYAMT+ NVL(A.TAXAMT,0), 0)) * -1 AS AMT6
	INTO :ldc_amt1, :ldc_amt2, :ldc_amt3, :ldc_amt4, :ldc_amt5, :ldc_amt6
FROM DAILYPAYMENT A
WHERE CUSTOMERID = :is_customerid
    AND ORDERNO  = :is_orderno
    AND APPROVALNO = (SELECT DISTINCT APPROVALNO FROM DAILYPAYMENT WHERE CUSTOMERID = A.CUSTOMERID AND ORDERNO = A.ORDERNO AND PGM_ID = 'DIRECT')
    AND PGM_ID IN ('DIRECT', 'REISSUE');

//SELECT      SUM(DECODE( PAYMETHOD, :is_method[1], PAYAMT, 0)) * -1 AS AMT1,
//            SUM(DECODE( PAYMETHOD, :is_method[2], PAYAMT, 0)) * -1 AS AMT2,
//            SUM(DECODE( PAYMETHOD, :is_method[3], PAYAMT, 0)) * -1 AS AMT3,
//            SUM(DECODE( PAYMETHOD, :is_method[4], PAYAMT, 0)) * -1 AS AMT4,
//            SUM(DECODE( PAYMETHOD, :is_method[5], PAYAMT, 0)) * -1 AS AMT5,
//            SUM(DECODE( PAYMETHOD, :is_method[6], PAYAMT, 0)) * -1 AS AMT6
//				INTO :ldc_amt1, :ldc_amt2, :ldc_amt3, :ldc_amt4, :ldc_amt5, :ldc_amt6
//FROM DAILYPAYMENT
//WHERE CUSTOMERID = :is_customerid
//    AND ORDERNO  = :is_orderno
//    AND APPROVALNO = (SELECT APPROVALNO FROM DAILYPAYMENT
//    AND PGM_ID IN ('DIRECT', 'REISSUE');
if ldc_amt1 <> 0 then dw_cond.object.amt1[1] = ldc_amt1
if ldc_amt2 <> 0 then dw_cond.object.amt2[1] = ldc_amt2
if ldc_amt3 <> 0 then dw_cond.object.amt3[1] = ldc_amt3
if ldc_amt4 <> 0 then dw_cond.object.amt4[1] = ldc_amt4
if ldc_amt5 <> 0 then dw_cond.object.amt5[1] = ldc_amt5
if ldc_amt6 <> 0 then dw_cond.object.amt6[1] = ldc_amt6
//END BY HMK
...
FOR ll = 1 to ll_row
	...
	ll_priority = ldw.object.priority[ll]
	IF ls_chk = "Y" AND ls_onefee = "Y" AND ls_bil ="N" THEN
		
		ldc_unitamt  	  = ldw.object.unit_amt[ll] * -1
		ldc_unititem 	  = ldw.object.unit_item[ll] * -1
		ld_vat_amt         = ldw.object.tax_amt[ll] * -1
		
		if ldc_unitamt = 0 then continue
		
		IF ldc_unitamt < ldc_unititem THEN
			ldc_unitamt = ldc_unititem
		END IF		
...
// 2019.04.24 surtaxyn 추가
		SELECT DECODE(surtaxyn,'N','*',' '), FNC_GET_TAXRATE(:is_customerid, 'I', :ls_itemcod, :ldt_paydt)
		  INTO :ls_surtaxyn                , :ld_taxrate
		  FROM itemmst
		 WHERE itemcod = :ls_itemcod;

		//ld_vat_amt = truncate(ldc_unitamt * ld_taxrate / 100,2)
		IF ll_cnt > 0 THEN
			idc_deposit = idc_deposit + ldc_unitamt
		END IF

		If IsNull(ldc_unitamt) then ldc_unitamt = 0
		//IF ldc_unitamt <> 0 then // 0 원이어도 수납취소 되도록 2015-04-10
...
// 신청일 : orderdt 기준으로 14일 이후로는 환불처리 불가능
// RQ-UBS-201303-08 의해 수정함.
// BY HMK		
STRING 	ls_orderdt, ls_order_14

select  to_char(orderdt,'yyyymmdd'), to_char(orderdt  + 14 , 'yyyymmdd')  
into : ls_orderdt, :ls_order_14
from svcorder 
  where orderno = :is_orderno;

IF string(ldt_paydt,'yyyymmdd') > ls_order_14 THEN
	f_msg_usr_err(9000, Title, "신청일 이후 14일이 지나면 처리할 수 없습니다. Pay Date 를 확인하세요!")
	dw_cond.SetFocus()
	dw_cond.SetRow(1)
...
String 	ls_itemcod,			ls_paydt,		ls_customerid, ls_memberid, &
			ls_shop_closedt,	ls_operator,   ls_orderno
...
// 신청일 : orderdt , 요청일 : reqdt 어느일자 기준으로 14일 이후로는 환불처리 불가능
// RQ-UBS-201303-08 의해 수정함.
// BY HMK		
STRING 	ls_orderdt, ls_order_14
...
IF idc_deposit < 0 THEN
	...
	SELECT PAYSEQ, 'O', PAYDT, SHOPID, OPERATOR,
			 CUSTOMERID, 0, ITEMCOD, PAYMETHOD, REGCOD,
			 PAYAMT, BASECOD, PAYCNT, PAYID, REMARK,
...
type dw_cond from w_a_reg_m`dw_cond within b1w_reg_directpaycan_pop_sams
integer x = 46
integer y = 40
integer width = 2981
integer height = 596
...
event dw_cond::itemchanged;STRING	ls_paydt_1, ls_sysdate, ls_empnm, ls_paydt, ls_paydt_c
...
choose case dwo.name
	...
	CASE "paydt"
		ldt_paydt = F_FIND_SHOP_CLOSEDT(GS_SHOPID)  //샵별 최근 마감일
...
type p_ok from w_a_reg_m`p_ok within b1w_reg_directpaycan_pop_sams
...
type p_close from w_a_reg_m`p_close within b1w_reg_directpaycan_pop_sams
integer x = 3058
integer y = 180
boolean originalsize = false
end type

type gb_cond from w_a_reg_m`gb_cond within b1w_reg_directpaycan_pop_sams
integer width = 3013
integer height = 644
end type

type p_delete from w_a_reg_m`p_delete within b1w_reg_directpaycan_pop_sams
boolean visible = false
integer x = 1010
integer y = 1884
end type

type p_insert from w_a_reg_m`p_insert within b1w_reg_directpaycan_pop_sams
boolean visible = false
integer x = 709
integer y = 1884
end type

type p_save from w_a_reg_m`p_save within b1w_reg_directpaycan_pop_sams
integer x = 50
integer y = 1884
end type

type dw_detail from w_a_reg_m`dw_detail within b1w_reg_directpaycan_pop_sams
boolean visible = false
integer x = 23
integer y = 1828
integer width = 3013
...
type p_reset from w_a_reg_m`p_reset within b1w_reg_directpaycan_pop_sams
integer x = 352
integer y = 1884
...
integer taborder = 11
string title = "none"
string dataobject = "b1dw_reg_directitem_can_pop_sams"
