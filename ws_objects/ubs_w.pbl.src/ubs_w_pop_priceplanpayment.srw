$PBExportHeader$ubs_w_pop_priceplanpayment.srw
$PBExportComments$[jhchoi] 가격정책 변경 수납 화면 - 2009.06.14
forward
global type ubs_w_pop_priceplanpayment from w_a_reg_m
end type
type dw_detail2 from datawindow within ubs_w_pop_priceplanpayment
end type
end forward
...
global ubs_w_pop_priceplanpayment ubs_w_pop_priceplanpayment
...
on ubs_w_pop_priceplanpayment.create
int iCurrent
call super::create
...
on ubs_w_pop_priceplanpayment.destroy
call super::destroy
...
-------------------------------------------------------------------------*/
//Integer i
String  ls_itemcod, 		ls_itemnm, 		ls_customernm
String  ls_itemcod_old, ls_itemnm_old

datawindow ldw,	ldw_old
...
dw_cond.InsertRow(0)
//
Long		ll_row, ll, row, ll_cnt, ll_priority, ll_priority_old, ll_row_old, ll_cnt_old
String	ls_ref_desc, ls_temp, ls_reqdt, ls_regcod
String	ls_reqdt_old, ls_regcod_old

DEC{2}	ldc_unitamt, ldc_unitamt_old
date 		ldt_paydt
...
is_reqdt_old  		= iu_cust_msg.is_data[3]
is_reqdt				= iu_cust_msg.is_data[4]
is_priceplan_old	= iu_cust_msg.is_data[5]
is_priceplan		= iu_cust_msg.is_data[6]
...
String ls_chk_old,	ls_onefee_old, ls_bil_old


ldw_old 	= iu_cust_msg.idw_data[1]
ldw  		= iu_cust_msg.idw_data[2]

ll_row_old		= ldw_old.RowCount()
ll_row 			= ldw.RowCount()
...
//기존 가격정책 item 값 입력.
FOR ll = 1 to ll_row_old
	ls_chk_old 			= Trim(ldw_old.object.chk[ll])
	ls_onefee_old 		= Trim(ldw_old.object.oneoffcharge_yn[ll])
	ls_bil_old 			= Trim(ldw_old.object.bilitem_yn[ll])
	ls_itemcod_old		= Trim(ldw_old.object.itemcod[ll])
	ls_itemnm_old 		= Trim(ldw_old.object.itemnm[ll])
	ll_priority_old	= ldw_old.object.priority[ll]
	IF ls_chk_old = "Y" AND ls_onefee_old = "Y" AND ls_bil_old ="N" THEN
	
		select unitcharge  INto :ldc_unitamt_old
  		from   priceplan_rate2
 		where  priceplan = :is_priceplan_old
		and    itemcod   = :ls_itemcod_old
  		and	 to_char(fromdt, 'yyyymmdd') = ( select MAX(to_char(fromdt, 'yyyymmdd'))
		  													FROM   priceplan_rate2
														   WHERE  priceplan = :is_priceplan_old
														   AND    itemcod	  = :ls_itemcod_old
														   AND    to_char(fromdt, 'yyyymmdd') <= :is_reqdt_old ) ;
		...
		SELECT COUNT(*) INTO :ll_cnt_old
		FROM   DEPOSIT_REFUND
		WHERE ( IN_ITEM = :ls_itemcod_old OR OUT_ITEM = :ls_itemcod_old );
		
		IF ll_cnt_old > 0 THEN
			idc_deposit = idc_deposit + (ldc_unitamt_old * -1)
		END IF

		If IsNull(ldc_unitamt_old) then ldc_unitamt_old = 0
		IF ldc_unitamt_old > 0 then
			row = dw_detail2.InsertRow(0)
			dw_detail2.Object.itemcod[row] 	= ls_itemcod_old
			dw_detail2.Object.contno[row] 	= ""
			dw_detail2.Object.modelno[row] 	= ""
			dw_detail2.Object.remark[row] 	= ""
			dw_detail2.Object.qty[row] 		= -1
			dw_detail2.Object.itemnm[row] 	= ls_itemnm_old
			dw_detail2.Object.sale_amt[row] 	= ldc_unitamt_old * -1
			dw_detail2.Object.paycnt[row] 	= -1
			dw_detail2.Object.priority[row] 	= ll_priority_old
			
			IF ll_cnt_old <= 0 THEN    // DEPOSIT 항목임
				dw_detail2.Object.impack_card[row] 	= (Round(ldc_unitamt_old * 0.1, 2)) * -1
				dw_detail2.Object.impack_not[row] 	= (Round(ldc_unitamt_old - Round(ldc_unitamt_old * 0.1, 2), 2)) * -1
			ELSE
				dw_detail2.Object.impack_card[row] 	= 0
				dw_detail2.Object.impack_not[row] 	= ldc_unitamt_old * -1
			END IF
			
			idc_total += (ldc_unitamt_old * -1)
			
			select regcod into :ls_regcod_old from itemmst
			 where itemcod = :ls_itemcod_old ;
			 
			If IsNull(ls_regcod_old) then ls_regcod_old = ""
			dw_detail2.Object.regcod[row] 	= ls_regcod_old
...
type dw_cond from w_a_reg_m`dw_cond within ubs_w_pop_priceplanpayment
integer x = 46
integer y = 40
integer width = 2981
integer height = 596
string dataobject = "ubs_dw_pop_priceplanpayment_cnd"
...
type p_ok from w_a_reg_m`p_ok within ubs_w_pop_priceplanpayment
...
type p_close from w_a_reg_m`p_close within ubs_w_pop_priceplanpayment
integer x = 3058
integer y = 180
boolean originalsize = false
end type

type gb_cond from w_a_reg_m`gb_cond within ubs_w_pop_priceplanpayment
integer width = 3013
integer height = 644
end type

type p_delete from w_a_reg_m`p_delete within ubs_w_pop_priceplanpayment
boolean visible = false
integer x = 1010
integer y = 1884
end type

type p_insert from w_a_reg_m`p_insert within ubs_w_pop_priceplanpayment
boolean visible = false
integer x = 709
integer y = 1884
end type

type p_save from w_a_reg_m`p_save within ubs_w_pop_priceplanpayment
integer x = 50
integer y = 1884
end type

type dw_detail from w_a_reg_m`dw_detail within ubs_w_pop_priceplanpayment
boolean visible = false
integer x = 23
integer y = 1828
integer width = 3013
...
type p_reset from w_a_reg_m`p_reset within ubs_w_pop_priceplanpayment
integer x = 352
integer y = 1884
...
integer taborder = 11
string title = "none"
string dataobject = "ubs_dw_pop_priceplanpayment_det"
