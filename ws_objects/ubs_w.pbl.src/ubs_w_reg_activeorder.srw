$PBExportHeader$ubs_w_reg_activeorder.srw
$PBExportComments$[jhchoi] 서비스 신규신청 - 2009.05.06
forward
global type ubs_w_reg_activeorder from w_a_reg_m_m3
end type
type uo_1 from u_calendar_sams within ubs_w_reg_activeorder
end type
end forward
...
global ubs_w_reg_activeorder ubs_w_reg_activeorder
...
String is_partner_cus_yn, is_date_allow_yn, is_amt_check
...
on ubs_w_reg_activeorder.create
int iCurrent
call super::create
...
on ubs_w_reg_activeorder.destroy
call super::destroy
destroy(this.uo_1)
end on
...
String 	ls_svccod, 		ls_priceplan, 		ls_customerid, &
			...
			ls_selfequip,	ls_macaddr,			ls_macaddr2
...
ls_macaddr		 = Trim(dw_cond.object.mac_addr[1])
ls_macaddr2		 = Trim(dw_cond.object.mac_addr2[1])
...
If IsNull(ls_selfequip) 		Then ls_selfequip 		= ""
If IsNull(ls_macaddr) 			Then ls_macaddr 			= ""
If IsNull(ls_macaddr2) 			Then ls_macaddr2 			= ""

//자급장비 일 경우 장비가 있는 장비인지 체크함!
IF ls_selfequip = "Y" THEN
	IF ls_macaddr = "" THEN
		f_msg_info(200, Title, "MAC Address")
		dw_cond.SetFocus()
		dw_cond.SetColumn("mac_addr")
		Return		
	END IF
	
	//판매장비인지 확인. 장비 있는 건지도 확인.
	SELECT COUNT(*), 
			 NVL(SUM(DECODE(STATUS, '300', 1, '600', 1, '700', 1, '999', 1, 0)), 0)
   INTO   :ll_cnt, :ll_status_cnt
	FROM   EQUIPMST
	WHERE  MAC_ADDR = LOWER(:ls_macaddr)
	AND    SALE_FLAG = '1';
	
	IF ll_cnt <= 0 THEN
		f_msg_info(200, Title, "존재하지 않는 장비입니다.")
		dw_cond.SetFocus()
		dw_cond.SetColumn("mac_addr")
		Return
	ELSE
		IF ll_status_cnt > 0 THEN
			f_msg_info(200, Title, "존재는 하나 사용할 수 없는 상태입니다.")
			dw_cond.SetFocus()
			dw_cond.SetColumn("mac_addr")
			Return
		END IF			
	...
	IF ls_macaddr2 <> "" THEN
	   ...
	   INTO   :ll_cnt, :ll_status_cnt
		...
		IF ll_cnt <= 0 THEN
			f_msg_info(200, Title, "존재하지 않는 장비입니다.(MAC_Address2)")
			dw_cond.SetFocus()
			dw_cond.SetColumn("mac_addr2")
			Return
		ELSE
			IF ll_status_cnt > 0 THEN
				f_msg_info(200, Title, "존재는 하나 사용할 수 없는 상태입니다.")
...
//2009.06.24 Hot Bill 이력이 있으면 개통 불가능하도록 막아달라. 김소연 주임 요청
//2009.09.02 전체 핫빌한 고객만 막아달라. 이윤주 대리 요청
...
//IF is_inout_svc_gu = 'N' Then
dw_detail.dataObject = "ubs_dw_reg_activeorder_det"
...
If ll_row >= 0 Then
	...
	for i_chk = 1 to ll_row
		if dw_detail2.object.grouptype[i_chk] = 9 then
			dw_detail2.object.chk[i_chk] = 'N'
		end if	
	next
	//2013-06-13 BY HMK 
...
If il_validkey_cnt > 0 Then
	...
	THIS.TRIGGER EVENT RESIZE(2, workspacewidth(), workspaceheight())
...
//	If ll_row = 0 Then Return 0
	...
	End If
	//할부 품목 신청 했을 때 팝업 띄우기...end
	
	//임대 품목 신청 했을 때 팝업 띄우기
...
//	If ll_row = 0 Then Return 0
	...
	End If
	//임대 품목 신청 했을 때 팝업 띄우기...end	
	
	//ICP, USA PLAN 선택시 카드 선택할 수 있는 팝업...
	...
	IF ll_item_cnt > 0 then
		...
		iu_cust_msg.is_data2[6] = ls_reqdt              		 //req date
		 ...
		 OpenWithParm(b1w_reg_admst_pop_new, iu_cust_msg)
...
//	If ll_row = 0 Then Return 0
	...
	For i = 1 To ll_row
		...
		ls_onefee 	= Trim(dw_detail2.object.ONEOFFCHARGE_YN[i])
		ls_bil 		= Trim(dw_detail2.object.bilitem_yn[i])
		ls_itemcod  = Trim(dw_detail2.object.itemcod[i])
...
//2014.10.30일 막음.. WAM9014일감 참고 start by HMK 
//0원일때 즉시불 취소창 뜨지 않으면 dailypayment에 데이터 쌓이지 않으므로 풀어달라는 요청에 의해서.
//	IF ldc_total <= 0 THEN
//		lb_direct = False
//	END IF
//2014.10.30일 막음.. WAM9014일감 참고 end by HMK 
...
//2009.06.10 위로 올림...서비스 저장 전에 수납 확인 하기 위해서!
////=========================================
////즉시불 처리 항목 여부 check
//// bilitem_yn = 'N' AND oneoffcharge_yn = 'Y'
////=========================================
...
If il_validkey_cnt > 0 Then
	...
	IF ll_row > 1 THEN
		FOR ii = 1 TO ll_row 
			ls_validkey = dw_detail.Object.validkey[ii]
			
			IF ls_validkey = "" OR IsNull(ls_validkey) THEN
				dw_detail.DeleteRow(ii)
			END IF
		NEXT
	END IF		
End if
...
//	dw_detail.Height = 0
	IF dw_detail.Visible = TRUE THEN
		dw_detail2.height = newheight - 152 - iu_cust_w_resize.ii_dw_button_space - iu_cust_w_resize.ii_button_space - iu_cust_w_resize.ii_dw_button_space
		st_horizontal.Y = newheight - 152 - iu_cust_w_resize.ii_dw_button_space - iu_cust_w_resize.ii_button_space - iu_cust_w_resize.ii_dw_button_space
		dw_detail.height = 152		
	ELSE
		dw_detail2.height = newheight - iu_cust_w_resize.ii_button_space - iu_cust_w_resize.ii_dw_button_space		
		st_horizontal.Y = 1836
	END IF		
...
Else
	st_horizontal.Y = newheight - 152 - iu_cust_w_resize.ii_button_space - iu_cust_w_resize.ii_dw_button_space
//	st_horizontal.Y = newheight - dw_detail.height - iu_cust_w_resize.ii_button_space - iu_cust_w_resize.ii_dw_button_space	
...
//IF messagebox("확인", "선택", Question!, YesNo!, 1) = 1 THEN
//	OpenWithParm(b1w_reg_svcorder_validinfo_pop_v20_sams, iu_cust_msg)
//ELSE
	OpenWithParm(ubs_w_pop_validkey, iu_cust_msg)
//END IF
end event

type dw_cond from w_a_reg_m_m3`dw_cond within ubs_w_reg_activeorder
integer x = 32
integer y = 20
integer width = 2674
integer height = 676
integer taborder = 10
string dataobject = "ubs_dw_reg_activeorder_cond"
...
type p_ok from w_a_reg_m_m3`p_ok within ubs_w_reg_activeorder
integer x = 3662
end type

type p_close from w_a_reg_m_m3`p_close within ubs_w_reg_activeorder
integer x = 1701
integer y = 2076
boolean originalsize = false
end type

type gb_cond from w_a_reg_m_m3`gb_cond within ubs_w_reg_activeorder
integer x = 23
integer width = 2697
integer height = 712
integer taborder = 0
...
type dw_master from w_a_reg_m_m3`dw_master within ubs_w_reg_activeorder
...
type dw_detail from w_a_reg_m_m3`dw_detail within ubs_w_reg_activeorder
integer x = 23
integer y = 1900
integer width = 3611
integer height = 152
integer taborder = 0
string dataobject = "ubs_dw_reg_activeorder_det"
end type
...
type p_insert from w_a_reg_m_m3`p_insert within ubs_w_reg_activeorder
integer x = 23
integer y = 2076
end type

type p_delete from w_a_reg_m_m3`p_delete within ubs_w_reg_activeorder
integer x = 329
integer y = 2076
end type

type p_save from w_a_reg_m_m3`p_save within ubs_w_reg_activeorder
integer x = 635
integer y = 2076
end type

type p_reset from w_a_reg_m_m3`p_reset within ubs_w_reg_activeorder
integer x = 1390
integer y = 2076
end type

type dw_detail2 from w_a_reg_m_m3`dw_detail2 within ubs_w_reg_activeorder
integer x = 23
integer y = 736
integer width = 3616
integer height = 1140
integer taborder = 0
string dataobject = "ubs_dw_reg_activeorder_mas"
boolean hscrollbar = false
...
type st_horizontal2 from w_a_reg_m_m3`st_horizontal2 within ubs_w_reg_activeorder
integer x = 18
integer y = 712
integer height = 20
integer textsize = -3
...
type st_horizontal from w_a_reg_m_m3`st_horizontal within ubs_w_reg_activeorder
boolean visible = false
integer x = 23
integer y = 1876
integer height = 20
