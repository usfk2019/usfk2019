$PBExportHeader$ubs_w_reg_hotbill_contract.srw
$PBExportComments$[jhchoi]  Hot Bill 계산 ( 부분핫빌 가능 ) - 2009.06.28
forward
global type ubs_w_reg_hotbill_contract from w_a_reg_s
end type
type p_cancel from u_p_cancel within ubs_w_reg_hotbill_contract
end type
type cb_hotbill from commandbutton within ubs_w_reg_hotbill_contract
end type
type dw_split from datawindow within ubs_w_reg_hotbill_contract
end type
type hpb_1 from hprogressbar within ubs_w_reg_hotbill_contract
end type
type cb_1 from commandbutton within ubs_w_reg_hotbill_contract
end type
type dw_others from datawindow within ubs_w_reg_hotbill_contract
end type
type p_1 from picture within ubs_w_reg_hotbill_contract
end type
type cb_deposit from commandbutton within ubs_w_reg_hotbill_contract
end type
type gb_3 from groupbox within ubs_w_reg_hotbill_contract
end type
end forward
...
global ubs_w_reg_hotbill_contract ubs_w_reg_hotbill_contract
...
//위에 쿼리가 문제 있어서. HOTSALE 에서 가져오면 HOTSALE 에 없을때는 DEPOSIT 도 REFUND 안됨. CJH 20090906
//SELECT COUNT(*) INTO :ll_term_cnt
//FROM   CUSTOMERM A, CONTRACTMST B
//WHERE  A.PAYID = :is_payid
//AND    A.CUSTOMERID = B.CUSTOMERID
...
//	SELECT B.CONTRACTSEQ INTO :ls_contractseq
//	FROM   CUSTOMERM A, CONTRACTMST B
//	WHERE  A.PAYID = :is_payid
//	AND    A.CUSTOMERID = B.CUSTOMERID
//	AND    B.BILL_HOTBILLFLAG = 'P';	
...
ls_trcod0[5] 		= dw_cond.object.trcod6[1]
...
ELSE
	FOR ll = 1 to ll_cnt
		...
		//-------------------------------------------------------------------------
		IF ldc_impack > 0 THEN
			IF ldc_im <> 0 THEN   //임팩카드 처리 부분 수정 2011.02.15 JHCHOI. IF ldc_im > 0 THEN 
				IF li_first 	=  0 then
					li_paycnt 	= 1
					li_first 	= 1
				else
...
on ubs_w_reg_hotbill_contract.create
int iCurrent
call super::create
...
on ubs_w_reg_hotbill_contract.destroy
call super::destroy
...
type dw_cond from w_a_reg_s`dw_cond within ubs_w_reg_hotbill_contract
integer width = 2455
integer height = 536
...
type p_ok from w_a_reg_s`p_ok within ubs_w_reg_hotbill_contract
boolean visible = false
integer x = 2619
integer y = 252
end type

type p_close from w_a_reg_s`p_close within ubs_w_reg_hotbill_contract
integer x = 1271
integer y = 1852
boolean originalsize = false
end type

type gb_cond from w_a_reg_s`gb_cond within ubs_w_reg_hotbill_contract
integer width = 2478
integer height = 588
end type

type dw_detail from w_a_reg_s`dw_detail within ubs_w_reg_hotbill_contract
integer x = 50
integer y = 608
integer width = 2455
integer height = 832
...
type p_delete from w_a_reg_s`p_delete within ubs_w_reg_hotbill_contract
boolean visible = false
integer x = 471
integer y = 1856
end type

type p_insert from w_a_reg_s`p_insert within ubs_w_reg_hotbill_contract
integer x = 50
integer y = 1852
end type

type p_save from w_a_reg_s`p_save within ubs_w_reg_hotbill_contract
integer x = 352
integer y = 1852
end type

type p_reset from w_a_reg_s`p_reset within ubs_w_reg_hotbill_contract
integer x = 965
integer y = 1852
end type
...
LONG		ll_row,				ll_check,		ll_max_seq,		ll_ret_row,		ll_ret_cnt,		ll,			&
			ll_deposit_cnt
DATE		ldt_paydt,			ldt_trdt
DEC{2}	ldc_sale_amt,		ldc_tramt,		ldc_tot, ldc_sum_tramt
Window 	lw_help
