$PBExportHeader$ubs_w_reg_hotreqdtl_others_new.srw
$PBExportComments$[jhchoi]  Hot Bill 계산 ( 부분핫빌 가능 ) - 2009.06.28
forward
global type ubs_w_reg_hotreqdtl_others_new from w_a_reg_s
end type
type p_cancel from u_p_cancel within ubs_w_reg_hotreqdtl_others_new
end type
type cb_hotbill from commandbutton within ubs_w_reg_hotreqdtl_others_new
end type
type dw_split from datawindow within ubs_w_reg_hotreqdtl_others_new
end type
type hpb_1 from hprogressbar within ubs_w_reg_hotreqdtl_others_new
end type
type cb_1 from commandbutton within ubs_w_reg_hotreqdtl_others_new
end type
type dw_others from datawindow within ubs_w_reg_hotreqdtl_others_new
end type
type p_1 from picture within ubs_w_reg_hotreqdtl_others_new
end type
type cb_deposit from commandbutton within ubs_w_reg_hotreqdtl_others_new
end type
type gb_3 from groupbox within ubs_w_reg_hotreqdtl_others_new
end type
end forward
...
global ubs_w_reg_hotreqdtl_others_new ubs_w_reg_hotreqdtl_others_new
...
WHERE  CONTRACTSEQ IN ( SELECT B.CONTRACTSEQ
								...
								AND    B.BILL_HOTBILLFLAG = 'S' )
AND    BILL_HOTBILLFLAG = 'S';
...
public function integer wf_non_payment ();STRING	ls_trcod,	ls_trcodnm
DATE		ldt_trdt
DEC{2}	ldc_tramt
LONG		ll_row
...
Long		ll_row,			ll_max_seq,		ll_count = 0,		ll_in_cnt
DEC{2}	ldc_in_amt,		ldc_in_amtH, &
			ldc_out_amt,	ldc_out_amtH, &
			ldc_deposit, ldc_refund, ldc_remaind
...
IF ll_in_cnt > 0 THEN
	...
	SELECT ROWNUM + :ll_max_seq,				:is_reqnum,							:is_payid,			to_date(:ls_trdt, 'yyyymmdd'), 
			 ...
			 X.TRCOD,								(X.PAYAMT + X.OUT_AMT) * -1,	0,						:is_termdt,
 	  		 SYSDATE, 								:gs_user_id,	 					SYSDATE, 			:gs_user_id, 							:gs_pgm_id[gi_open_win_no]
	FROM ( SELECT A.ITEMCOD, B.IN_ITEM, B.OUT_ITEM, A.PAYAMT, E.TRCOD,
					  SUM(NVL(( SELECT C.PAYAMT
									FROM   DAILYPAYMENT C
									WHERE  C.CUSTOMERID = A.CUSTOMERID
									AND    C.ITEMCOD = B.OUT_ITEM
									UNION ALL
									SELECT D.PAYAMT
			 ...
			 FROM ( SELECT CUSTOMERID, ITEMCOD, SUM(PAYAMT) AS PAYAMT
					  FROM   DAILYPAYMENT
					  WHERE  CUSTOMERID IN ( SELECT CUSTOMERID FROM CUSTOMERM WHERE PAYID = :is_payid )
					  GROUP BY CUSTOMERID, ITEMCOD
					  UNION ALL
					  SELECT CUSTOMERID, ITEMCOD, SUM(PAYAMT) AS PAYAMT
					  FROM   DAILYPAYMENTH
					  WHERE  CUSTOMERID IN ( SELECT CUSTOMERID FROM CUSTOMERM WHERE PAYID = :is_payid )
					  GROUP BY CUSTOMERID, ITEMCOD ) A, DEPOSIT_REFUND B, ITEMMST E 
			 WHERE  A.ITEMCOD = B.IN_ITEM
			 AND    B.OUT_ITEM = E.ITEMCOD
			 GROUP BY A.ITEMCOD, B.IN_ITEM, B.OUT_ITEM, A.PAYAMT, E.TRCOD ) X
	WHERE  X.PAYAMT <> 0
...
SELECT SUM(TRAMT) INTO :ldc_in_amt
FROM   PREPAYDET
WHERE  PAYID = :is_payid;
...
IF ldc_remaind <> 0 THEN
   ...
   WHERE  ITEMCOD = :ls_out_item ;
	...
	INSERT INTO HOTREQDTL (	reqnum, 		seq, 				payid, 		trdt, 			
									paydt,											transdt,		
									trcod, 		tramt, 			adjamt, 		remark,
									crtdt, 		crt_user, 		updtdt, 		updt_user, 			pgm_id)
...
on ubs_w_reg_hotreqdtl_others_new.create
int iCurrent
call super::create
...
this.Control[iCurrent+6]=this.dw_others
this.Control[iCurrent+7]=this.p_1
this.Control[iCurrent+8]=this.cb_deposit
this.Control[iCurrent+9]=this.gb_3
end on

on ubs_w_reg_hotreqdtl_others_new.destroy
call super::destroy
...
type dw_cond from w_a_reg_s`dw_cond within ubs_w_reg_hotreqdtl_others_new
integer width = 2455
integer height = 536
...
type p_ok from w_a_reg_s`p_ok within ubs_w_reg_hotreqdtl_others_new
boolean visible = false
integer x = 2619
integer y = 252
end type

type p_close from w_a_reg_s`p_close within ubs_w_reg_hotreqdtl_others_new
integer x = 1271
integer y = 1852
boolean originalsize = false
end type

type gb_cond from w_a_reg_s`gb_cond within ubs_w_reg_hotreqdtl_others_new
integer width = 2478
integer height = 588
end type

type dw_detail from w_a_reg_s`dw_detail within ubs_w_reg_hotreqdtl_others_new
integer x = 50
integer y = 608
integer width = 2455
integer height = 832
...
type p_delete from w_a_reg_s`p_delete within ubs_w_reg_hotreqdtl_others_new
boolean visible = false
integer x = 416
integer y = 1848
end type

type p_insert from w_a_reg_s`p_insert within ubs_w_reg_hotreqdtl_others_new
integer x = 50
integer y = 1852
end type

type p_save from w_a_reg_s`p_save within ubs_w_reg_hotreqdtl_others_new
integer x = 352
integer y = 1852
end type

type p_reset from w_a_reg_s`p_reset within ubs_w_reg_hotreqdtl_others_new
integer x = 965
integer y = 1852
end type
...
STRING 	ls_type,				ls_name,			ls_paydt,		ls_payid,		ls_trcod,		ls_trdt,		&
			ls_itemcod,			ls_itemnm,		ls_regcod
LONG		ll_row,				ll_check,		ll_max_seq,		ll_ret_row,		ll_ret_cnt,		ll,			&
			ll_deposit_cnt
DATE		ldt_paydt,			ldt_trdt
DEC{2}	ldc_sale_amt,		ldc_tramt,		ldc_tot
Window 	lw_help
	...
	CASE 'b_add'
		...
		IF ll_check = 1 THEN
			...
			Values(
					:is_reqnum, :ll_max_seq, 	:is_payid, 	to_date(:ls_trdt, 'yyyymmdd'), 
					to_date(:ls_paydt, 'yyyymmdd'),			to_date(:ls_paydt, 'yyyymmdd'), 
					:ls_trcod, 	:ldc_sale_amt,	0, 			:is_termdt,
					sysdate, 	:gs_user_id,	 sysdate, 	:gs_user_id, 		:gs_pgm_id[gi_open_win_no] );
...
String 	ls_errmsg,	ls_pgm_id,		ls_chargedt, 	ls_hotbillingflag
String 	ls_reqdt, 	ls_where, 		ls_reqdt_add
Date 		ld_reqdt, 	ld_reqdt_next, ldt_trdt,		ld_hot_date
integer 	li_day,		li_progress
Long 		ll_return, 	ll_row,			LL,		ll_cnt,	ll_deposit_cnt,		ll_bill_cnt
...
SELECT ADD_MONTHS(TO_DATE(TO_CHAR(SYSDATE, 'YYYYMM')||'01', 'YYYYMMDD'), 1) INTO :ld_hot_date
FROM   DUAL;

SELECT COUNT(*) INTO :ll_bill_cnt
FROM   HOTREQDTL
WHERE  PAYID = :is_payid
AND    TRDT = :ld_hot_date;
