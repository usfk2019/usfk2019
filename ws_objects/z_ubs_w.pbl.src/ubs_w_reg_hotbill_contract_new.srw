$PBExportHeader$ubs_w_reg_hotbill_contract_new.srw
$PBExportComments$[jhchoi]  Hot Bill 계산 ( 부분핫빌 가능 ) - 2009.06.28
forward
global type ubs_w_reg_hotbill_contract_new from w_a_reg_s
end type
type gb_3 from groupbox within ubs_w_reg_hotbill_contract_new
end type
type p_cancel from u_p_cancel within ubs_w_reg_hotbill_contract_new
end type
type cb_hotbill from commandbutton within ubs_w_reg_hotbill_contract_new
end type
type dw_split from datawindow within ubs_w_reg_hotbill_contract_new
end type
type hpb_1 from hprogressbar within ubs_w_reg_hotbill_contract_new
end type
type cb_1 from commandbutton within ubs_w_reg_hotbill_contract_new
end type
type dw_others from datawindow within ubs_w_reg_hotbill_contract_new
end type
type p_1 from picture within ubs_w_reg_hotbill_contract_new
end type
type cb_deposit from commandbutton within ubs_w_reg_hotbill_contract_new
end type
end forward
...
global ubs_w_reg_hotbill_contract_new ubs_w_reg_hotbill_contract_new
...
DEC{2}   ldc_taxrow_tot, ldc_tax_tot
...
//ldc_vat_tot = dw_detail.object.cp_vat[1] //부가세 총합
	...
	FOR ll = 1 to ll_cnt
		...
		IF ls_end = 'N' THEN
			FOR li_pp =  li_pay_cnt to ii_method_cnt
				...
				IF ldc_rem > 0 THEN
					IF ldc_rem - (ldc_tramt) <= 0 THEN			//수납금액에서 ITEM 제외 금액이 0 이하이면 다음 수납유형
						ldc_tax_dec = ldc_rem - ( ldc_rem/ld_tax_rate)
						li_tax_dec = integer(left(right(string(ldc_tax_dec),2),1))  //소수점 둘째자리 
...
//						end if
				...
				ELSEIF ldc_rem < 0 THEN								//수납유형에 - 금액이면
					IF ldc_rem - (ldc_tramt) >= 0 THEN			//수납금액에서 ITEM 제외 금액이 0 이상이면 다음 수납유형
						ldc_tax_dec = ldc_rem - ( ldc_rem/ld_tax_rate)
						li_tax_dec =integer(left(right(string(ldc_tax_dec),2),1))
          ...
          AND   I.REGCOD   = R.REGCOD;
			...
			values 
				( :ls_customerid , seq_prepaydet.nextval, 'O'         , :ldt_paydt , :ldt_trdt    , 
				  :ls_trcod      , :ldc_saleamt        , 'N'         , null       , :ls_itemcod  ,
...
//		IF jj = 1 then 
			//ldc_shopCount = f_pos_header(GS_SHOPID, 'A', LL_SHOPCOUNT, 1 )
			ldc_shopCount = f_pos_header_vat(GS_SHOPID, 'A', LL_SHOPCOUNT, 1 )
//		ELSE 
//			ldc_shopCount = f_pos_header(GS_SHOPID,  'Z', LL_SHOPCOUNT, 0 )
//		END IF
...
// 2019.04.08 Refund금액도 net + vat로 변경 modified by han
			...
			f_printpos_vat(' ' + ls_temp)		//f_printpos(ls_temp)				
...
//		Fs_POS_FOOTER2(ls_payid,ls_customerid,ls_appseq,gs_user_id) // 2019.04.08 Approval No 수정 modified by Han
		Fs_POS_FOOTER3_VAT(ls_payid,ls_customerid,ls_appseq,gs_user_id, ls_paydt) // 2019.04.08 Approval No 수정 및 Pay Date Arg 추가 modified by Han
...
INT		i
DEC{2}	ld_tax_rate, 	ldc_saleamt,	ld_taxamt, 	ldc_saleamt_mob,	ld_taxamt_mob
	...
	IF ll_in_cnt > 0 THEN
		...
		SELECT ROWNUM + :ll_max_seq,				:is_reqnum,							:is_payid,			to_date(:ls_trdt, 'yyyymmdd'), 
		   ...
		   FROM ( SELECT A.ITEMCOD, B.IN_ITEM, B.OUT_ITEM, SUM(A.PAYAMT) AS PAYAMT, E.TRCOD,
						  SUM(NVL((SELECT SUM(C.PAYAMT) +NVL(SUM(C.TAXAMT),0)
													  FROM   DAILYPAYMENT C     
													  WHERE  C.CUSTOMERID = A.CUSTOMERID
													  AND    C.ITEMCOD = B.OUT_ITEM ), 0)) +
						  SUM(NVL((SELECT SUM(D.PAYAMT) +NVL(SUM(D.TAXAMT),0)
							  ...
							  FROM ( SELECT CUSTOMERID, ITEMCOD, SUM(PAYAMT) AS PAYAMT
											FROM ( SELECT CUSTOMERID, ITEMCOD, (PAYAMT + nvl(TAXAMT,0)) as PAYAMT
													  FROM   DAILYPAYMENT
													  WHERE  CUSTOMERID IN ( SELECT CUSTOMERID FROM CUSTOMERM WHERE PAYID = :is_payid )
													  UNION ALL
...
//RQ-UBS-201407-13 HOTBILL처리시 계약번호별 보증금계산하여 처리되도록 START by HMK 2014-07-21
		...
		DECLARE READ_DEPOSIT_ITEM2 CURSOR FOR  	
			SELECT X.ITEMCOD, X.IN_ITEM, X.OUT_ITEM, X.PAYAMT, X.TRCOD, X.OUT_AMT
			  FROM ( SELECT A.ITEMCOD, B.IN_ITEM, B.OUT_ITEM, SUM(A.PAYAMT) AS PAYAMT, E.TRCOD,
									  SUM(NVL((SELECT SUM(C.PAYAMT) + NVL(SUM(C.PAYAMT),0)
													  FROM   DAILYPAYMENT C     
													  WHERE  C.CUSTOMERID = A.CUSTOMERID
													  AND CONTRACTSEQ in (SELECT CONTRACTSEQ 
																							  ...
																							  AND    HOTDT = TO_DATE(TO_CHAR(SYSDATE, 'YYYYMMDD'), 'YYYYMMDD'))
													  AND    C.ITEMCOD = B.OUT_ITEM ), 0)) +
									  SUM(NVL((SELECT SUM(D.PAYAMT)  + NVL(SUM(D.PAYAMT),0)
													  FROM   DAILYPAYMENTH D     
													  WHERE  D.CUSTOMERID = A.CUSTOMERID
													  AND CONTRACTSEQ in (SELECT CONTRACTSEQ 
																							  FROM   HOTCONTRACT
										...
										FROM ( SELECT CUSTOMERID, ITEMCOD, (PAYAMT + NVL(TAXAMT,0)) AS PAYAMT
												  ...
												  WHERE  CUSTOMERID IN ( SELECT CUSTOMERID FROM CUSTOMERM WHERE PAYID = :is_payid  )
														 AND CONTRACTSEQ in (SELECT CONTRACTSEQ 
																							  ...
																							  AND    HOTDT = TO_DATE(TO_CHAR(SYSDATE, 'YYYYMMDD'), 'YYYYMMDD'))
													UNION ALL
												  SELECT CUSTOMERID, ITEMCOD, PAYAMT + NVL(TAXAMT,0) AS PAYAMT
												  FROM   DAILYPAYMENTH
												  WHERE  CUSTOMERID IN ( SELECT CUSTOMERID FROM CUSTOMERM WHERE PAYID =  :is_payid  )
														 AND CONTRACTSEQ  in (SELECT CONTRACTSEQ 
																							  FROM   HOTCONTRACT
...
IF ll_last_cont = 0 THEN				//마지막 고객
   ...
   SELECT SUM(DECODE(SUB_ITEMCOD, NULL, TRAMT + NVL(TAXAMT,0),0)), SUM(DECODE(SUB_ITEMCOD, NULL, 0,TRAMT+ NVL(TAXAMT,0)))
	...
	IF ldc_remaind <> 0 then
		...
		//payid별 , 기간별, 아이템별 , 요율 가져온다.  ex) 5월부터 부가세 적용이라, 5월 이전자료에는 부가세 0이 되어야 함..
		SELECT FNC_GET_TAXRATE(:is_payid, 'T', :ls_trcod, to_date(:ls_trdt))  into  :ld_tax_rate from dual;
		if ld_tax_rate <> 0 then  
			//합계액으로 공급가액과 부가세를 나눠주는 공식임.
			ld_tax_rate = (100 / 100) + ( ld_tax_rate / 100)  //1.1
		else 
		...
		ldc_saleamt	 = round(ldc_remaind / ld_tax_rate, 2) // 품목공급금액을 넣는다.
		ld_taxamt    = ldc_remaind - ldc_saleamt          // 부가세
		
		 ldc_saleamt = ldc_saleamt * -1
		 ld_taxamt = ld_taxamt *-1
	   ...
	   Values(
						:is_reqnum, :ll_max_seq, 	:is_payid, 	to_date(:ls_trdt, 'yyyymmdd'), 
						to_date(:ls_paydt, 'yyyymmdd'),			to_date(:ls_paydt, 'yyyymmdd'), 
						:ls_trcod, 	:ldc_saleamt,	0, 			:is_termdt,
				  		sysdate, 	:gs_user_id,	 sysdate, 	:gs_user_id, 		:gs_pgm_id[gi_open_win_no],  :ld_taxamt);
	...
	IF ldc_remaind_mob <> 0 then
		...
		ldc_saleamt_mob	 = round(ldc_remaind_mob / ld_tax_rate, 2) // 품목공급금액을 넣는다.
		ld_taxamt_mob    = ldc_remaind_mob - ldc_saleamt_mob          // 부가세
		
		 ldc_saleamt_mob = ldc_saleamt_mob * -1
		 ld_taxamt_mob = ld_taxamt_mob *-1
	   ...
	   Values(
						:is_reqnum, :ll_max_seq, 	:is_payid, 	to_date(:ls_trdt, 'yyyymmdd'), 
						to_date(:ls_paydt, 'yyyymmdd'),			to_date(:ls_paydt, 'yyyymmdd'), 
						:ls_trcod, 	:ldc_saleamt_mob,	0, 			:is_termdt,
				  		sysdate, 	:gs_user_id,	 sysdate, 	:gs_user_id, 		:gs_pgm_id[gi_open_win_no] , :ld_taxamt_mob);
...
on ubs_w_reg_hotbill_contract_new.create
int iCurrent
call super::create
...
on ubs_w_reg_hotbill_contract_new.destroy
call super::destroy
destroy(this.gb_3)
...
//	END IF	
	...
	WHERE  CONTRACTSEQ IN ( SELECT B.CONTRACTSEQ
									...
									WHERE  A.PAYID = :is_payid
									AND    A.CUSTOMERID = B.CUSTOMERID
									AND    B.BILL_HOTBILLFLAG in ('R', 'P' ))
	AND    BILL_HOTBILLFLAG in ('R', 'P');
...
integer li_ret
li_ret = f_vat_diff_update('HOTBILL', is_payid,' SEQAPP', is_seq_app)
if li_ret < 0 then
...
//		AND    HOTDT = TO_DATE(TO_CHAR(SYSDATE, 'YYYYMMDD'), 'YYYYMMDD');
		...
		WHERE  PAYID = :is_payid
		AND    HOTDT = TO_DATE(TO_CHAR(SYSDATE, 'YYYYMMDD'), 'YYYYMMDD')
		AND   CONTRACTSEQ  IN ( SELECT B.CONTRACTSEQ
									FROM   CUSTOMERM A, CONTRACTMST B
									WHERE  A.PAYID = :is_payid
									AND    A.CUSTOMERID = B.CUSTOMERID
									AND    B.BILL_HOTBILLFLAG in ('R', 'P' ));
...
type dw_cond from w_a_reg_s`dw_cond within ubs_w_reg_hotbill_contract_new
integer width = 2455
integer height = 452
...
type p_ok from w_a_reg_s`p_ok within ubs_w_reg_hotbill_contract_new
boolean visible = false
integer x = 2619
integer y = 252
end type

type p_close from w_a_reg_s`p_close within ubs_w_reg_hotbill_contract_new
integer x = 1271
integer y = 1788
boolean originalsize = false
end type

type gb_cond from w_a_reg_s`gb_cond within ubs_w_reg_hotbill_contract_new
integer width = 2478
integer height = 516
end type

type dw_detail from w_a_reg_s`dw_detail within ubs_w_reg_hotbill_contract_new
integer x = 50
integer y = 528
integer width = 2455
integer height = 832
...
type p_delete from w_a_reg_s`p_delete within ubs_w_reg_hotbill_contract_new
boolean visible = false
integer x = 471
integer y = 1792
end type

type p_insert from w_a_reg_s`p_insert within ubs_w_reg_hotbill_contract_new
integer x = 50
integer y = 1788
end type

type p_save from w_a_reg_s`p_save within ubs_w_reg_hotbill_contract_new
integer x = 352
integer y = 1788
end type

type p_reset from w_a_reg_s`p_reset within ubs_w_reg_hotbill_contract_new
integer x = 965
integer y = 1788
end type
